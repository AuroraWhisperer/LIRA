...
页面覆盖当前直播画面，持续渲染一套与主播形象匹配的动态舞台：背景光晕、唱片环、心拍流光灯轨、人物轻动态、音符、粒子、EQ 和可配置文案。页面没有固定的播放结束状态，主播切换到正式直播场景后，浏览器源自然停止显示。
...
用户提供的主播图像是透明背景 PNG，尺寸约为 `1086×1448`，比例约 `3:4`，包含完整人物、麦克风、音符、蝴蝶结和粉黑配色。它适合在 16:9 画布的右侧作为主视觉，人物伸出的手可以与左侧文案和灯轨形成视觉连接。
...
```text
甜酷歌姬 · 深色录音棚 · 粉色舞台灯 · 唱片 · 心拍 · 轻微动态
```

这套方向保留角色自身的粉色、黑色和音符识别度，不使用泛化的紫色赛博霓虹模板。主动态集中在“唱片轨道”和“麦克风附近”，人物本体只做轻微呼吸和漂浮，保证角色脸部、服装和麦克风始终清晰。

### Color tokens

```text
--opening-night:       #17131B   深紫黑背景
--opening-pink:        #FF5F9A   主高亮粉
--opening-pink-soft:   #FFB5D0   奶油粉
--opening-text:        #FFF7FB   柔白文字
--opening-gold:        #FFD88A   少量金色高光
--opening-shadow:      rgba(8, 5, 12, 0.56)
```

### Layout

设计基准固定为 `1920×1080`，浏览器源改变尺寸时保持整体比例，不将人物和文字分别拉伸。根画布使用 `aspect-ratio: 16 / 9` 居中缩放；不足的边缘区域填充同色深紫黑，不产生变形。
...
│       心拍流光灯轨                         麦克风        │
│                                                         │
│  @主播名                       SINGING LIVE · 记得戴耳机 │
└─────────────────────────────────────────────────────────┘
```

- 人物位于右侧，建议高度为画布的 `86%–90%`，底部对齐。
- 左侧保留约 `38%` 的安静区域放置主标题、状态和主播名。
- 人物伸出的手朝向灯轨终点，灯轨终点再指向麦克风和脸部，形成“文案 → 轨道 → 麦克风 → 人物”的阅读路径。
- 背景光晕、唱片环和 EQ 位于人物后方，透明度低于文字和人物轮廓，不与脸部争夺注意力。
...
| 主要 | 心拍灯轨 | 3.2 秒 | 内部斜纹持续单向移动，轨道长度不变 |
| 环境 | 背景光晕 | 17 秒 | 右上到右下缓慢漂移 |
| 环境 | 音符 | 7/9/11/13 秒 | 3–6 个错开上浮、旋转、淡出 |
| 环境 | EQ | 150–400ms | 程序生成目标高度，平滑过渡 |
| 细节 | 麦克风光点 | 1.5–3 秒生命周期 | 随机间隔 2–5 秒，小范围出现 |
| 细节 | 文字高光 | 4.5 秒 | 只做低对比度扫描，不改变布局 |

### 心拍流光灯轨

灯轨是主题签名元素，但不表示真实时间进度。它使用固定宽度的轨道和重复纹理：
...
纹理使用粉色、柔白和极少量金色，必须通过超宽伪元素的 `transform: translate3d()` 连续移动；不得使用 JS 每帧更新，也不得用 `background-position` 驱动灯轨纹理。伪元素宽度覆盖一个完整纹理周期，移动一个周期后无缝衔接。轨道中偶尔出现一次小心拍波形，随后恢复普通流光，避免持续跳动造成视觉噪声。
...
`.track::before` 使用 `width: calc(100% + 144px)` 和 `transform: translate3d(...)`，其中 `144px` 代表一组完整纹理周期，实际数值由素材密度调整，但必须保持固定且可测量。不得增加承担同一职责的 `.track-flow` DOM 子元素。

如果未来需要真实的“等待倒计时”，另加独立的数值倒计时组件；不能把主题灯轨同时当作语义进度条。
...
`opening.html` 只保留稳定的语义 DOM：背景层、唱片环、人物层、人物 transform wrapper、灯轨、文案、EQ 和粒子容器。不要把大量动态 HTML 作为模板字符串反复重建。

`opening.css` 负责固定布局、颜色令牌、关键帧和画质降级。动画优先使用 `transform`、`opacity`，避免持续改变 `top/left/width/height`；人物变换按 wrapper 分层，心拍灯轨按伪元素 `translate3d()` 实现。
...
第一版不做真实 FFT。EQ 使用定时器生成平滑目标高度即可；以后确实需要音乐同步时，再增加 Web Audio `AnalyserNode`，并保留无音频时的程序动画兜底。
...
```text
high:
  人物、唱片环、灯轨、6 个音符、最多 24 个粒子、16 根 EQ、光晕、高光

normal:
  人物、唱片环、灯轨、4 个音符、最多 12 个粒子、10 根 EQ、基础光晕

low:
  人物呼吸、唱片环、灯轨、文字入场；关闭音符、粒子、EQ、重滤镜
```
...
- `quality=low` 仍保留人物呼吸、唱片和灯轨；`prefers-reduced-motion: reduce` 的优先级更高，会停止人物漂浮/摆动、唱片旋转、灯轨滚动、音符、粒子和高光，只保留不超过 180ms 的必要淡入和静态画面。
...
新增 `opening.html`、`opening.css`、`opening.js` 和已确认的主播 PNG，完成固定 1920×1080 构图、人物层、背景层、文案层、唱片环和灯轨。

验证：Chrome 本地预览无控制台错误，深紫黑背景覆盖整个 16:9 画布，人物不裁头、不裁脚，1920×1080 下文案可读。
...
加入多周期人物、唱片、灯轨、音符、粒子和 EQ；实现 `quality=high/normal/low`、`prefers-reduced-motion` 和页面隐藏暂停。
...
2. 在 1920×1080、30 FPS 下，页面覆盖整个画面，人物和主要文案不被裁切。
3. 页面首次加载有短入场动画，随后进入持续运行的多周期待机动画；不存在“所有元素同时归零”的明显大循环。
4. 人物至少具备呼吸、轻漂浮和轮廓光变化；呼吸、漂浮、摆动分别由嵌套 wrapper 持有自己的 `transform`，不得互相覆盖。
5. 心拍流光灯轨持续向前流动，轨道长度固定；纹理由超宽伪元素 `translate3d()` 无缝移动，不使用 JS 每帧更新。
6. 唱片环、音符、粒子、EQ 和麦克风光点存在数量上限，不会无限创建 DOM 节点。
7. `quality=low` 和 `prefers-reduced-motion: reduce` 能显著减少动态特效，页面仍保持可读和可用。
8. 查询参数文案使用安全的文本渲染，并受到长度限制。
9. 默认 `audio=none` 时视觉动画不依赖音频；MVP 支持的 `audio=browser` 被阻止时正式画面静默降级，只在 `debug=1` 显示诊断。两种音频配置分别验收，外部媒体源互斥由操作者保证。
10. 管理页可以即时编辑标题、副标题、主播名、底部文字、画质、音符和 EQ，并生成不落库的 `openingUrl`。
11. 普通隐藏再显示恢复原动画相位；Browser Source 真正刷新或重新创建时重新执行一次入场动画。
12. 新增功能不改变现有 overlay、WebSocket、IPC、认证、数据库和直播页面契约。
...
- 灯轨由 `.track::before` 超宽伪元素和 `translate3d()` 动画实现，不依赖 JS 每帧更新，也不存在同职责 `.track-flow` 子元素。
...
5. 通过多个不同周期的轻动画保持长期动态感，同时用 low 画质和 reduced-motion 控制直播机资源占用。

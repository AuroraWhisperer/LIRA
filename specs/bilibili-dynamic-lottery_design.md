...
Draft。2026-09-14 已开始 M0/M1 的代码实现：加入合成响应下的读取适配器、会话 fence、独立抽奖库、分页事务和持久请求预算；尚未接入产品路由或界面，也未使用专门主播账号验证上游接口，因此评论、关注、点赞和转发仍不得作为已开放能力。本文是规则与验收的单一来源；[实施计划](plans/2026-09-14-bilibili-dynamic-lottery.md)说明文件、接口、阶段和验证方法，不另行定义规则。
...
- 凭据复用现有 Electron 分区与 `safeStorage`，不扩展明文 Cookie 导出；新的动态功能不改变现有远程凭据同步契约，不能因此宣传整个 LIRA 的凭据从不离开本机。
...
- 当前已开始实现 M0/M1 的内部读取、会话、数据库和采集合同；尚未新增产品 HTTP/界面，也未扩展数据清理范围。现有登录、点歌、礼物、OBS、更新器及云端凭据同步行为保持其合同，不借机重构。

...
export function normalizeDesktopLyricSettings(settings = {}, defaults = {}) {
  ...
  const explicitKaraokeMode = KARAOKE_MODES.includes(settings.desktopLyricKaraokeMode)
    ...
    : null;
  const karaokeMode =
    explicitKaraokeMode || (boolSetting(settings.desktopLyricKaraokeEnabled, true) ? 'continuous' : 'off');
  return {
...
export function resolveLyricTime(currentMs, settings) {
  ...
  return Math.max(0, (Number.isFinite(number) ? number : 0) + (Number.isFinite(offset) ? offset : 0));
}
...
export function resolveNoLyricText(timeline, settings, defaults = {}) {
  ...
  return String(settings?.noLyricText || defaults.desktopLyricNoLyricText).trim() || defaults.desktopLyricNoLyricText;
}

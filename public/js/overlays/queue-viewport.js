...
export function calculateQueuePanelScale(
  viewportWidth,
  viewportHeight,
  panelWidth,
  panelHeight,
  ...
  return Math.min(
    availableWidth / safePanelWidth,
    availableHeight / safePanelHeight,
  );
}
...
export function syncQueuePanelViewport(panel) {
  ...
  const edge = panelStyle
    ...
    : 0;
  const viewportWidth =
    Number(view.innerWidth || ownerDocument.documentElement?.clientWidth) || 1;
  const viewportHeight =
    Number(view.innerHeight || ownerDocument.documentElement?.clientHeight) ||
    1;

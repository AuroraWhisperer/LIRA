...
const client = createInteractionClient({
  ...
  async fetchState() {
    ...
    const response = await fetch('/api/interactions/session', {
      cache: 'no-store',
...
function render(state) {
  ...
  if (session.kind === 'poll') {
    ...
    byId('interactionParticipants').textContent =
      finished && session.participants ? `${session.participants} 人评分` : '';
  }
  byId('interactionParticipants').hidden =
    appearance.interactionShowParticipants !== 'true' || !byId('interactionParticipants').textContent;
  byId('interactionParticipants').parentElement.hidden =
    byId('interactionParticipants').hidden && !byId('interactionWarning').textContent;
}

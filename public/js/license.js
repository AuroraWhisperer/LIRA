...
(function initLicensePage() {
  ...
  const messages = {
    ...
    ACCOUNT_NAME_INVALID: '用户名只能使用小写字母、数字和中划线，且不能以中划线开头或结尾。',
  ...
  function errorMessage(code) { return messages[String(code || '')] || '激活失败，请检查信息后重试。'; }
  ...
  function render(snapshot = {}) {
    ...
    else if (state === 'authorizing') setStatus('正在绑定设备并验证授权，请稍候…', 'loading');
    else if (state === 'needs_connection') setStatus(errorMessage(snapshot.error || 'NETWORK_UNAVAILABLE'), 'error');
  ...
  async function loadState() {
    ...
    try { render(await api.getState()); } catch (_) { setStatus(errorMessage('NETWORK_UNAVAILABLE'), 'error'); }
  }
  ...
  async function activate(event) {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        setTimeout(() => { window.location.href = '/admin?desktop=1'; }, 60);
      } else {
  ...
  async function retry() {
    ...
    try {
      ...
      if (result?.ok && result.state === 'authorized') window.location.href = '/admin?desktop=1';
      else render(result);
    } catch (_) { setStatus(errorMessage('NETWORK_UNAVAILABLE'), 'error'); }
    finally { busy = false; }
  }

  form?.addEventListener('submit', activate);
  retryButton?.addEventListener('click', retry);
  document.getElementById('licenseMinimizeBtn')?.addEventListener('click', () => window.songAssistantDesktop?.minimizeWindow?.());
  document.getElementById('licenseMaximizeBtn')?.addEventListener('click', () => window.songAssistantDesktop?.maximizeWindow?.());
  document.getElementById('licenseCloseBtn')?.addEventListener('click', () => window.songAssistantDesktop?.closeWindow?.());
  if (api?.onStateChanged) unsubscribe = api.onStateChanged((snapshot) => {
    if (snapshot?.state === 'authorized') window.location.href = '/admin?desktop=1';

...
(function initLicensePage() {
  ...
  const initializationCard = document.getElementById('giftCatalogInitializationCard');
  ...
  const initializationHeading = document.getElementById('giftCatalogInitializationHeading');
  const initializationPercent = document.getElementById('giftCatalogInitializationPercent');
  const initializationProgress = document.getElementById('giftCatalogInitializationProgress');
  const initializationStatus = document.getElementById('giftCatalogInitializationStatus');
  const initializationRetryButton = document.getElementById('giftCatalogInitializationRetryBtn');
  const initializationBackButton = document.getElementById('giftCatalogInitializationBackBtn');
  let busy = false;
  ...
  const messages = {
    ...
    ACCOUNT_NAME_INVALID: '用户名只能使用英文字母、数字和中划线（-），且不能以中划线开头或结尾，不支持中文。',
    ACCOUNT_NAME_RESERVED: '此用户名为系统保留名称，请更换。',
    ACCOUNT_NAME_ALREADY_EXISTS: '此用户名已被使用；已有账号请选择“登录已有账号”。',
    ...
    DEVICE_IDENTITY_AMBIGUOUS: '检测到多条相同电脑的历史记录，请联系管理员核对后重试。',
    ...
    SESSION_SUPERSEDED: '当前设备已由另一个 LIRA 进程登录。如需使用本窗口，请先关闭另一进程后重试。',
    ...
    CHALLENGE_PROTOCOL_MISMATCH: '当前客户端授权协议与服务器不兼容，请更新 LIRA。',
    TOO_MANY_PAIRING_CODE_REQUESTS: '新设备授权码生成过于频繁，请稍后再试。',
    TOO_MANY_ACTIVE_PAIRING_CODES: '当前已有多张仍有效的新设备授权码，请先使用、撤销或等待过期。',
    PAIRING_CODE_NOT_FOUND: '找不到这张新设备授权码。',
    PAIRING_CODE_ALREADY_CONSUMED: '这张新设备授权码已使用或已撤销，不能再次操作。',
  ...
  function errorMessage(code) {
    ...
    if (/^HTTP_(429|5\d\d)$/.test(value)) return '授权服务器暂时不可用，请稍后重试。';
    return messages[value] || '激活失败，请检查信息后重试。';
  }

  function connectionErrorMessage(code) {
    const value = String(code || '');
    if (messages[value] || /^HTTP_(429|5\d\d)$/.test(value)) return errorMessage(value);
  ...
  function validate() {
    ...
    if (accountName.length < 2 || accountName.length > 32) return messages.ACCOUNT_NAME_LENGTH;
    if (!/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(accountName)) return messages.ACCOUNT_NAME_INVALID;
    if (!password) return '请输入密码。';
    if (!activationCode) return registration ? '请输入注册激活码。' : '请输入短效登录码。';
    return '';
  }
  ...
  function setAccountMode(isRegistration) {
    ...
    for (const [id, value] of [
      ...
      [
        ...
        registration ? '创建账号并授权这台电脑' : '重装或使用新电脑，请向管理员获取短效登录码',
      ],
    ...
    passwordInput.setAttribute('autocomplete', registration ? 'new-password' : 'current-password');
    passwordInput.value = '';
    codeInput.value = '';
  ...
  function setPasswordVisible(visible) {
    ...
    passwordIcon.setAttribute('href', `/img/shared/password-visibility.svg#${visible ? 'eye-off' : 'eye'}`);
  }

  function render(snapshot = {}) {
    const state = String(snapshot.state || 'needs_activation');
    licenseState = state;
    if (state === 'authorizing' || state === 'authorized') showSavedAuthorizationNotice = false;
    ...
    retryButton.hidden = !(state === 'needs_connection' || state === 'blocked' || state === 'authorized');
    retryButton.textContent = state === 'authorized' ? '继续准备' : '重试连接';
    retryButton.disabled = isAuthorizing;
    if (state === 'authorized') setStatus('本机授权已通过，可继续准备。', 'good');
    else if (state === 'checking') setStatus('正在检查本机设备授权…', 'loading');
    else if (state === 'authorizing') setStatus('正在验证账号与本机授权，请稍候…', 'loading');
    else if (state === 'needs_connection') setStatus(connectionErrorMessage(snapshot.error), 'error');
    else if (state === 'blocked' && snapshot.error === 'DEVICE_REVOKED' && showSavedAuthorizationNotice)
      setStatus('本机保留的旧设备授权已撤销，请获取新的短效登录码后重新登录。');
    else if (state === 'blocked') setStatus(errorMessage(snapshot.error), 'error');
  ...
  function renderGiftCatalogState(snapshot = {}) {
    ...
    showGiftCatalogInitialization();
    const catalogStatus =
      snapshot?.ok === false && snapshot?.status !== 'ready' ? 'error' : String(snapshot.status || 'required');
  ...
  function catalogErrorMessage(code) {
    const value = String(code || '');
    if (value === 'CATALOG_INVALID' || value === 'REMOTE_CATALOG_SCHEMA_UNSUPPORTED')
  ...
  async function retryGiftCatalog() {
    if (initializationBusy || licenseState !== 'authorized' || !api?.retryGiftCatalog) return;
  ...
  document
    ...
    ?.addEventListener('click', () => window.songAssistantDesktop?.minimizeWindow?.());
  maximizeButton?.addEventListener('click', () => window.songAssistantDesktop?.maximizeWindow?.());
  document
    .getElementById('licenseCloseBtn')
    ?.addEventListener('click', () => window.songAssistantDesktop?.closeWindow?.());
  if (maximizeButton && window.songAssistantDesktop?.onWindowMaximized) {
    unsubscribeWindowMaximized = window.songAssistantDesktop.onWindowMaximized((isMaximized) => {
  ...
  if (api?.onGiftCatalogStateChanged) unsubscribeGiftCatalog = api.onGiftCatalogStateChanged(renderGiftCatalogState);

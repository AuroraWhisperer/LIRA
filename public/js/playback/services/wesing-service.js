...
export class WeSingService {
  constructor(options = {}) {
    ...
    this.status = {
      ...EMPTY_STATUS,
      lyricState: { ...EMPTY_STATUS.lyricState },
    };
    this.selected = null;
  ...
  async refresh(options = {}) {
    try {
      const data = await this.request('/api/music/wesing/refresh', {
        method: 'POST',
        body: {},
      });
  ...
  applyStatus(nextState) {
    ...
    this.status = {
      ...
      ...nextState,
      lyricState: {
        ...this.status.lyricState,
        ...(nextState.lyricState || {}),
      },
    };
  ...
  render() {
    ...
    setText('weSingTrackTitle', status.trackTitle || '等待播放');
    setText(
      'weSingTrackMeta',
      status.trackTitle
...
function formatLyricSource(source) {
  ...
  if (source === 'wesing') return '全民 K 歌';
  return '全民 K 歌';
}

function getCompactStatusMessage(status) {
  if (status.status === 'error') return status.message || '检测失败';
  if (status.status === 'loading') return '读取中';
  if (status.status === 'empty') return '无歌词';
  if (status.qrcReady) return '同步中';
  if (!status.cacheReady) return '未配置';
  return '等待播放';
}

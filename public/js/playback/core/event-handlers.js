...
export function createEventHandlers(deps) {
  ...
  function setupPendingConfirmButtons() {
    document
      ...
      ?.addEventListener('click', () => handlePlaybackPendingAction('confirm', 0));
    document
      .getElementById('pendingConfirmRejectBtn')
      ?.addEventListener('click', () => handlePlaybackPendingAction('ignore', 0));
  }
  ...
  function setupSearchButtons() {
    ...
    document.getElementById('playbackSearchResults')?.addEventListener('click', async (event) => {
      ...
      if (!button) return;
      await handlePlaybackSearchAction(button.dataset.playbackSearchAction, Number(button.dataset.playbackSearchIndex));
    });
  }
  ...
  function setupVolumeControls() {
    ...
    document.getElementById('playbackVolume')?.addEventListener('input', (event) => {
      stateActions.setVolume(Math.max(0, Math.min(1, Number(event.target.value))));
  ...
  function setupQualityControls() {
    ...
    qualityButton?.addEventListener('keydown', (event) => {
      ...
      if (!open) {
        ...
        const nextIndex =
          event.key === 'ArrowUp' ? Math.max(0, selectedIndex - 1) : Math.min(options.length - 1, selectedIndex + 1);

...
export function serverOverlayUrl(snapshot, settings) {
  ...
  try {
    ...
    if (origin.protocol !== 'https:' || origin.username || origin.password ||
        ...
        !/^\/overlay\/[A-Za-z0-9_-]{16}$/.test(url.pathname)) return '';
    return url.href;
  } catch {
    return '';
  }
...
async function refresh(snapshot) {
  ...
  const owner = snapshot?.state === 'authorized'
    ? JSON.stringify([snapshot.streamer?.accountName, snapshot.streamer?.songPageUrl]) : '';
  if (owner !== currentOwner) { currentOwner = owner; publish(''); }
  if (!owner) return;
  try {
...
export function observeServerOverlayUrl(listener) {
  ...
  if (!initialized) {
    ...
    const dispose = bridge?.onStateChanged?.((snapshot) => { void refresh(snapshot); });
    const requestedGeneration = generation;

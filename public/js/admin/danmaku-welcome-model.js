...
export const LIBRARIES = {
  messages: '欢迎词库',
  greetingMessages: '问候词库',
  attentionWelcomeMessages: '专属欢迎词库',
  attentionGreetingMessages: '专属问候词库',
};
export const SWITCHES = {
  enabled: 'Toggle',
  greetingEnabled: 'GreetingToggle',
  attentionEnabled: 'AttentionToggle',
  rareNamePinyinEnabled: 'PinyinToggle',
};
...
export function parameterValues(draft, settings) {
  ...
  if (
    ...
    !errors.greetingDelaySeconds &&
    !errors.welcomeDelaySeconds &&
...
export function validMessages(messages) {
  return (
    ...
    messages.every(
      (text) =>
        typeof text === 'string' && text.trim() && Array.from(text).length <= 80 && !/[\x00-\x1f\x7f]/u.test(text),
    )
  );
}
...
export function confirmedSettings(result) {
  ...
  if (
    ...
    Object.keys(SWITCHES).some((key) => typeof result[key] !== 'boolean') ||
...
export function previewMessage(template, pinyin, stage) {
  const name = '小曌',
    annotated = pinyin ? '小曌(zhào)' : name;
  const format = (value) =>
    template.includes('{username}') ? template.replaceAll('{username}', value) : `${value}，${template}`;
...
export function welcomePreview(settings, level) {
  ...
  if (!passes(settings.welcomeMinHonorLevel))
    return [`首次欢迎：${reason(settings.welcomeMinHonorLevel)}`, '二次问候：需要首次欢迎发送成功'];
  ...
  return [
    `${prefix}欢迎 · 最早进场后 ${settings.welcomeDelaySeconds} 秒：${welcome}`,
    !settings.greetingEnabled
      ? '二次问候：未开启'
      : !passes(settings.greetingMinHonorLevel)
        ? `二次问候：${reason(settings.greetingMinHonorLevel)}`
        : greeting === welcome
          ? '二次问候：与首次文案相同，跳过'

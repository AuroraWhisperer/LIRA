...
export function initDanmakuWelcome({
  ...
  const status = element('Status'),
    refresh = element('RefreshBtn');
  if (![toggle, list, input, status, refresh].every(Boolean)) return;
  const libraries = Object.fromEntries(
    Object.keys(LIBRARIES).map((key) => [key, { items: [], dirty: false, revision: 0, addition: '', page: 0 }]),
  );
  ...
  const hasDrafts = () =>
    parameters.dirty || Object.values(libraries).some((domain) => domain.dirty || domain.addition);
  ...
  function updatePreview() {
    if (!isV2()) {
      ...
      element('PreviewFirst').textContent = '';
      element('PreviewSecond').textContent = '';
      return;
    }
    ...
    const config = draft
      ? {
          ...settings,
          ...parsed.values,
          ...Object.fromEntries(Object.keys(LIBRARIES).map((key) => [key, libraries[key].items])),
        }
      : settings;
    const invalid =
      draft && Object.keys(parsed.errors).length
        ? Object.values(parsed.errors)[0]
        : Object.keys(LIBRARIES).some((key) => !validMessages(config[key]))
          ? '词库需为 1～30 条，每条不超过 80 字'
          : '';
    if (invalid) {
      element('PreviewSummary').textContent = '草稿有错误';
      element('PreviewFirst').textContent = invalid;
      element('PreviewSecond').textContent = '';
      return;
    }
    ...
    const levels = [
      ...new Set([
        0,
        31,
        config.welcomeMinHonorLevel - 1,
        config.welcomeMinHonorLevel,
        config.greetingMinHonorLevel,
        config.attentionMinHonorLevel - 1,
        config.attentionMinHonorLevel,
      ]),
    ]
    ...
    if (levelSelect.dataset.levels !== signature) {
      ...
      for (const value of ['unknown', ...levels.map(String)]) {
        ...
        option.value = value;
        option.textContent = value === 'unknown' ? '等级未知' : `${value} 级`;
    ...
    element('PreviewFirst').textContent = result[0];
    element('PreviewSecond').textContent = result[1];
    element('PreviewSummary').textContent =
      `${draft ? '编辑草稿' : '已确认设置'} · ${config.enabled ? (config.greetingEnabled ? '最多两次' : '仅首次欢迎') : '欢迎已关闭'} · 不发送`;
  }
  function controls() {
    const loaded = Boolean(settings),
      busy = loading || saving;
    for (const [key, suffix] of Object.entries(SWITCHES)) {
      const field = element(suffix);
    ...
    element('LibraryStatus').textContent = library.addition
      ? '输入尚未添加，不会随词库保存'
      : library.dirty
        ? '当前词库有未保存修改'
        : loaded
          ? '当前词库已确认'
          : '词库尚未确认';
    const dirtyNames = Object.entries(libraries)
    ...
    element('ParameterStatus').textContent = !isV2()
      ? '参数尚未确认'
      : parameters.dirty
        ? '参数有未保存修改'
        : '参数已确认';
    element('EnableHint').textContent = parameters.dirty
      ...
      : dirtyNames.length
        ? '词库尚未保存，启用时使用已保存词库。'
        : !settings?.enabled
          ? '先开启进场欢迎，再按需开启二次问候或重点关注。'
          : '';
    element('OverviewStatus').textContent = /未确认|正在/.test(notice)
      ? notice
      : parameters.dirty
        ? '参数未保存 · 开启并保存参数'
        : dirtyNames.length
  ...
  function libraryChanged(key) {
    if (key) {
      libraries[key].dirty = true;
      libraries[key].revision += 1;
  ...
  function accept(result, submitted = {}) {
    const next = confirmedSettings(result);
    settings = next;
    if (!parameters.dirty || submitted.parameters === parameters.revision) {
      parameters.values = Object.fromEntries(
        Object.keys(PARAMETER_FIELDS).map((key) => [key, String(next[key] ?? '')]),
      );
    ...
    for (const key of Object.keys(LIBRARIES)) {
      ...
      if (!domain.dirty || submitted[key] === domain.revision) {
        domain.items = [...(next[key] || [])];
        domain.dirty = false;
        if (editor.current() === key) render = true;
      }
    }
    if (!isV2()) {
      editor.select('messages');
      showView('libraries');
  ...
  async function reload() {
    ...
    loading = true;
    displayNotice('正在读取服务器设置…');
    controls();
    try {
      const result = bridge.getWelcomeSettingsV2
        ? await bridge.getWelcomeSettingsV2()
  ...
  async function write(patch, domains = []) {
    if (!settings || loading || saving || disposed) {
      controls();
      return;
    }
    const requested = generation;
    ...
    try {
      ...
      if (!response?.ok && response?.fieldErrors?.length) {
        ...
        for (const { field, reason } of response.fieldErrors) {
          if (PARAMETER_FIELDS[field])
            fieldErrors[field] =
              reason === 'AFTER_WELCOME_REQUIRED'
                ? '问候时间需要大于首次欢迎时间'
                : `请检查${PARAMETER_FIELDS[field].label}`;
        }
  ...
  function addMessage() {
    ...
    if (!validMessages([text])) {
      toast('请输入不超过 80 字的文案');
      input.focus();
      return;
    }
    domain.items.push(text);
    domain.addition = '';
    domain.page = Math.floor((domain.items.length - 1) / 6);
    libraryChanged(editor.current());
    editor.render();
    input.focus();
  }
  function account(snapshot) {
    ...
    parameters.dirty = false;
    parameters.revision += 1;
    for (const domain of Object.values(libraries))
      Object.assign(domain, { items: [], dirty: false, revision: domain.revision + 1, addition: '', page: 0 });
    parameterInputs();
    showErrors({});
    editor.select('messages');
    displayNotice('连接已授权账号后可设置进场欢迎');
    controls();
    void reload();
  }
  ...
  for (const key of Object.keys(PARAMETER_FIELDS))
    element(key).addEventListener('input', () => {
      parameters.values[key] = element(key).value;
      parametersChanged();
    });
  ...
  element('SaveBtn').addEventListener('click', () => {
    const key = editor.current(),
      messages = libraries[key].items.map((text) => text.trim());
    if (!validMessages(messages)) {
      toast('请保留 1～30 条文案，每条不超过 80 字');
      return;
    }
  ...
  for (const button of documentRef.querySelectorAll('[data-welcome-library]'))
    button.addEventListener('click', () => {
      ...
      editor.select(button.dataset.welcomeLibrary);
      showView('libraries');
    });

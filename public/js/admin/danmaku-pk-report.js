export function initDanmakuPkReport({
  ...
  let owner = '', generation = 0, enabled = false, loaded = false, pending = false, disposed = false;
  function controls() {
  ...
  async function sync(value) {
    ...
    if (!owner || !available || disposed || pending || (writing && !loaded)) { controls(); return; }
    const requested = generation;
    pending = true; controls(); status.textContent = '正在同步 PK 播报设置…';
    try {
      const result = writing ? await bridge.updatePkReportSettings({ enabled: value })
        : await bridge.getPkReportSettings();
      if (disposed || generation !== requested) return;
      enabled = confirmed(result); loaded = true;
      status.textContent = enabled ? '已开启，服务器将在 PK 开始时播报' : 'PK 对手信息播报已关闭';
      if (writing) toast(enabled ? 'PK 对手信息播报已开启' : 'PK 对手信息播报已关闭');
    } catch {
      if (disposed || generation !== requested) return;
      status.textContent = writing && value === false
        ? '关闭尚未同步，服务器可能仍在播报；请刷新核对。'
        : 'PK 播报功能暂不可用，请检查连接或升级服务器后刷新。';
    } finally {
      if (!disposed && generation === requested) { pending = false; controls(); }
    }
  }
  const reload = () => { void sync(); };
  function account(snapshot) {
    if (disposed) return;
    const name = snapshot?.streamer?.accountName, url = snapshot?.streamer?.songPageUrl;
    ...
    owner = next; generation++; enabled = false; loaded = false; pending = false;
    controls();
    status.textContent = available ? '连接已授权账号后可设置 PK 播报' : '请升级桌面客户端后设置 PK 播报';
    reload();
  }
  toggle.addEventListener('change', () => { void sync(toggle.checked); });

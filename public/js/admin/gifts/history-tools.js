...
export function createGiftHistoryTools({ state, reload, resetPagination }) {
  ...
  let operation = 0;
  const rowSelection = createGiftHistorySelection({
    state,
    update,
    onManualSelection: () => {
      cancelSelection();
  ...
  function update() {
    ...
    if (header) {
      header.checked = state.items.length > 0 && selected === state.items.length;
      header.indeterminate = selected > 0 && selected < state.items.length;
    }
  ...
  function applyFilters() {
    ...
    const filters = {
      startDate: get('giftHistoryStartDate').value,
      endDate: get('giftHistoryEndDate').value,
      userQuery: get('giftHistoryUserQuery').value.trim(),
      giftQuery: get('giftHistoryGiftQuery').value.trim(),
  ...
  get('giftHistoryFilters')?.addEventListener('submit', (event) => {
  ...
  get('giftHistoryReset')?.addEventListener('click', () => {
    get('giftHistoryFilters').reset();
    applyFilters();
  });
  ...
  get('giftHistorySelectPage')?.addEventListener('change', (event) => {
    ...
    for (const item of state.items) {
      if (event.target.checked) state.selected.add(item.eventId);
      else state.selected.delete(item.eventId);
    }
    update();
  });
  get('giftHistorySelectAll')?.addEventListener('click', () =>
    run(async () => {
      cancelSelection();
      ...
      try {
        const response = await fetch('/api/gifts/selection', {
          method: 'POST',
        ...
        get('giftHistorySelectionNotice').textContent =
          `已选 ${state.selected.size} 条${result.data.partial ? '已同步记录，仍可能有记录待补齐' : '记录'}`;
        update();
      } catch (error) {
        if (current === operation) throw error;
      } finally {
        if (current === operation) cancelSelection();
      }
    }),
  );
  get('giftHistoryExport')?.addEventListener('click', () =>
    run(() => exporter.open({ ...options(), eventIds: [...state.selected] })),
  );

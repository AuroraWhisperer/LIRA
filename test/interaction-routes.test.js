...
test('interaction routes require session identity and propagate conflict/validation statuses', async () => {
  ...
  const context = { interactions: { finish(id) { assert.equal(id, 'old'); throw Object.assign(new Error('changed'), { statusCode: 409 }); } } };
...
test('interaction projections remove secrets and suppress unrevealed rating even if owner accidentally includes fields', () => {
  const data = { runtimeId: 'r', revision: 1, cookie: 'secret', session: { kind: 'rating', phase: 'collecting', average: 8, sum: 80, participants: 10, uid: 'secret', options: [{ votes: 10 }] } };
  ...
  assert.deepEqual(projectWebSocketPayload({ type: 'overlay', scope: 'interactions' }, { type: 'interaction:update', state: data }), { type: 'interaction:update', state: expected });
  assert.equal(projectWebSocketPayload({ type: 'overlay', scope: 'games' }, { type: 'interaction:update', state: data }), null);
  ...
  assert.deepEqual(projectOverlayResponse('interactions', '/api/interactions/session', { runtimeId: 'r', revision: 2, session: null }), { runtimeId: 'r', revision: 2, session: null });
...
test('interactions scope is read-only and independent of game privileges', () => {
  ...
  for (const [method, path] of [['POST', '/api/interactions/session'], ['POST', '/api/interactions/session/finish'], ['POST', '/api/interactions/session/clear'], ['GET', '/api/interactions/host-state'], ['GET', '/api/games/session'], ['POST', '/api/wheel/spin']]) {

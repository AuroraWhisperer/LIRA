...
test('optional display v1 survives projection and repeated final deliveries without exposing viewer identity', (t) => {
  ...
  const service = createGiftProjectionService(fx.context); t.after(() => service.dispose());
  const event = { eventId: 'profile-test', cursor: 1, phase: 'final', gift: {
    giftId: '1', giftName: '礼物', userName: '观众', num: 1, unitPrice: 100, totalPrice: 100,
    coinType: 'gold', isBlindBox: false, blindBoxId: null, blindBoxName: '', blindBoxPrice: null,
    blindProfit: null, createdAt: '2026-09-01T12:00:00Z', display: {
      version: 1, avatarUrl: 'https://i0.hdslb.com/bfs/face/test.webp', guardLevel: 3,
    },
  } };
  ...
  const old = structuredClone(event); delete old.gift.display;
  assert.doesNotThrow(() => normalizeProcessedGiftEvent(old));
  for (const display of [{ ...event.gift.display, version: 2 }, { ...event.gift.display, guardLevel: 4 },
    { ...event.gift.display, avatarUrl: 'https://evil.invalid/image.png' }, { ...event.gift.display, uid: '1' }]) {

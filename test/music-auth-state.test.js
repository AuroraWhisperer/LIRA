...
test('music auth projection retains status fields and excludes credentials', () => {
  ...
  assert.deepEqual(sanitizeAuthState(null), {
    loggedIn: false, cookieCount: 0, keyCookieNames: [],
    encryptedSnapshotExists: false, lastSavedAt: '',
  });
  assert.deepEqual(sanitizeAuthState({
    loggedIn: 1, cookieCount: '3', keyCookieNames: ['MUSIC_U'],
    encryptedSnapshotExists: true, lastSavedAt: 'fixture-time',
    cookie: 'synthetic-secret', accessToken: 'synthetic-token',
  }), {
    loggedIn: true, cookieCount: 3, keyCookieNames: ['MUSIC_U'],
    encryptedSnapshotExists: true, lastSavedAt: 'fixture-time',
  });

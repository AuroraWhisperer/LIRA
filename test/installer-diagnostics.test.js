...
test('Windows installer diagnostics collect relevant evidence without application data', {
  skip: process.platform !== 'win32',
}, async (t) => {
  for (const withEvidence of [true, false]) {
    await t.test(withEvidence ? 'installer and matching crash' : 'no installer or migration report', () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-diagnostics-test-'));
      const tempDir = path.join(root, 'temp');
      fs.mkdirSync(tempDir);
      fs.mkdirSync(path.join(root, 'data'));
      fs.writeFileSync(path.join(root, 'data', 'private.txt'), 'NEVER_READ_APPLICATION_DATA');
      const helper = path.join(root, 'collect-install-diagnostics.ps1');
      fs.copyFileSync(path.join(__dirname, '../scripts/collect-install-diagnostics.ps1'), helper);
      if (withEvidence) {
        fs.writeFileSync(path.join(root, 'lira-setup-fixture.exe'), 'fixture, never executed');
        fs.writeFileSync(path.join(tempDir, 'LIRA-install-error.txt'),
          '\ufeff复制旧版数据\r\nfixture migration error', 'utf16le');
      }
...
`;
      try {
        ...
        const result = spawnSync(powershell, ['-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', script], {
          windowsHide: true, timeout: 30000, encoding: 'utf8',
          env: { ...process.env, TEMP: tempDir, TMP: tempDir, PSModulePath: '' },
        });
        assert.equal(result.error, undefined);
        assert.equal(result.status, 0, result.stdout + result.stderr);
        const reports = fs.readdirSync(root).filter((name) => name.startsWith('LIRA安装诊断-') && name.endsWith('.txt'));
        assert.equal(reports.length, 1);
        const report = fs.readFileSync(path.join(root, reports[0]), 'utf8');
        assert.ok(report.includes('19045.1'));
        assert.doesNotMatch(report, /DO_NOT_COLLECT_THIS_EVENT|NEVER_READ_APPLICATION_DATA/);
        if (withEvidence) {
        ...
        assert.ok(path.basename(resolved).startsWith('lira-diagnostics-test-'));

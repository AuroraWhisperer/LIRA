...
test('installer defaults respect existing paths, explicit paths and computers without D', {
  ...
  const choose = installer.match(/Function liraSelectDefaultDirectory\b[\s\S]*?FunctionEnd/)[0]
  ...
  try {
    for (const [name, previous, selected, explicit, hasD, expected] of [
      ...
      fs.writeFileSync(script, [
        'Unicode true', 'Name "LIRA directory fixture"', 'RequestExecutionLevel user',
        `OutFile "${quote(executable)}"`, 'Var liraPreviousInstallDir',
        `!define FIXTURE_PREVIOUS "${quote(previous)}"`, `!define FIXTURE_EXPLICIT "${quote(explicit)}"`,
        `!define FIXTURE_DRIVE "${quote(hasD ? root : path.join(root, 'absent-drive'))}"`,
        choose, 'Function .onInit', `StrCpy $INSTDIR "${quote(selected)}"`, 'Call liraSelectDefaultDirectory',
        `FileOpen $0 "${quote(output)}" w`, 'FileWriteUTF16LE /BOM $0 "$INSTDIR$\\r$\\n$liraPreviousInstallDir"',
        'FileClose $0', 'SetErrorLevel 0', 'Quit', 'FunctionEnd', 'Section', 'SectionEnd',
      ].join('\n'));
      const build = spawnSync(process.env.LIRA_TEST_MAKENSIS, ['/V2', '-INPUTCHARSET', 'UTF8', script], { encoding: 'utf8', windowsHide: true, timeout: 30000 });
      assert.equal(build.status, 0, build.stdout + build.stderr);
      const run = spawnSync(executable, ['/S'], { windowsHide: true, timeout: 10000 });
      assert.equal(run.status, 0, name);
      const actual = fs.readFileSync(output, 'utf16le').replace(/^\uFEFF/, '').split('\r\n');

...
function record(req) {
  ...
  requests.push({ path: url.pathname, management: req.headers.authorization === `Bearer ${token}`,
    origin: req.headers.origin || '', scoped: req.headers.authorization === 'Bearer overlay-clock' });
  return url;
}
...
function createWindow(partition, webPreferences = {}) {
  const window = new BrowserWindow({ show: false, webPreferences: {
    contextIsolation: true, nodeIntegration: false, sandbox: false, ...(partition ? { partition } : {}),
    ...webPreferences,
  } });
  windows.push(window);
  return window;
...
app.whenReady().then(async () => {
  ...
  const local = await listen((req, res) => {
    ...
    if (url.pathname === '/api/redirect') {
      res.writeHead(302, { Location: `${externalUrl}/target` }); res.end(); return;
    }
    if (url.pathname === '/api/redirect-overlay') {
      res.writeHead(302, { Location: '/clock?redirected=1' }); res.end(); return;
    }
  ...
  const auth = createDesktopRequestAuth({
    desktopSession: session.defaultSession, getMainWindow: () => mainWindow,
    getBaseUrl: () => baseUrl, getToken: () => token,
  });
  configureMediaRequestHeaders(session.defaultSession, {}, auth);
  mainWindow = createWindow();
  auth.bindWindow(mainWindow, { openExternal() { assert.fail('probe must not open external browsers'); } });
  ...
  for (const route of ['/api/main', '/api/media', '/api/beacon', '/ws', '/api/reloaded'])
    assert.ok(requests.some((row) => row.path === route && row.management), route);
  for (const route of ['/clock', '/external', '/api/iframe', '/api/export-window', '/api/login-window', '/api/license-page']) {
    const rows = requests.filter((row) => row.path === route);
    assert.ok(rows.length, route);
    assert.equal(rows.some((row) => row.management), false, route);
  }

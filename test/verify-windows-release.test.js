...
function verifyFixture(expectedPublisher, options = {}) {
  ...
  vm.runInNewContext(
    `(function () {\n${source}\n})()`,
    {
      Buffer,
      process: {
        argv: ['node', filename, "synthetic setup's.exe", expectedPublisher],
        exit(code) {
          exitCode = code;
        },
      },
      ...
      require(name) {
        ...
        if (name === 'node:child_process')
          return {
            execFileSync(command, args, executionOptions) {
...
test('publisher verification rejects a different CN that merely contains the expected name', () => {
  const f = verifyFixture('Synthetic Publisher', {
    attributes: [['03', 'Other Synthetic Publisher Company']],
  });
  assert.equal(f.exitCode, 1);
...
test('publisher verification never matches organization, organizational unit, issuer or timestamp names', () => {
  for (const attributes of [
    ...
    const f = verifyFixture('Synthetic Publisher', {
      attributes,
      signature: {
        SignerCertificateIssuer: 'CN=Synthetic Publisher',
        TimeStamperCertificateSubject: 'CN=Synthetic Publisher',
      },
    });
    assert.equal(f.exitCode, 1);
  }
...
test('publisher verification handles punctuation, escaped display names and Unicode from certificate data', () => {
  for (const commonName of [
    'Synthetic, Publisher',
    'Synthetic "Publisher"',
    'Synthetic + Publisher\\Branch',
    '合成发布者研发团队',
  ]) {
    assert.equal(
      verifyFixture(commonName, {
        attributes: [
          ['03', commonName],
          ['0a', 'Synthetic Org'],
        ],
...
test('publisher verification rejects ambiguous multiple CNs and missing or malformed certificate data', () => {
  assert.equal(
    verifyFixture('Synthetic Publisher', {
      attributes: [
        ['03', 'Synthetic Publisher'],
        ['03', 'Other Publisher'],
      ],
    }).exitCode,
    1,
  ...
  for (const raw of [null, '', 'not-a-certificate']) {
    assert.equal(
      verifyFixture('Synthetic Publisher', {
        signature: { SignerCertificateRawData: raw },
      }).exitCode,
      1,
    );
  }
...
test('publisher verification retains case handling, certificate rotation and Authenticode validity policy', () => {
  for (const serial of [1, 2]) {
    const f = verifyFixture('synthetic publisher', {
      serial,
      signature: { SignerCertificateNotAfter: '2000-01-01T00:00:00.000Z' },
    });
    assert.equal(f.exitCode, 0);

...
test('query indexes upgrade without changing rows and migrate idempotently', (t) => {
  ...
  f.giftDb.exec('DROP INDEX IF EXISTS idx_gift_events_source_recent; DROP INDEX IF EXISTS idx_gift_events_source_time_asc');
  ...
  for (const sourceScope of [{ sql: 'source_id = ?', params: [source.id] }, { sql: 'source_id IS NULL', params: [] }]) {
    ...
    const plan = prepare(`EXPLAIN QUERY PLAN ${sql}`).all(...params).map((r) => r.detail).join('\n');
...
for (const sortDirection of ['asc', 'desc']) {
  test(`time history ${sortDirection} seeks both boundaries and preserves equal-time ordering`, (t) => {
    ...
    for (let i = 0; i < 17; i += 1) f.insertGift(source.id, `event-${i}`, {
      createdAt: `2026-09-01T12:00:0${Math.floor(i / 6)}.000Z`,
    });
    const expected = f.giftDb.prepare(`SELECT platform_id FROM gift_events ORDER BY created_at ${sortDirection}, id DESC`).all().map((r) => r.platform_id.slice(12));
    const { queries, prepare } = observe(f.giftDb);
    const ids = [];
    let cursor;
    do {
    ...
    const store = createGiftQueryStore(f.giftDb);
    const options = { sourceId: source.id, asOf: '2026-09-02T00:00:00.000Z', sortDirection,
      cursor: { sortValue: '2026-09-01T12:00:01.000Z', id: 12 }, limit: 2 };
    queries.length = 0;
    ...
    assert.deepEqual(store.listHistory({ ...options, cursor: {
      sortValue: sortDirection === 'asc' ? '2026-09-01T23:00:00.000Z' : '2026-09-01T00:00:00.000Z', id: 1,
    } }), []);
  });
}
...
test('history count cache follows data versions, source and filters, including external writes and rollback', (t) => {
  ...
  const filename = f.giftDb.prepare('PRAGMA database_list').all().find((r) => r.name === 'main').file;
  const external = new DatabaseSync(filename);
  try {
    external.prepare('DELETE FROM gift_events WHERE source_id = ? AND platform_id = ?').run(source.id, 'lira-server:late');
...
test('both time segments and cached total share one snapshot during an external commit', (t) => {
  ...
  const source = f.resolveSource('a'.repeat(64));
  f.setActiveSource(source.id);
  for (let i = 0; i < 7; i += 1) f.insertGift(source.id, `event-${i}`, {
  ...
  assert.deepEqual(page.items.map((item) => item.eventId), ['event-4', 'event-3', 'event-2']);
...
test('recent expression index preserves legacy timestamp, zone and same-second ID order', (t) => {
  ...
  for (const [i, createdAt] of [
    '2026-09-01T04:00:00.900Z', '2026-09-01 04:00:00',
...
test('fan list loads only candidates and batches full lists without changing record order or scope', (t) => {
  ...
  f.store.transaction(() => {
    for (let i = 0; i < 503; i += 1) {
      const p = f.store.save(SCOPE, { alias: `观众-${i}`, tags: ['中文'], favorite: i === 502,
        archived: false, summary: '', milestoneReminders: false }, null, '2026-09-18T04:00:00.000Z');
      ids.push(p.id);
      for (const id of ['z', 'a']) f.store.records.insert(SCOPE, p.id, {
        id: `${p.id}-${id}`, kind: 'note', occurredAt: '2026-09-17T04:00:00.000Z', data: { text: id },
      });

...
const {
  ...
  chinaDateKey,
} = require('../src/bilibili/checkin-service');
const {
  CHECKIN_BLESSINGS,
...
test('check-in bot records first, duplicate, and next-day check-ins by uid', () => {
  const dataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'song-plugin-checkin-'),
  );
  ...
  try {
    ...
    const duplicate = service.handleDanmaku({
      message: '签到',
      uid: '123',
      userName: 'AliceNew',
    });
    ...
    assert.equal(
      duplicate.autoReply.message,
      '今天已经签到过啦，已累计 1 天。祝你今天顺利。',
    );
    ...
    const row = databases.checkinDb
      .prepare(
        'SELECT uid, user_name, total_days, last_checkin_date FROM checkin_users WHERE uid = ?',
      )
      .get('123');
...
test('check-in blessings use saved phrases and recover from invalid settings', () => {
  assert.deepEqual(parseCheckinBlessings('[" 祝你天天开心。 ",""]'), [
    '祝你天天开心。',
  ]);

...
test('latest 牛来、豹拉 and 涨艇 gifts use local Bilibili artwork', () => {
  const manifest = JSON.parse(
    fs.readFileSync(path.join(GIFT_ROOT, 'bilibili-gifts.json'), 'utf8'),
  );
  const expectedGifts = [
  ...
  const expectedGifts = [
    {
      ...
      battery: 350,
      rmb: 35,
      image: 'bilibili-gifts/0000-under-0100/35777.webp',
    },
    {
      id: 35778,
      name: '常伴珠钗',
      battery: 1000,
      rmb: 100,
      image: 'bilibili-gifts/0100-0200/35778.webp',
    },
    {
      id: 35779,
      name: '缘起瓷瓶',
      battery: 3000,
      rmb: 300,
      image: 'bilibili-gifts/0300-0400/35779.webp',
    },
    {
      id: 35780,
      name: '倾心宝冠',
      battery: 8000,
      rmb: 800,
      image: 'bilibili-gifts/0800-0900/35780.webp',
    },
    {
      id: 35600,
      name: '万象天衣',
      battery: 30000,
      rmb: 3000,
      image: 'bilibili-gifts/3000-above/35600.webp',
    },
  ];
...
test('paid backpack gift artwork and mappings use their RMB price folders', () => {
  ...
  const paidBackpackGifts = manifest.gifts.filter(
    (gift) => gift.bagGift === true,
  );
  ...
  for (const gift of paidBackpackGifts) {
    ...
    assert.equal(
      gift.category,
      expectedCategory,
      `gift ${gift.id} manifest category`,
    );
    assert.equal(
      path.posix.dirname(gift.image.replace(/^bilibili-gifts\//, '')),
      expectedCategory,
    );
    assert.equal(mappings.get(gift.id)?.imagePath, `/img/${gift.image}`);
    assert.equal(
      fs.existsSync(path.join(GIFT_ROOT, gift.image)),
      true,
      `gift ${gift.id} image`,
    );
  }
...
test('silver and free backpack gifts remain code-only remote mappings', () => {
  ...
  assert.match(source, /^\|\s*35460\s*\|\s*https:\/\/[^|]+\|\s*元宝\s*\|/m);
  assert.doesNotMatch(source, /^\|\s*35460\s*\|\s*\[[^\]]+\]\([^)]+\)\s*\|/m);
});

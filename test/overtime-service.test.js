...
test('pausing recovery stops zero-timer writes and resumes the elapsed clock', () => {
  ...
  try {
    ...
    service.act('enable');
    service.act('start');
    const before = fixture.db.giftDb.prepare('SELECT * FROM overtime_machine_state').get();
    assert.equal(service.pauseRecovery(), true);
    assert.equal(service.pauseRecovery(), false);
    fixture.clock.advance(121_000);
    assert.deepEqual(fixture.db.giftDb.prepare('SELECT * FROM overtime_machine_state').get(), before);
...
test('pausing recovery blocks settlement retries and incoming gifts until resumed', () => {
  ...
  try {
    ...
    service.replaceRules([{
      giftId: 'blind', giftName: 'Blind', mode: 'random', enabled: true,
...
for (const scenario of [
  ...
  test(`${scenario.name} save failure preserves the committed state and allows retry`, () => {
    const fixture = createFixture();
    const updates = [];
    let service = fixture.createService({ onUpdate: (update) => updates.push(update) });
...
test('pause save failure keeps the original countdown and zero timer', () => {
  ...
  try {
    ...
    assert.equal(
      fixture.db.giftDb.prepare('SELECT remaining_ms FROM overtime_machine_state').get().remaining_ms,
      0,
    );
    assert.deepEqual(updates.map((update) => update.reason), ['finished']);
  } finally {
    service.dispose();
    fixture.close();
  }
...
test('disable transaction failure preserves pending gifts and their recovery timer', () => {
  ...
  try {
    ...
    assert.deepEqual(updates.map((update) => update.reason), ['gift']);
    const progress = fixture.insertProgressGift({ giftId: 'blind', overtimeEpoch: 1 });

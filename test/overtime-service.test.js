...
const { clearAllData, closeDatabases, createDatabases, getSchemaVersions } = require('../src/storage/database');
...
test('gift database v10 preserves v5 overtime state while widening its bounds', () => {
  ...
  try {
    ...
    db.giftDb
      .prepare('UPDATE overtime_machine_state SET remaining_ms = ? WHERE id = 1')
...
test('gift database v10 adds nullable blind_box_id to an existing v8 database', () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'song-plugin-blind-box-id-migration-'));
  let db = createDatabases({ dataDir });
  try {
    db.giftDb.exec(`

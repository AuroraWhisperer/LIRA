...
const item = {
  ...
  giftCount: 2,
  giftTotalPrice: 12.5,
  avatarUrl: 'https://i0.hdslb.com/avatar',
  emotes: [{ text: '[笑]', url: 'https://i0.hdslb.com/emote', kind: 'inline', width: 10, height: 10, secret }],
  secret,
};
const state = {
  secret,
  settings: {
    ...
    songBoardTitle: '歌单',
    overlayTitle: '队列',
    blindboxOverlayTitle: '盲盒',
    desktopLyricFontSize: '56',
    danmakuOverlayStyle: 'glow',
    giftEffectDanmakuEnabled: 'true',
    giftDisplayConfig: secret,
    unknown: secret,
  },
  queue: {
    current: { song_name: '歌', requester_name: '观众', requester_uid: secret, secret },
    waiting: [],
    history: secret,
  },
    ...
    rules: [
      {
        ...
        giftId: '1',
        giftName: '花',
        fixedEffect: { operation: 'add', value: 30, secret },
        outcomes: [{ secret }],
        secret,
      },
    ],
  ...
  lyricState: {
    ...
    generation: 7,
    sequence: 8,
    words: [{ text: '歌', startMs: 0, endMs: 100, secret }],
    secret,
  },
  lyricTimeline: {
    status: 'ready',
    lines: [{ text: '歌', startMs: 0, endMs: 100, translation: 'song', secret }],
    secret,
  },
};
...
test('every scope receives only its own snapshot fields and unknown principals fail closed', () => {
  const allowed = {
    queue: ['settings', 'queue', 'superChats'],
    songlist: ['settings'],
    blindbox: ['settings'],
    overtime: ['overtime'],
  ...
  for (const scope of scopes) {
    const result = projectWebSocketPayload(
      { type: 'overlay', scope },
      { type: 'snapshot', reason: 'connect', state, secret },
    );
...
test('WS event scope matrix strips nested extras while preserving display and drawing payloads', () => {
  const events = [
    ...
    [
      'gift-effects',
      {
        ...
        source: 'danmaku',
        effect: {
          mp4Url: 'https://i0.hdslb.com/effect.mp4',
          layout: {
            videoWidth: 100,
            videoHeight: 100,
            rgbFrame: [0, 0, 50, 100],
            alphaFrame: [50, 0, 50, 100],
            secret,
          },
          secret,
        },
    ...
    [
      'games',
      { type: 'game:update', session: { game: 'number-bomb', state: { min: 1, max: 100, bomb: secret }, secret } },
    ],
    [
      'wheel',
      {
        type: 'wheel:update',
        state: {
          entries: [{ label: '一', weight: 1, secret }],
          spin: { id: 'spin', index: 0, startedAt: 123, durationMs: 5000, turns: 6, secret },
          secret,
        },
      },
    ],
    ...
    [
      'overtime',
      {
        ...
        adjustment: { mode: 'fixed', giftName: '花', effect: { operation: 'add', value: 3, secret }, secret },
      },
    ],
  ];
  ...
  assert.deepEqual(projectWebSocketPayload({ type: 'overlay', scope: 'gift-feed' }, events[1][1]), {
...
test('emote kinds survive public live events and reconnect snapshots', () => {
  ...
  for (const kind of ['inline', 'sticker']) {
    ...
    assert.equal(
      projectWebSocketPayload(principal, { type: 'danmaku:message', item: message }).item.emotes[0].kind,
      kind,
    );
...
test('game HTTP and WS projections hide unrevealed answers independently of the owner DTO', () => {
  const session = {
    ...
    state: {
      ...
      wordLength: 3,
      answer: secret,
      answerAliases: [secret],
      revealedAnswer: secret,
      answerRevealed: false,
      categoryIds: [secret],
      correct: [{ name: '观众', rank: 1, points: 3, secret }],
      scores: [{ name: '观众', score: 3, secret }],
      canvas: {
        revision: 4,
        totalPoints: 1,
        strokes: [{ id: 's', color: '#fff', width: 4, points: [{ x: 0.1, y: 0.2, secret }], secret }],
        secret,
      },
    },
  };
  ...
  assert.deepEqual(
    projectWebSocketPayload({ type: 'overlay', scope: 'games' }, { type: 'game:update', session }).session,
    result,
  );
...
test('REST projections retain display fields, strip nested ledger/config metadata and deny cross-page paths', () => {
  const cases = [
    ...
    [
      ...
      { thresholds: [100, 200, 300], visibleRows: 3, intervalSeconds: 4, paused: false, lowPower: true, secret },
    ],
    [
      'gift-feed',
        ...
        gifts: [
          {
            id: 1,
            name: '花',
            imagePath: '/overtime-gift-images/1.webp',
            giftIdentity: { variantId: 'variant', secret },
            secret,
          },
        ],
        sourceId: secret,
    ...
    [
      'clock',
      '/api/clock/config',
      { style: 'digital', showDate: true, showSeconds: true, hourFormat: '24', label: '时钟', secret },
    ],
    [
      'opening',
      '/api/opening/config',
    ...
    [
      'wheel',
      '/api/wheel/spin',
      { entries: [{ label: '一', weight: 1, secret }], spin: null, lastResult: { index: 0, secret }, secret },
    ],
  ];
...
test('dynamic queue styles, songboard theme fallback and lyric settings survive explicit key projection', () => {
  ...
  for (const prefix of ['storybook', 'neonVinyl', 'cherryRibbon', 'goldenLily']) {
    Object.assign(input, {
      [`${prefix}QueueFontSize`]: '39',
      [`${prefix}QueueFontFamily`]: 'Serif',
      [`${prefix}QueueFontWeight`]: '600',
      [`${prefix}QueueUseCustomTextColor`]: 'true',
      [`${prefix}QueueTextColor`]: '#123456',
      [`${prefix}QueueScrollMode`]: 'loop',
      [`${prefix}QueueScrollSpeed`]: '45',
    });
  }
...
test('allowlisted scalar names cannot smuggle nested objects and admin messages remain unchanged', () => {
  const result = projectOverlayState('queue', {
    settings: { overlayTitle: { secret } },
    queue: { current: { song_name: { secret } }, waiting: [] },
  });
  assert.doesNotMatch(JSON.stringify(result), /PRIVATE_SENTINEL/);
  ...
  for (const scope of scopes) {
    ...
    assert.deepEqual(
      projectWebSocketPayload({ type: 'overlay', scope }, { type: 'shutdown', reason: 'restart', state, secret }),
...
test('actual shared gift banner renders the same avatar, guard, artwork, amount and text from projected REST data', () => {
  ...
  const catalog = {
    gifts: [{ id: 1, name: '花', variantId: 'v1', imagePath: '/overtime-gift-images/1.webp', secret }],
  };
  const config = {

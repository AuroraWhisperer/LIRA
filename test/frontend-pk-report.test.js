...
async function fixture(profile = account('one')) {
  ...
  const get = (key) => {
    if (!nodes.has(key)) nodes.set(key, { checked: false, disabled: false, textContent: '', events: {},
  ...
  const module = new vm.SourceTextModule(fs.readFileSync(path.join(__dirname, '../public/js/admin/danmaku-pk-report.js'), 'utf8'), { context: vm.createContext({}) });
  await module.link(() => {}); await module.evaluate();
  module.namespace.initDanmakuPkReport({ documentRef: { getElementById: (id) => get(id.replace('danmakuPkReport', '')) },
    windowRef: get('window'), bridge: { getProfile: async () => profile,
      onStateChanged: (fn) => { listener = fn; return () => { unsubscribed = true; }; },
      getPkReportSettings: () => new Promise((resolve, reject) => reads.push({ resolve, reject })),
      updatePkReportSettings: (patch) => new Promise((resolve, reject) => writes.push({ patch: JSON.parse(JSON.stringify(patch)), resolve, reject })),
    } });
  await flush();
  return { get, reads, writes, account: (name) => listener(account(name)),
    toggle: (value) => { get('Toggle').checked = value; get('Toggle').events.change(); },
...
test('pending and failed writes preserve confirmed state, failed disable warns explicitly', async () => {
  ...
  f.reads[0].resolve(saved(false)); await flush();
  f.toggle(true);
  ...
  f.toggle(false); f.writes[1].reject(new Error('offline')); await flush();
  assert.equal(f.get('Toggle').checked, true);
...
test('old server is unavailable and explicit refresh can recover', async () => {
  const f = await fixture();
  f.reads[0].resolve({ ok: false, error: 'NOT_FOUND' }); await flush();
  assert.equal(f.get('Toggle').disabled, true);
  assert.match(f.get('Status').textContent, /暂不可用/);
  f.get('RefreshBtn').events.click();
  f.reads[1].resolve(saved(false)); await flush();
...
test('account changes and page disposal discard late reads and writes', async () => {
  ...
  assert.equal(f.get('Toggle').checked, false);
  f.reads[1].resolve(saved(false)); await flush();
  f.toggle(true); f.account('three');
  f.writes[0].resolve(saved(true)); await flush();
  assert.equal(f.get('Toggle').checked, false);
  assert.equal(f.get('Toggle').disabled, true);
  f.get('window').events.pagehide();
  f.reads[2].resolve(saved(true)); await flush();

    ...
    for (const game of ['gomoku', 'number-bomb']) {
      ...
      try {
        ...
        const result = service.handleDanmaku({
          uid: 'host', userName: '主播', isStreamer: true, message: value,
        });
        ...
        if (mode === 'multi') {
          assert.equal(service.handleDanmaku({
            uid: 'viewer', userName: '观众', message: value,
          }).accepted, true);
        }
      } finally {
        service.dispose();

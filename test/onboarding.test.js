...
let onboarding;
test.before(async () => {
  onboarding = await loadModuleExports(
    path.join(__dirname, '..', 'public', 'js', 'admin', 'onboarding.js'),
    { window: {} },
  );
});
...
test('onboarding state gates required steps and optional AI', () => {
  assert.equal(
    onboarding.getStepGate('bilibili', {
      bilibiliAvailable: false,
  ...
  assert.equal(
    onboarding.getStepGate('import', { importAcknowledged: false }),
    false,
  );
  assert.equal(
    onboarding.getStepGate('import', { importAcknowledged: true }),
    true,
  );
  assert.equal(onboarding.getStepGate('ai', { aiEnabled: false }), true);
  assert.equal(
    onboarding.getStepGate('ai', {
      aiEnabled: true,
...
test('onboarding navigation stays within bounds and tracks optional omissions', () => {
  ...
  assert.equal(
    onboarding.getNextStep('bilibili', { bilibiliAvailable: false }),
    'bilibili',
  );
  assert.equal(onboarding.getNextStep('complete', {}), 'complete');
  assert.equal(
    JSON.stringify(onboarding.getIncompleteOptionalSteps({ aiEnabled: false })),
    JSON.stringify(['ai']),
  );
  assert.equal(
    JSON.stringify(
      onboarding.getIncompleteOptionalSteps({
        aiEnabled: false,
        skippedOptional: ['ai'],
      }),
    ),
...
test('normalizeOnboardingState keeps the Quanmin host-login platform distinct', () => {
  const state = onboarding.normalizeOnboardingState({
    musicPlatform: 'wesing',
  });

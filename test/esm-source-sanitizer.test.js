...
const { sanitizeSource, auditPublicEsModules } = require('./helpers/esm-scope-audit');

function assertLocations(source, sanitized, names) {
  assert.equal(sanitized.length, source.length);
  for (let index = 0; index < source.length; index += 1) {
    if (source[index] === '\n' || source[index] === '\r') assert.equal(sanitized[index], source[index]);
  }
  for (const name of names) assert.equal(sanitized.indexOf(name), source.indexOf(name), name);
}
...
test('source sanitizer keeps UTF-16 offsets and line endings through nested template interpolations', () => {
  ...
  assertLocations(source, sanitized, ['first', 'second', 'third', 'fourth', 'finalReference']);
...
test('source sanitizer masks strings, comments, and regex classes without masking division operands', () => {
  ...
  assert.doesNotMatch(sanitized, /regexText|stringName|commentName|blockName|nextCommentLine|returnRegex/);
...
test('source sanitizer retains references in interpolations containing braces, regex, and division', () => {
  ...
  const sanitized = sanitizeSource(source);
  assertLocations(source, sanitized, ['format', 'input', 'inner', 'total', 'divisor', 'after']);
});

test('scope audit ignores numeric literal suffixes and reports unresolved interpolation references at original lines', (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-esm-sanitizer-'));
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  const file = path.join(directory, 'sample.js');
  fs.writeFileSync(
    file,
    ['export const number = 0x8000 + 0b10 + 0o10 + 1e3 + 10n;', 'export const text = `escaped \\', '${missing}`;'].join(
      '\n',
    ),
  );

...
test('playback page offers a dedicated WeSing source and cache capture workspace', () => {
  ...
  assert.match(
    html,
    /data-source="wesing"[\s\S]*src="\/img\/playback\/wesing-icon\.jpg"/,
  );
  ...
  assert.match(
    html,
    /id="weSingLyricOffsetMs"[^>]*min="-3000"[^>]*max="3000"[^>]*step="50"/,
  );
  assert.match(
    html,
    /id="weSingLyricOffsetMsNumber"[^>]*min="-3000"[^>]*max="3000"[^>]*step="50"/,
  );
...
test('WeSing browser client activates capture and renders WebSocket lyrics safely', () => {
  ...
  assert.match(
    source,
    /pendingLyricOffsetMs \?\? numberValue\(this\.status\.lyricOffsetMs, 0\)/,
  );
...
test('WeSing lyric-offset preview survives older live status while saving', async () => {
  ...
  const document = {
    ...
    getElementById(id) {
      return (
        { weSingLyricOffsetMs: range, weSingLyricOffsetMsNumber: number }[id] ||
        null
      );
    },
  };

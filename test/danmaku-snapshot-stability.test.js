...
async function fixture() {
  ...
  await loadModuleExports(path.resolve(__dirname, '../public/js/overlays/danmaku.js'), {
    ...
    setTimeout(callback, delay) {
      const id = ++sequence; timers.set(id, { callback, deadline: now + delay }); return id;
    },
  ...
  sockets[0].emit('open');
  return {
    root: element('danmakuFeed'), timers, frames, sockets,
    snapshot(items, style = 'signal', duration = 6) {
      sockets.at(-1).emit('message', { data: JSON.stringify({ type: 'snapshot', state: {
        danmakuFeed: items, settings: { danmakuOverlayStyle: style, danmakuFullscreenDurationSeconds: duration },
        liveStatus: { enabled: true, roomId: '1', connected: true },
      } }) });
    },
    append(item) { sockets.at(-1).emit('message', { data: JSON.stringify({ type: 'danmaku:message', item }) }); },
    flush() {
      while (frames.size) {
...
test('unchanged snapshots preserve actual nodes and expiration timers in all nine styles', async () => {
  for (const style of ['bubble', 'signal', 'minimal', 'ranked', 'transparent', 'identity', 'outline', 'cream', 'glow']) {
    const f = await fixture();
    const items = [message('one')];
    f.snapshot(items, style); f.flush();
...
test('matching snapshot preserves queued incremental append and does not duplicate it', async () => {
  const f = await fixture(); const first = message('one'); const second = message('two');
  f.snapshot([first]); f.flush(); const retained = f.root.children[0];
  f.append(second);
  ...
  f.append(second); f.flush(); assert.equal(f.root.children.length, 2);
});

test('first empty snapshot, content corrections, clear and feed configuration changes still render', async () => {
  const f = await fixture();
  f.snapshot([]); f.flush(); assert.equal(f.root.children[0].textContent, '等待直播消息…');
  const first = message('one');
  f.snapshot([first]); f.flush(); let node = f.root.children[0];
  f.snapshot([message('one', { message: '更正正文' })]); f.flush();
  assert.notEqual(f.root.children[0], node);
  f.snapshot([first], 'outline'); f.flush(); node = f.root.children[0];
  f.snapshot([first], 'outline', 8); f.flush(); assert.notEqual(f.root.children[0], node);
  assert.equal([...f.timers.values()][0].deadline, 18000);
  f.snapshot([], 'outline', 8); f.flush();
...
test('reconnect restores new snapshot contents, and the message window remains bounded and deduplicated', async () => {
  const f = await fixture(); const first = message('one');
  f.snapshot([first]); f.flush();
  f.sockets[0].emit('close'); f.advance(800); f.sockets.at(-1).emit('open');
  f.snapshot([first, message('two')]); f.flush(); assert.equal(f.root.children.length, 2);
  f.root.clientHeight = 10000;
  const items = Array.from({ length: 55 }, (_, index) => message(index + 1));
  f.snapshot([...items, items[0]]); f.flush(); assert.equal(f.root.children.length, 50);
  const retained = f.root.children.at(-1);
  f.snapshot(items); f.flush(); assert.equal(f.root.children.at(-1), retained);
});

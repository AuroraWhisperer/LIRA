...
test('games admin uses the restored single-column card layout', () => {
  const styles = fs.readFileSync(path.join(ROOT_DIR, 'public', 'css', 'admin', 'other-features', 'games.css'), 'utf8');
...
test('admin game styles keep shared, wheel, draw, and responsive ownership', () => {
  const readGameStyle = (name) =>
    fs.readFileSync(path.join(ROOT_DIR, 'public', 'css', 'admin', 'other-features', name), 'utf8');
...
test('games admin gives the word library a compact selectable shelf', () => {
  const styles = fs.readFileSync(
    path.join(ROOT_DIR, 'public', 'css', 'admin', 'other-features', 'games-draw.css'),
    'utf8',
  );

...
const makeItem = (eventId, options = {}) => ({ eventId, gift: { giftId: '1', giftName: '礼物',
  userName: '旧名字', num: 1, unitPrice: 10, totalPrice: 10, guardLevel: 3,
  createdAt: '2026-09-19T01:00:00Z', ...options } });
const evidence = (item, senderId = '100', changes = {}) => ({ eventId: item.eventId, senderId,
  userName: item.gift.userName, avatarUrl: null, guardLevel: item.gift.guardLevel,
  createdAt: item.gift.createdAt, ...changes });

test('today cards merge by UID and exact gift ID/name using exact historical values without changing inputs', () => {
  const items = [makeItem('a', { num: 2, unitPrice: 10 }), makeItem('b', { num: 3, unitPrice: 20 }),
    makeItem('c', { giftId: '2' }), makeItem('d', { giftName: '另一个礼物' }), makeItem('e')];
  ...
  assert.equal(buildGiftCards([...items, makeItem('new')], { day, profiles: [...profiles, evidence(makeItem('new'))] })[0].eventId, result[0].eventId);
});
...
test('latest known name and rank apply to every today card including selected earlier records', () => {
  ...
  profiles.push(evidence(makeItem('unselected'), '100', { userName: '新名字', guardLevel: 2, createdAt: '2026-09-19T02:00:00Z' }));
  ...
  profiles.push(evidence(makeItem('unknown'), '100', { userName: '', guardLevel: null, createdAt: '2026-09-19T03:00:00Z' }));
  const previous = buildGiftCards(items, { day, profiles });
  profiles.push(evidence(makeItem('renewal'), '100', { userName: '新名字', guardLevel: 0, createdAt: '2026-09-19T04:00:00Z' }));
...
test('merged values retain cent precision and large quantities without using an averaged unit price', () => {
  const items = [makeItem('a', { unitPrice: 0.01, num: Number.MAX_SAFE_INTEGER }), makeItem('b', { unitPrice: 0.02, num: 2 })];

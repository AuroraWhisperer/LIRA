...
} = require('../src/electron/dynamic-lottery-session');
...
test('lottery session fences authorization and Bilibili credential changes', async () => {
  ...
  let cookieHeader =
    'DedeUserID=9007199254740993123; SESSDATA=session-a; bili_jct=csrf-a; sid=one';
  ...
  cookieHeader =
    'sid=two; bili_jct=csrf-a; SESSDATA=session-a; DedeUserID=9007199254740993123';
  assert.equal((await session.getContext()).sessionEpoch, 1);

  cookieHeader =
    'DedeUserID=9007199254740993123; SESSDATA=session-b; bili_jct=csrf-a';
...
test('lottery session invalidates logout and rejects use after disposal', async () => {
  let cookieHeader =
    'DedeUserID=123; SESSDATA=session-a; bili_jct=csrf-a';
  const session = createLotterySession({
    getCookieHeader: async () => cookieHeader,
  ...
  await assert.rejects(
    session.getContext(),
    (error) => error.code === 'LOTTERY_SESSION_UNAVAILABLE',
  );
  ...
  await assert.rejects(
    session.getContext(),
    (error) => error.code === 'LOTTERY_SESSION_DISPOSED',
  );
});
...
test('lottery session requires a trusted LIRA identity', async () => {
  ...
  await assert.rejects(
    session.getContext(),
    (error) => error.code === 'LOTTERY_IDENTITY_UNAVAILABLE',
  );
});

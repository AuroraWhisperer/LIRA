...
function setup(t, remote) {
  ...
  const source = {
    ...
    blessings: fixture.snapshot.blessings,
    fortunes: fixture.snapshot.fortunes,
  };
  const manager = {
    ...
    dailyBotRequestInternal: remote,
  };
  const controller = createDailyBotController({
    licenseManager: manager,
    sourceLabel: 'synthetic-local-source',
    now: () => Date.parse(fixture.snapshot.cutoffAt),
    getLegacyReader: () => ({
      summary: () => ({
        count: source.checkins.length,
        minDays: 128,
        maxDays: 128,
        lastDate: '2026-09-18',
        customLibraries: true,
      }),
...
test('main forwards direct enabling without a legacy decision and accepts automatic readiness', async (t) => {
  ...
  const { controller } = setup(t, async (operation, input) => {
    ...
    return {
      ...initial,
      checkin: { enabled: true, revision: 1, reason: 'running' },
...
test('main rejects stale contexts and unconfirmed migration and discards late account responses', async (t) => {
  let resolve;
  const { controller, switchAccount } = setup(
    t,
    () =>
      new Promise((done) => {
        resolve = done;
  ...
  switchAccount();
  resolve(fixture.defaultResponse);
...
test('migration stages only confirmed local snapshot, detects resumed old writers, and never adds a second base', async (t) => {
  ...
  await assert.rejects(
    invoke('decide', { decision: 'no-legacy', expectedRevision: 0, legacyStoppedConfirmed: true }),
    /LEGACY_PRESENT/,
  );
  const draft = await invoke('prepare', {
  ...
  assert.equal(
    calls.some((call) => call.operation === 'commit'),
    false,
  );
});
test('restricted IPC refuses other windows/frames/origins and never returns arbitrary errors', async () => {
  ...
  const frame = { url: 'http://127.0.0.1:3000/admin' },
    win = { isDestroyed: () => false, webContents: { mainFrame: frame } };
  const dispose = registerDailyBotIpc({
    ipcMain: {
      handle: (_name, fn) => {
        handler = fn;
      },
  ...
  assert.deepEqual(await handler({ sender: win.webContents, senderFrame: frame }, {}), {
    ok: false,
    error: 'DAILY_BOT_UNAVAILABLE',
  });

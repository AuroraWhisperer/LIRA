...
test('amount threshold filters record totals strictly before pagination and selection', (t) => {
  ...
  for (const [id, totalPrice] of [['below', 9.99], ['equal', 10], ['above', 10.01], ['batch', 20]]) {
    fx.insertGift(source.id, id, { totalPrice, unitPrice: id === 'batch' ? 1 : totalPrice, num: id === 'batch' ? 20 : 1, userName: '小明', giftName: '小礼物' });
  }
  ...
  assert.throws(() => getGiftHistory(fx.context, { ...options, amountAbove: 9, cursor: first.nextCursor }), { code: 'INVALID_GIFT_CURSOR' });
  const selectionOptions = { ...options, viewRevision: first.viewRevision };
  assert.deepEqual(getGiftSelection(fx.context, selectionOptions).items.map((item) => item.eventId), ['batch', 'above']);
  assert.throws(() => getGiftSelection(fx.context, { ...selectionOptions, eventIds: ['equal'] }), { code: 'GIFT_VIEW_STALE' });
...
test('amount thresholds validate money and preserve cursors without a threshold', (t) => {
  const fx = createFixture(); t.after(() => fx.close());
  const source = fx.resolveSource('a'.repeat(64)); fx.setActiveSource(source.id);
  fx.insertGift(source.id, 'first'); fx.insertGift(source.id, 'second');
  const first = getGiftHistory(fx.context, { range: 'all', limit: 1 });
  ...
  assert.equal(getGiftHistory(fx.context, { range: 'all', limit: 1, amountAbove: '', cursor: first.nextCursor }).items.length, 1);
  for (const amountAbove of [-1, 'NaN', Infinity, '0.001', '1 OR 1=1', {}, [], true, Number.MAX_SAFE_INTEGER]) {
    ...
    assert.throws(() => getGiftSelection(fx.context, { amountAbove, viewRevision: first.viewRevision }), { code: 'INVALID_GIFT_FILTER' });
  }
});

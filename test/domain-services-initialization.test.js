...
function fixture({ failure = '', cleanupFails = false, shared = true } = {}) {
  ...
  db.giftDb
    .prepare(
      `UPDATE overtime_machine_state SET enabled=1, enable_epoch=1,
  ...
  const remoteCatalog = {
    ...
    refresh: async () => null,
    start: () => events.push('remote:start'),
    stop: stopBorrowed,
  };
  const initializer = {
  ...
  const domain = loadModule(
    'src/server/domain-services.js',
    {
      '../overtime': {
        ...overtimeModule,
        createOvertimeService(options) {
          const store = createOvertimeStore(options.giftDb);
          if (phase === 'recovery')
            store.listRecoverableFinal = () => {
              throw originalError;
            };
          const service = overtimeModule.createOvertimeService({
            ...options,
            store,
            now: clock.now,
      ...
      '../bilibili/gift': {
        ...
        createGiftService(context, options) {
          ...
          if (phase === 'gift-result')
            Object.defineProperty(result, 'assemblyProbe', {
              enumerable: true,
              get() {
                throw originalError;
              },
  ...
  return {
    ...
    retry() {
      phase = '';
      return domain.createDomainServices(options);
    },
    closeDatabases() {
      closeDatabases(db);
      closed = true;
...
test('server startup failure disposes the domain before closing its database handles', async () => {
  ...
  const serverModule = loadModule('src/server.js', {
    ...
    './storage/database': {
      ...
      createDatabases(options) {
        acquired = createDatabases(options);
        return acquired;
      },
      closeDatabases(db) {
        assert.equal(f.clock.timers.size, 0);
  ...
  try {
    await assert.rejects(
      runtime.start({ startPort: 0, remoteGiftCatalog: f.remoteOptions }),

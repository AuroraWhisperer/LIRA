...
test('subscriptions report only projected material changes and generic room invalidation', () => {
  ...
  service.subscribe((event) => roomEvents.push(event), {
    fields: ['fansMedal'],
  });
  service.subscribe((event) => explicitRoomEvents.push(event), {
    roomId: '100',
    fields: ['fansMedal'],
  });
...
test('room generation and run token reject stale mixed hints and snapshots', () => {
  ...
  assert.deepEqual(
    service.listOnline().map((item) => item.uid),
    [],
  );
  ...
  const staleRunResult = service.ingestHint(
    { uid: '789', name: 'Old run' },
    {
      ...currentRun,
...
test('ensure deduplicates profile requests, negative-caches failures, and protects explicit callers', async () => {
  ...
  const nameRequest = service.ensure('123', {
    fields: ['name'],
    roomId: '100',
  });

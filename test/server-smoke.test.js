...
} = require('./helpers/server-smoke-scenarios');
...
test('server keeps its core HTTP, state, song and queue behavior', async () => {
  ...
  try {
    ...
    const connection = { baseUrl: app.baseUrl, token: _testToken };
    await assertSmokeHealth(connection, dataDir);
    await assertSmokePagesAndExports(connection);
    await assertSmokeLyrics(connection);
    await assertSmokeSongsAndSettings(connection);
    await assertSmokeQueueAndClearing(connection);
  } finally {

...
test('V2 IPC projects all typed fields, rejects bad patches, invalid success and untrusted frames', async () => {
  ...
  const read = handlers.get('license:get-welcome-settings-v2'),
    write = handlers.get('license:update-welcome-settings-v2');
  ...
  for (const invalid of [
    { ...settings, greetingDelaySeconds: '10' },
    { ...settings, greetingEnabled: true },
    { ...settings, welcomeDelaySeconds: undefined },
...
test('V2 uses fixed endpoints, carries only safe field errors and rejects late account writes', async () => {
  ...
  for (const operation of [
    () => remote.getWelcomeSettingsV2('synthetic'),
  ...
  assert.ok(
    calls.every(
      ({ url, init }) =>
        url === 'https://api.example.test/api/device/welcome-settings/v2' &&
        init.headers.Authorization === 'Bearer synthetic',
    ),
  );
  let owner = 'one',
    finish;
  ...
  const pending = operations.updateWelcomeSettingsV2({ enabled: true });
  owner = 'two';
  finish(settings);

...
test('filter IPC requires the main frame, validates patches and strips private response fields', async () => {
  ...
  const settings = {
    blockedUsers: [{ uid: '123', name: '观众', cookie: 'private' }],
    blockedKeywords: ['广告'],
    token: 'secret',
  };
  registerLicenseIpc({
  ...
  const read = handlers.get('license:get-overlay-filters'),
    update = handlers.get('license:update-overlay-filters');
  ...
  for (const patch of [
    ...
    { streamerId: 2 },
    { blockedKeywords: [''] },
    { blockedUsers: [{ uid: 123, name: '' }] },
    { blockedUsers: settings.blockedUsers },
    { blockedKeywords: ['x'.repeat(101)] },
    { blockedKeywords: Array(201).fill('x') },
  ]) {
...
test('new remote operations use only fixed Device URLs with main-process bearer auth', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ['/api/device/overlay-filters', 'GET'],
      ['/api/device/overlay-filters', 'PUT'],
      ['/api/device/overlay-viewers', 'GET'],
    ],
  );

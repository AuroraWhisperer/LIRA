...
const { inspectInteractionText, validateInteractionConfig, pollPageDuration } = require('../public/js/shared/interaction-rules.js');
...
function fixture() {
  ...
  const updates = [];
  let games;
  const service = createInteractionSessionService({ now: () => time, wallNow: () => time,
    getSourceState: () => source, isGameActive: () => games.isActive(),
    subscribe(fn) { listener = fn; return () => { listener = null; }; },
    setTimeout(fn, delay) { const key = {}; tasks.set(key, { fn, delay }); return key; }, clearTimeout: (key) => tasks.delete(key),
    broadcast: (event) => updates.push(event),
  });
  games = createGameSessionService({ isInteractionCollecting: () => service.isCollecting() });
  return { service, source, tasks, updates, games, setTime(value) { time = value; }, get listener() { return listener; },
    send(uid, message, extra = {}) { listener?.({ ...source, uid, message, source: 'danmaku', receivedAt: time, ...extra }); },
  };
}
...
test('shared Unicode rules preserve visible graphemes and reject invisible input without deletion', () => {
  ...
  for (const value of ['\u200b', '唱\u200b歌', 'A\nB', '\u0000', '\u0301', 'a\u2060b', 'a\u200db', 'a\ufe0f', 'a\ufeffb', '\u202eabc']) assert.ok(inspectInteractionText(value).error, value);
  assert.equal(inspectInteractionText(' 唱 歌 ').text, '唱 歌');
  assert.throws(() => validateInteractionConfig({ ...poll, options: ['é', 'e\u0301'] }), /重复/);
  for (const options of [[], ['a'], ['a', ''], ['a', 'a']]) assert.throws(() => validateInteractionConfig({ ...poll, options }));
  assert.equal(validateInteractionConfig({ ...poll, options: Array.from({ length: 20 }, (_, i) => String(i)) }).options.length, 20);
  assert.throws(() => validateInteractionConfig({ ...poll, options: Array.from({ length: 3000 }, (_, i) => String(i)) }), /16 KiB|64 KiB/);
...
test('poll accepts the first complete valid choice per reliable viewer and preserves order', () => {
  ...
  f.send('1', 'hello'); f.send('1', '唱\u200b歌'); f.send('1', ' 唱歌 '); f.send('1', '聊天');
  f.send('2', '聊天'); f.send('300', '唱歌'); f.send('0', '唱歌'); f.send('masked', '唱歌'); f.send(9007199254740992, '唱歌');
  const state = f.service.getState();
  assert.deepEqual(state.session.options.map((item) => item.votes), [1, 1]);
...
test('rating keeps final valid score, rejects stale timestamps/replays and hides all aggregate inputs until finish', () => {
  ...
  f.send('2', '6'); f.send('2', '文字'); f.send('3', '9');
...
test('receiving window excludes pre-start frames and deadline equality despite late timer', () => {
  ...
  f.setTime(1999); f.send('1', '唱歌');
  f.setTime(2000); f.send('2', '聊天', { receivedAt: 1999 });
...
test('disconnect retains scores, same-room reconnect continues, identity switch never resumes old session', () => {
  ...
  const id = f.service.start({ kind: 'rating' }).session.sessionId;
  f.send('1', '8');
  f.source.ready = false; f.service.sourceChanged();
  f.send('2', '10');
  f.source.ready = true; f.source.connectionKey = '2'; f.service.sourceChanged();
  f.send('2', '6');
  f.source.accountUid = '201'; f.service.sourceChanged();
  assert.equal(f.service.getState().session.phase, 'interrupted');
  assert.equal(f.listener, null);
  f.source.accountUid = '200'; f.service.sourceChanged();
  assert.equal(f.service.getState().session.phase, 'interrupted');
...
test('runtime broadcasts restart availability when interaction collection starts and stops', (t) => {
  ...
  const { games, interactions } = createGameRuntime({
    broadcast: (event) => updates.push(event), getSourceState: () => source,
    subscribe: () => () => {},
  });
  t.after(() => { interactions.dispose(); games.dispose(); });

...
async function fixture({ reloadError } = {}) {
  ...
  function node() {
    return {
      value: '', checked: false, hidden: false, disabled: false, files: [], textContent: '', children: [], events: {},
  ...
  const module = new vm.SourceTextModule(fs.readFileSync(require.resolve('../public/js/admin/song-import-update.js'), 'utf8'), { context });
  await module.link(() => new vm.SyntheticModule(['api'], function () { this.setExport('api', () => {}); }, { context }));
  await module.evaluate();
  module.namespace.initSongImportUpdate({
    documentRef: { getElementById: element, createElement: node },
    imports: { parseTable: (text) => JSON.parse(text), readTextFile: async (file) => file.text, readFileAsBase64: async (file) => file.base64 },
    reloadSongs: async () => { reloadCount += 1; if (reloadError) throw reloadError; },
    request: (url, payload) => new Promise((resolve, reject) => calls.push({ url, payload, resolve, reject })),
  });
  function updateMode() { element('songImportMode').value = 'update'; element('songImportMode').events.change(); }
  function input(rows) { element('importText').value = JSON.stringify(rows); element('importText').events.input(); }
  ...
  return { element, calls, updateMode, input, preview, reloadCount: () => reloadCount };
}

function previewData(count = 1, canApply = true) {
  return { previewToken: 'original-token', canApply, counts: { inserted: count, updated: 0, unchanged: 0, conflict: 0, invalid: 0 },
    rows: Array.from({ length: count }, (_, index) => ({ row: index + 1, name: '<script>歌</script>', artist: '', status: 'inserted',
...
test('update UI defaults to add-only and paginates every preview row without inserting untrusted HTML', async () => {
  ...
  assert.equal(ui.element('songImportPreviewRows').children[0].children[1].textContent, '<script>歌</script> / （无歌手）');
...
test('confirmation submits the reviewed input and token once, while stale errors require a new preview', async () => {
  ...
  assert.deepEqual(ui.calls[1].payload.rows, [{ name: '歌', requestPrice: '' }]);

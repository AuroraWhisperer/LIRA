...
function member(uid, extra = {}) {
  return {
    ...
    uinfo: {
      uid,
      base: { name: `虚构粉丝${uid}`, face: 'https://i0.hdslb.com/bfs/face/fixture.jpg' },
      guard: { level: 3 },
    },
    ...extra,
  };
...
test('guard roster resolves short room ID and owner, merges top3 and all pages without credentials', async () => {
  ...
  assert.equal(
    f.requests.every((r) => !r.options.headers.Cookie),
    true,
  );
  assert.equal(
    f.requests.every((r) => r.options.signal instanceof AbortSignal),
    true,
  );
});
...
test('guard roster reads only valid fan medal levels belonging to the room owner', async () => {
  ...
  assert.deepEqual(
    result.members.map((m) => m.medalLevel),
...
test('guard roster rejects partial pages, changing totals, and another room owner', async () => {
  for (const pages of [
    [
      { info: { num: 2 }, top3: [], list: [member(1)] },
      { info: { num: 2 }, list: [] },
    ],
    [
      { info: { num: 2 }, top3: [], list: [member(1)] },
      { info: { num: 2 }, list: [member(1)] },
    ],
    [
      { info: { num: 2 }, top3: [], list: [member(1)] },
      { info: { num: 3 }, list: [member(2)] },
    ],

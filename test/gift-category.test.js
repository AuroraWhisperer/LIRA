...
function sign(catalog) {
  const variants = catalog.variants.map(({ firstSeenAt, lastSeenAt, firstSeenRunId, lastSeenRunId, ...gift }) => gift);
  catalog.version = `sha256:${crypto
    .createHash('sha256')
    .update(JSON.stringify({ variants, blindBoxes: catalog.blindBoxes }))
    .digest('hex')}`;
  return catalog;
}
...
function categoryCatalog() {
  ...
  catalog.variants = [
    ...
    catalog.variants[1],
    catalog.variants[2],
    structuredClone(catalog.variants[0]),
  ].map((gift, index) => {
  ...
  catalog.blindBoxes = [0, 3].map((index) => ({
    ...
    outputVariantIds: [catalog.variants[1].variantId],
    awards: [],
  }));
  return sign(catalog);
}
...
test('all named categories and shared pools survive normalization, cache and restart', async (t) => {
  ...
  assert.deepEqual(
    snapshot.gifts.map((gift) => gift.giftCategory),
...
test('missing, old numeric and contradictory categories never replace a successful snapshot', async (t) => {
  ...
  for (const mutate of [
    (value) => {
      value.variants[1].giftCategory = 'directGift';
    },
    (value) => {
      value.variants[0].giftCategory = 'directGift';
    },
    (value) => {
      value.variants[2].isBlindBox = false;
    },
  ]) {
    response = categoryCatalog();
    mutate(response);

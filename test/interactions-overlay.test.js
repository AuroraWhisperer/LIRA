...
test('live poll updates preserve row nodes and scroll position while showing zero and small percentages', async () => {
  ...
  assert.equal(row.children[0].textContent, '<img>');
  assert.equal(row.children[1].children[0].style.width, '99.5%');
  assert.equal(container.children[1].children[1].children[1].textContent, '1 票 · 0.5%');
  session.phase = 'finished';
  renderPollRows(container, session);
  assert.match(row.children[0].textContent, /最高票/);
  ...
  assert.doesNotMatch(container.children[0].children[0].textContent, /最高/);
});

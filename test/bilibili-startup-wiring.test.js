...
test('server composition exposes only processed gifts and leaves legacy history alone', () => {
  ...
  const domainSource = fs.readFileSync(
    path.join(ROOT_DIR, 'src', 'server', 'domain-services.js'),
    'utf8',
  );
  assert.doesNotMatch(domainSource, /processedOnly/);
  const giftSource = fs.readFileSync(
    path.join(ROOT_DIR, 'src', 'bilibili', 'gift', 'index.js'),
    'utf8',
  );
  assert.match(giftSource, /createGiftProjectionService/);
  assert.doesNotMatch(
    giftSource,
    /createGiftDetectionService|repairGiftV2Events/,
  );
  assert.doesNotMatch(
    source,
    /LOCAL_GIFT_DETECTION_ENABLED|giftDetectionEnabled/,
  );

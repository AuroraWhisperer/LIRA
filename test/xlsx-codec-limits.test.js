...
test('ZIP enforces cumulative output while inflating the next entry', (t) => {
  ...
  assert.deepEqual(
    inflate.mock.calls.map((call) => call.arguments[1].maxOutputLength),
    [64, 32],
  );
});
...
test('ZIP supports data descriptors with and without a signature and EOCD comments', () => {
  for (const signature of [true, false]) {
    const zip = zipFixture([{ name: 'a', text: '中文' }], {
      descriptor: true,
      signature,
    });
...
test('ZIP rejects understated output and corrupt data descriptors, including local metadata', () => {
  ...
  assert.equal(
    readZipFiles(
      ...
      { limits: { entryBytes: 1, totalBytes: 1 } },
    ).get('empty'),
    '',
  );
});
...
test('shared strings and expanded worksheet text obey count and text budgets', () => {
  assert.deepEqual(
    parseSharedStrings('<si/><si><r><t>中</t></r><r><t>文</t></r></si>', {
      sharedStrings: 2,
      textChars: 2,
    }),
    ['', '中文'],
  ...
  assert.throws(
    () =>
      parseSharedStrings(sharedXml, {
        entryBytes: Buffer.byteLength(sharedXml) - 1,
      }),
    /Excel/,
  );
  assert.deepEqual(
  ...
  const zip = zipFixture([
    {
      ...
      text: '<x:sst xmlns:x="urn:test"><x:si><x:t>歌曲名字</x:t></x:si><x:si/><x:si><x:r><x:t>中文</x:t></x:r><x:r><x:t>&amp;&#x1F600;</x:t></x:r></x:si></x:sst>',
    },
    {
      name: 'xl/worksheets/sheet1.xml',
      text:
...
test('failed XLSX import causes no writes, broadcasts or sync and next import works', async () => {
  ...
  const context = {
    songs: {
      import(rows) {
        effects.push(['write', rows]);
        return { inserted: rows.length };
      },
    },
    broadcastSnapshot(event) {
      effects.push(['broadcast', event]);
    },
    cloudSync: {
      request(scope) {
        effects.push(['sync', scope]);
      },
    },
  };

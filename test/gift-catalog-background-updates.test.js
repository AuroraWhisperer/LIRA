...
test('authorized startup checks the persisted ETag without downloading unchanged images', async () => {
  ...
  try {
    ...
    await service.initializeGlobalCatalog({
      force: true,
      reason: 'authorized-startup',
    });
    service.stop();
    service = createService(options);
...
test('publishes local artwork only after downloads complete and preserves room membership', async () => {
  ...
  try {
    ...
    const started = new Promise((resolve) => {
      beginDownload = resolve;
    });
    let beginSecondDownload;
    const secondStarted = new Promise((resolve) => {
      beginSecondDownload = resolve;
    });
    let downloadCalls = 0;
    let releaseDownload;
    service = createService({
...
test('identity changes recheck artwork and incremental progress counts only changed artwork', async () => {
  ...
  try {
    ...
    assert.equal(
      states
        .filter((state) => state.phase === 'images')
        .every((state) => state.status === 'updating' && state.total === 1),
      true,
    );
    assert.equal(service.getInitializationState().total, 1);

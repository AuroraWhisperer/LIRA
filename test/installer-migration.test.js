...
test('NSIS preserves installation-local data and stops safely on failure', {
  ...
  const removal = installer.match(/!macro customRemoveFiles\b[\s\S]*?!macroend/)[0].replaceAll('$TEMP', '${FIXTURE_TEMP}');
  assert.doesNotMatch(functions + removal, /\$(?:APPDATA|TEMP|LOCALAPPDATA)\b|ReadReg|DeleteReg/);

  for (const scenario of ['legacy-update', 'preserving-update', 'change-directory', 'appdata-return',
    'existing-local', 'recover-backup', 'backup-conflict', 'target-conflict', 'copy-failure',
    'restore-failure', 'report-failure', 'locked-data', 'running-app', 'uninstall-preserves-data']) {
    await t.test(scenario, async () => {
      const root = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-installer-test-'));
      const oldInstall = path.join(root, '旧版 LIRA');
      const newInstall = ['change-directory', 'target-conflict'].includes(scenario) ? path.join(root, '新版 LIRA') : oldInstall;
      ...
      try {
        ...
        if (['copy-failure', 'report-failure'].includes(scenario)) fs.writeFileSync(backup + '.partial', 'obstructs staging directory');
        if (scenario === 'report-failure') {
          fs.rmdirSync(reportDir);
          fs.writeFileSync(reportDir, 'obstructs report creation');
        }
        if (scenario === 'uninstall-preserves-data') {
        ...
        if (scenario === 'locked-data') {
          ...
          const command = `$lock = [IO.File]::Open(${literal(path.join(oldData, 'Network', 'Cookies'))}, 'Open', 'ReadWrite', 'None'); ` +
            `[IO.File]::WriteAllText(${literal(ready)}, 'ready'); try { [Console]::ReadLine() } finally { $lock.Dispose() }`;
          holder = spawn('powershell.exe', ['-NoLogo', '-NoProfile', '-EncodedCommand', Buffer.from(command, 'utf16le').toString('base64')], { windowsHide: true, stdio: ['pipe', 'ignore', 'ignore'] });
          const deadline = Date.now() + 10000;
          while (!fs.existsSync(ready) && Date.now() < deadline) await new Promise((resolve) => setTimeout(resolve, 25));
          assert.ok(fs.existsSync(ready), 'fixture file lock must be acquired');
        }
        const fixture = [
          'Unicode true', 'Name "LIRA isolated preservation test"', 'RequestExecutionLevel user', '!define VERSION "fixture"',
          ...
          `!define FIXTURE_APPDATA "${definePath(appData)}"`, `!define FIXTURE_TEMP "${definePath(reportDir)}"`,
          functions, removal, 'Function .onInit',
          `StrCpy $INSTDIR "${definePath(newInstall)}"`, `StrCpy $liraPreviousInstallDir "${definePath(oldInstall)}"`,
          'Call liraWaitForAppExit', 'Call liraPreserveInstallData',
          `FileOpen $0 "${definePath(prepared)}" w`, 'FileWrite $0 "$INSTDIR | $liraDataSource | $liraDataBackup"', 'FileClose $0',
          ['legacy-update', 'change-directory', 'appdata-return', 'restore-failure'].includes(scenario) ? `RMDir /r "${definePath(oldInstall)}"` : '',
          scenario === 'uninstall-preserves-data' ? '!insertmacro customRemoveFiles' : '', 'CreateDirectory "$INSTDIR"',
          scenario === 'restore-failure' ? 'FileOpen $0 "$INSTDIR\\data" w\nFileWrite $0 "obstruction"\nFileClose $0' : '',
          'Call liraRestoreInstallData', `FileOpen $0 "${definePath(completed)}" w`, 'FileWrite $0 "completed"', 'FileClose $0',
          'SetErrorLevel 0', 'Quit', 'FunctionEnd', 'Section', 'SectionEnd',
        ].join('\n');
        const script = path.join(root, 'fixture.nsi');
        fs.writeFileSync(script, fixture);
        const build = spawnSync(compiler, ['/V2', '-INPUTCHARSET', 'UTF8', script], { encoding: 'utf8', windowsHide: true, timeout: 30000 });
        assert.equal(build.error, undefined);
        assert.equal(build.status, 0, build.stdout + build.stderr);
        const run = spawnSync(path.join(root, 'fixture.exe'), ['/S'], { windowsHide: true, timeout: 30000 });
        assert.equal(run.error, undefined);
        const failed = scenario.includes('failure') || scenario.includes('conflict') || ['locked-data', 'running-app'].includes(scenario);
        const reportPath = path.join(reportDir, 'LIRA-install-error.txt');
        assert.equal(run.status, failed ? 2 : 0, fs.existsSync(reportPath) ? fs.readFileSync(reportPath, 'utf16le') : 'installer exit code');
        assert.equal(fs.existsSync(completed), !failed);
        if (!failed) {
          assert.ok(fs.existsSync(path.join(destination, 'fixture.txt')), fs.readFileSync(prepared, 'utf8') + '\n' + fs.readdirSync(root, { recursive: true }).join('\n'));
        ...
        if (scenario === 'backup-conflict') assert.equal(fs.readFileSync(path.join(backup, 'fixture.txt'), 'utf8'), 'previous recovery fixture');
        if (scenario === 'target-conflict') assert.equal(fs.readFileSync(path.join(destination, 'fixture.txt'), 'utf8'), 'newer fixture');
        if (scenario === 'existing-local') assert.equal(fs.readFileSync(path.join(legacyAppData, 'fixture.txt'), 'utf8'), 'stale AppData fixture');
        if (scenario === 'appdata-return') assert.equal(fs.readFileSync(path.join(legacyAppData, 'fixture.txt'), 'utf8'), 'legacy fixture');
        if (scenario === 'uninstall-preserves-data') {
          for (const name of ['logs', 'updates']) assert.equal(fs.readFileSync(path.join(newInstall, name, 'fixture.txt'), 'utf8'), name);

...
test('Admin optional explanations use one contextual help component', () => {
  ...
  assert.doesNotMatch(componentSource, /addEventListener\(['"]mouseenter['"]/);
  assert.doesNotMatch(componentSource, /addEventListener\(['"]focus['"]/);
  assert.match(componentSource, /setAttribute\(['"]aria-expanded['"], ['"]false['"]\)/);
  assert.match(componentSource, /toggleTooltip\(\)[\s\S]*?this\.tooltipOpen[\s\S]*?this\.hideTooltip\(\)[\s\S]*?this\.showTooltip\(\)/);
  assert.match(componentSource, /onClick\(event\)[\s\S]*?this\.toggleTooltip\(\)/);
  assert.match(componentSource, /event\.key === ['"]Escape['"]/);
  assert.match(componentSource, /event\.key !== ['"]Enter['"] && event\.key !== ['"] ['"]/);
  assert.match(componentSource, /event\.key !== ['"]Enter['"][\s\S]*?this\.toggleTooltip\(\)/);

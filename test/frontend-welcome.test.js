...
async function fixture(initialProfile = account('one'), v2 = false) {
  function node() {
    return { value: '', checked: false, disabled: false, textContent: '', children: [], events: {}, dataset: {}, hidden: false,
      append(...items) { this.children.push(...items); }, appendChild(item) { this.children.push(item); },
      replaceChildren(...items) { this.children = items; }, setAttribute(key, value) { this[key] = value; },
      addEventListener(key, fn) { this.events[key] = fn; }, removeEventListener(key) { delete this.events[key]; }, focus() {}, dispatchEvent() {},
    };
  }
  ...
  windowRef.CustomEvent = class { constructor(type) { this.type = type; } };
  const bridge = { getProfile: async () => initialProfile, onStateChanged: (fn) => { listener = fn; return () => {}; },
  ...
  if (v2) { bridge.getWelcomeSettingsV2 = bridge.getWelcomeSettings; bridge.updateWelcomeSettingsV2 = bridge.updateWelcomeSettings; }
  module.initDanmakuWelcome({
    documentRef: { getElementById: (id) => get(id.replace('danmakuWelcome', '')), createElement: node, querySelectorAll: () => [] }, windowRef,
    bridge, toast: (value) => toasts.push(value),
  });
  await flush();
  return { get, reads, writes, toasts, account: (name) => listener(account(name)),
    ...
    parameter: (key, value) => { get(key).value = value; get(key).events.input(); },
    library: (key) => { get('LibraryKind').value = key; get('LibraryKind').events.change(); },
  };
}
...
const savedV2 = (patch = {}) => ({ ok: true, schemaVersion: 2, enabled: false,
  ...
  messages: ['欢迎 {username}', '{username} 来啦'], greetingMessages: ['{username}，你好呀～'],
  attentionWelcomeMessages: ['专属 {username}'], attentionGreetingMessages: ['{username}，来点歌吧'], ...patch });

test('suggestions only edit the draft; enabling atomically includes parameters and leaves libraries dirty', async () => {
  const f = await fixture(account('one'), true); f.reads[0].resolve(savedV2()); await flush();
  f.edit(0, '草稿 {username}'); f.click('SuggestBtn');
  assert.equal(f.writes.length, 0); assert.equal(f.get('Toggle').checked, false);
  assert.match(f.get('OverviewStatus').textContent, /开启并保存参数/);
  f.get('Toggle').checked = true; f.change('Toggle');
  assert.deepEqual(f.writes[0].patch, { enabled: true, welcomeDelaySeconds: 5, welcomeMinHonorLevel: 10,
    greetingDelaySeconds: 10, greetingMinHonorLevel: 10, attentionMinHonorLevel: 31 });
  f.writes[0].resolve(savedV2(f.writes[0].patch)); await flush();
  assert.equal(f.get('GreetingToggle').checked, false); assert.equal(f.get('AttentionToggle').checked, false);
  assert.equal(f.get('ParameterSaveBtn').disabled, true); assert.equal(f.get('SaveBtn').disabled, false);
  assert.equal(f.get('List').children[0].children[1].value, '草稿 {username}');
});

test('parameter and four library domains preserve edits while writes serialize', async () => {
  const f = await fixture(account('one'), true); f.reads[0].resolve(savedV2({ enabled: true })); await flush();
  f.parameter('welcomeDelaySeconds', '5'); f.edit(0, '欢迎草稿');
  f.library('greetingMessages'); f.edit(0, '问候草稿');
  f.click('ParameterSaveBtn'); f.parameter('welcomeDelaySeconds', '6');
  f.click('SaveBtn'); assert.equal(f.writes.length, 1);
  f.writes[0].resolve(savedV2({ enabled: true, welcomeDelaySeconds: 5 })); await flush();
  assert.equal(f.get('welcomeDelaySeconds').value, '6'); assert.equal(f.get('SaveBtn').disabled, false);
  f.click('SaveBtn'); assert.deepEqual(f.writes[1].patch, { greetingMessages: ['问候草稿'] });
  f.library('messages');
  f.writes[1].resolve(savedV2({ enabled: true, welcomeDelaySeconds: 5, greetingMessages: ['问候草稿'] })); await flush();
  assert.equal(f.get('List').children[0].children[1].value, '欢迎草稿');
  assert.equal(f.get('SaveBtn').disabled, false); assert.equal(f.get('ParameterSaveBtn').disabled, false);
  f.library('greetingMessages'); assert.equal(f.get('SaveBtn').disabled, true);
});

test('invalid greeting delay blocks enabling/saving but pure disable still submits', async () => {
  const f = await fixture(account('one'), true); f.reads[0].resolve(savedV2({ enabled: true, greetingEnabled: true })); await flush();
  f.parameter('welcomeDelaySeconds', '20'); f.click('ParameterSaveBtn');
  assert.equal(f.writes.length, 0); assert.match(f.get('greetingDelaySecondsError').textContent, /大于/);
  f.get('Toggle').checked = false; f.change('Toggle');
  assert.deepEqual(f.writes[0].patch, { enabled: false });
  f.writes[0].resolve(savedV2()); await flush();
  assert.equal(f.get('GreetingToggle').checked, false); assert.equal(f.get('GreetingToggle').disabled, true);
...
test('empty first-delay and stage levels submit zero; unsupported and malformed replies stay distinct', async () => {
  const f = await fixture(account('one'), true); f.reads[0].resolve(savedV2()); await flush();
  f.parameter('welcomeDelaySeconds', ''); f.parameter('welcomeMinHonorLevel', ''); f.parameter('greetingMinHonorLevel', '');
  f.click('ParameterSaveBtn'); assert.equal(f.writes[0].patch.welcomeDelaySeconds, 0);
  assert.equal(f.writes[0].patch.welcomeMinHonorLevel, 0);
  f.writes[0].resolve({ ok: true, enabled: true }); await flush();
  assert.equal(f.get('Toggle').checked, false); assert.equal(f.get('ParameterSaveBtn').disabled, false);
  const old = await fixture(account('old'), true); old.reads[0].resolve({ ...saved(), schemaVersion: 1 }); await flush();
  assert.equal(old.get('Toggle').disabled, false); assert.equal(old.get('GreetingToggle').disabled, true);
  assert.equal(old.get('Capability').hidden, false);
  const failed = await fixture(account('failed'), true); failed.reads[0].resolve({ ok: false, error: 'HTTP_403' }); await flush();
  assert.equal(failed.get('Toggle').disabled, true); assert.equal(failed.get('Capability').hidden, true);
  assert.equal(failed.get('ServerStatus').hidden, false);
  const malformed = await fixture(account('malformed'), true);
  malformed.reads[0].resolve({ ...savedV2(), greetingDelaySeconds: '10' }); await flush();
...
test('sample preview identifies draft/confirmed values, unknown honor and the five-second gap', async () => {
  const f = await fixture(account('one'), true); f.reads[0].resolve(savedV2({ enabled: true, greetingEnabled: true,
    welcomeDelaySeconds: 5, greetingDelaySeconds: 6, welcomeMinHonorLevel: 20, greetingMinHonorLevel: 10 })); await flush();
  assert.match(f.get('PreviewSecond').textContent, /10 秒/);
  assert.match(f.get('DependencyHint').textContent, /至少 20 级/);
  f.get('PreviewLevel').value = 'unknown'; f.change('PreviewLevel');
  assert.match(f.get('PreviewFirst').textContent, /等级未知/);
  f.parameter('welcomeDelaySeconds', '30'); assert.match(f.get('PreviewSummary').textContent, /草稿有错误/);
  f.get('PreviewSource').value = 'confirmed'; f.change('PreviewSource');

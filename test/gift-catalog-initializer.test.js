...
const { STATE_FILE_NAME, createGiftCatalogInitializer } = require('../src/bilibili/gift/gift-catalog-initializer');
...
test('initializes the catalog and all available images on first login', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-gift-catalog-initializer-success-'));
  try {
    ...
    const persisted = JSON.parse(fs.readFileSync(path.join(dataDir, 'cache', STATE_FILE_NAME), 'utf8'));
...
test('reuses completed initialization state without redownloading images', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-gift-catalog-initializer-reuse-'));
  try {
...
test('allows initialization to complete when individual images fail', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-gift-catalog-initializer-partial-'));
  try {
...
test('fails first initialization when the catalog is unavailable and no local copy exists', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-gift-catalog-initializer-offline-'));
  try {
    const initializer = createGiftCatalogInitializer({
      dataDir,
      catalog: {
    ...
    assert.equal(fs.existsSync(path.join(dataDir, 'cache', STATE_FILE_NAME)), false);
  } finally {
    fs.rmSync(dataDir, { recursive: true, force: true });
  }
});
...
test('keeps a completed local catalog ready when its refresh fails', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-gift-catalog-initializer-stale-'));
  try {
    ...
    const first = createGiftCatalogInitializer({
      ...
      imageCache: {
        cacheGifts: async (gifts) => gifts.map((gift) => ({ ...gift, imagePath: '/cached.webp' })),
      },
      now: () => Date.parse(COMPLETED_AT),
      logger: QUIET_LOGGER,
    });
...
test('coalesces concurrent initialization requests into one scan', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'lira-gift-catalog-initializer-single-flight-'));
  try {

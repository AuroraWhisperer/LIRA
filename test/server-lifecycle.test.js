...
test(
  'packaged process recognition is case insensitive and requires the owning install path',
  { skip: process.platform !== 'win32' },
  async (t) => {
    const dataDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'lira-process-owner-'),
    );
    ...
    t.mock.method(process, 'kill', () =>
      assert.fail('synthetic process must never be terminated'),
    );
    for (const [executable, expectedShutdown] of [
      ...
      await lifecycle.cleanupOwnPortOccupant({
        ...cleanupOptions(
          dataDir,
          async (url) => {
            if (String(url).endsWith('/api/system/shutdown'))
...
test(
  'process cleanup requires an exact owned runtime entry, not a generic path or argument',
  { skip: process.platform !== 'win32' },
  async (t) => {
    const dataDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'lira-process-entry-'),
    );
    t.after(() => {
      assert.equal(
    ...
    for (const [executable, command, expectedStop] of [
      ...
      [
        'node.exe',
        'node.exe C:\\OtherProject\\main.js --data-dir C:\\Apps\\Lira',
        false,
      ],
      [
        'powershell.exe',
        'powershell.exe C:\\Apps\\Lira\\src\\server.js',
        false,
      ],
      ...
      [
        'electron.exe',
        'electron.exe C:\\Apps\\Lira\\src\\electron\\main.js',
        true,
      ],
    ]) {
      info = {
        ExecutablePath: `C:\\Runtime\\${executable}`,
        CommandLine: command,
      };
      stopped.length = 0;
      const requests = [];
      ...
      assert.deepEqual(
        stopped,
        expectedStop ? [{ pid: 12345, signal: 'SIGTERM' }] : [],
        command,
      );
      assert.equal(
        requests.some((url) => url.endsWith('/api/system/shutdown')),
        expectedStop,
        command,
      );
    }
...
test(
  'cleanup rechecks a runtime PID after the graceful shutdown wait',
  { skip: process.platform !== 'win32' },
  async (t) => {
    const dataDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'lira-process-reuse-'),
    );
    t.after(() => {
      assert.equal(
        path.dirname(fs.realpathSync(dataDir)),
    ...
    t.mock.method(process, 'kill', () =>
      assert.fail('reused PID must not be terminated'),
    );
    lifecycle.writeRuntimeInfo(dataDir, {
      pid: 12345,
      port: 3000,
...
test('forced cleanup refreshes service health and binds it to the same PID', async (t) => {
  const dataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'lira-process-health-'),
  );
  t.after(() => {
    assert.equal(
      path.dirname(fs.realpathSync(dataDir)),
          ...
          return new Response(
            JSON.stringify(
              pid
                ? {
                    ok: true,
                    data: { serviceId: lifecycle.SERVICE_ID, pid },
                  }
                : {},
            ),
            { status: pid ? 200 : 404 },

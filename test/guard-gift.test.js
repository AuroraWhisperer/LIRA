...
test('multi-month paid toast ignores the source-2 companion and creates one recent card row', () => {
  withGiftService(({ service }) => {
    const paid = packetParser.extractBilibiliGiftMessage(guardToast({
      uid: 150,
      price: 534000,
      num: 3,
    ...
    const companion = packetParser.extractBilibiliGiftMessage(guardToast({
      uid: 150,
      price: 534000,
      num: 3,
      payflowId: 'season-150',
...
test('non-month guard units do not multiply accounting quantity', () => {
  const webGift = packetParser.extractBilibiliGiftMessage(guardToast({
    uid: 160,
    price: 198000,
    num: 3,
    unit: '*3天',

...
test('live refresh toast resolves its migrated image from the nested stylesheet', () => {
  const source = fs.readFileSync(path.join(ROOT_DIR, 'public', 'css', 'admin', 'toasts', 'live.css'), 'utf8');
...
test('queue headers share a fixed minimum height and song queue controls stay compact', () => {
  ...
  const buttonRule = source.match(/\.queues-row \.queue-panel \.panel-header button\s*\{[\s\S]*?\n\}/)?.[0];
...
test('top navigation keeps its outer track and colored moving active capsule', () => {
  ...
  const desktop = readCssBundle('public', 'css', 'overlays', 'desktop.css');
  ...
  assert.match(railRule, /center\s+bottom 3px \/ 22px 3px no-repeat/);
...
test('toolbox owns independent settings, overtime, streamer planner, start animation, performance, usage guide, and update features', () => {
  ...
  assert.match(performanceHtml, /id="metricsRefreshBtn"[^>]*>\s*开始检测\s*<\/button>/);
  ...
  assert.match(featureStyles, /\.other-feature-panel-body\.stack\s*\{[^}]*grid-auto-rows:\s*max-content;/);
});

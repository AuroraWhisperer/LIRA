...
test('Windows native TCP/process lookup safely shuts down an owned legacy child', { skip: process.platform !== 'win32', timeout: 20000 }, async (t) => {
  ...
  `);
  const child = childProcess.fork(entry, [], { execArgv: [], stdio: ['ignore', 'ignore', 'pipe', 'ipc'], windowsHide: true });
  t.after(async () => {
  ...
  await lifecycle.cleanupOwnPortOccupant({
    port, host: '127.0.0.1', rootDir, dataDir: rootDir,
    cleanupTimeoutMs: 2000, cleanupPollMs: 20,
    sleep: (ms) => new Promise((resolve) => setTimeout(resolve, ms)),
    onPhase: (phase, durationMs, extra) => phases.push({ phase, durationMs, extra }),
  });
  assert.ok(phases.some((p) => p.extra.result === 'graceful'), JSON.stringify({ phases, ownerQueries }));

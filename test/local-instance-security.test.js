...
const OWNER = {
  ...
  CommandLine: 'node.exe C:\\Apps\\Lira\\src\\server.js',
};
...
async function peer(t, options = {}) {
  ...
  return {
    ...
    signals,
    cleanup: (extra = {}) =>
      lifecycle.cleanupOwnPortOccupant({
        rootDir: ROOT,
        dataDir,
        host: '127.0.0.1',
        port,
...
test('a valid challenge allows shutdown only on the same TCP connection', async (t) => {
  ...
  assert.deepEqual(
    f.requests.map((r) => r.path),
    ['/api/health', '/api/system/shutdown'],
  );
...
for (const [name, options] of [
  ...
  [
    'proof for another port',
    {
      proof: (_proof, req) =>
        createInstanceProof(
          TOKEN,
          req.headers[CHALLENGE_HEADER],
          req.socket.localPort === 65535 ? 1 : req.socket.localPort + 1,
        ),
    },
  ],
  [
    'proof for another challenge',
    { proof: (_proof, req) => createInstanceProof(TOKEN, '0'.repeat(64), req.socket.localPort) },
  ],
...
test(
  'Windows verifies legacy peers by the connected endpoint, not health paths',
...
test(
  'verified legacy peers without a token file still receive a graceful request',
  { skip: process.platform !== 'win32' },
  async (t) => {
...
test(
  'process entry recognition retains exact install and runtime boundaries',
  { skip: process.platform !== 'win32' },
  () => {

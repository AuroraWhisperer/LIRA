...
function fixture(t) {
  ...
  return { db, store, save: (song) => songService.saveSong(store, song),
...
function textParser() {
  ...
  vm.runInNewContext(fs.readFileSync(require.resolve('../public/js/admin/import.js'), 'utf8'), context);
  return context.window.AdminApp.imports.parseTable;
}
...
test('update preview preserves omitted and blank fields and matches exact artist including disabled songs', (t) => {
  ...
  const original = save({ name: '同歌', artist: '甲', categoryName: '原分类', isEnabled: false,
    requestPrice: '舰长', songClip: 'BV1', tags: '原标签', language: '国语', note: '备注', sourcePlatform: 'QQ音乐' });
  const other = save({ name: '同歌', artist: '乙', requestPrice: '总督' });
  const input = { rows: [{ name: '同歌', artist: '甲', requestPrice: '30元SC', songClip: '', categoryName: '' },
    { name: '新歌', artist: '丙', requestPrice: '免费' }] };
  const plan = preview(input);
  assert.deepEqual(plan.counts, { inserted: 1, updated: 1, unchanged: 0, conflict: 0, invalid: 0 });
...
test('allow empty clears only present text fields, defaults category and preserves blank enabled values', (t) => {
  ...
  save({ name: '原歌', artist: '甲', requestPrice: '舰长', songClip: 'BV1', categoryName: '流行', tags: '标签', isEnabled: false });
  const input = { rows: [{ name: '原歌', artist: '甲', requestPrice: '', songClip: '', categoryName: '', isEnabled: '' }], allowEmptyClear: true };
  const plan = preview(input);
  assert.deepEqual(plan.rows[0].differences.map((diff) => diff.field), ['categoryName', 'requestPrice', 'songClip']);
  apply(input, plan);
  ...
  const enabledInput = { rows: [{ name: '原歌', artist: '甲', isEnabled: '是' }] };
...
test('file duplicates collapse identical content and all differing identities become conflicts', (t) => {
  ...
  const plan = preview(input);
  assert.deepEqual(plan.counts, { inserted: 1, updated: 0, unchanged: 1, conflict: 2, invalid: 0 });
...
test('invalid rows and conflicting price aliases are visible and block the batch', (t) => {
  ...
  const plan = preview(input);
  assert.deepEqual(plan.counts, { inserted: 1, updated: 0, unchanged: 0, conflict: 1, invalid: 4 });
  ...
  assert.throws(() => apply(input, plan), { code: 'SONG_IMPORT_PREVIEW_INVALID' });
  for (const badInput of [{ rows: [] }, { rows: Array(5001).fill({ name: '歌' }) }, { rows: [{}], allowEmptyClear: 'true' }]) {
    assert.throws(() => preview(badInput), { code: 'SONG_IMPORT_INPUT_INVALID' });
  }
});
...
test('stale song, category, source input or clear option rejects before writes', (t) => {
  ...
  assert.throws(() => apply({ rows: [{ name: '原歌', requestPrice: '总督' }] }, plan), { code: 'SONG_IMPORT_PREVIEW_STALE' });
  assert.throws(() => apply({ ...input, allowEmptyClear: true }, plan), { code: 'SONG_IMPORT_PREVIEW_STALE' });
  save({ name: '无关新歌' });
  assert.throws(() => apply(input, plan), { code: 'SONG_IMPORT_PREVIEW_STALE' });
  plan = preview(input);
...
test('new songs cannot exceed the final 5000-song library boundary while existing updates remain available', (t) => {
  ...
  store.importRows(Array.from({ length: 5000 }, (_, index) => ({ name: `歌曲${index}`, artist: '', isEnabled: true })));
  ...
  assert.throws(() => apply(overLimitUpdate, overLimitPlan), { code: 'SONG_IMPORT_PREVIEW_INVALID' });
});
...
test('write failure rolls back updates, inserted categories, rows and import batch', (t) => {
  ...
  save({ name: '原歌', requestPrice: '舰长' });
  const input = { rows: [{ name: '原歌', requestPrice: '提督', categoryName: '新分类' }, { name: '失败歌曲', categoryName: '失败分类' }] };
  const plan = preview(input);
  const before = store.listRows();
  const categories = store.listCategories();
  db.exec("CREATE TRIGGER reject_import BEFORE INSERT ON songs WHEN NEW.name = '失败歌曲' BEGIN SELECT RAISE(ABORT, 'fixture failure'); END");
...
test('update parsing preserves missing columns and empty-name data rows across text and XLSX', (t) => {
  ...
  const files = readZipFiles(buildSongsWorkbook([{ name: '有效', request_price: '舰长' }, { name: '', request_price: '提督' }]));
  const xml = files.get('xl/worksheets/sheet1.xml').replace(/<c r="[B-FH-J]\d+"[\s\S]*?<\/c>/g, '');
  files.set('xl/worksheets/sheet1.xml', xml);
  const rows = parseSongsFromXlsx(createZip([...files]), { preserveMissing: true });
  assert.equal(rows.length, 2);
...
test('platform export preserves stored values with CSV formula protection and XLSX text cells', () => {
  const song = { name: '测试', source_platform: '=平台,"说明"', is_enabled: true };
...
test('preview/apply routes use server plans, reject stale requests and publish only once on success', async (t) => {
  ...
  const signals = [];
  const context = {
    songs: { previewImport: preview, applyImport: (input) => applySongImport(store, input) },
    broadcastSnapshot: (reason) => signals.push(reason), cloudSync: { request: (scope) => signals.push(scope) },
  };
  ...
  const stale = await call('import-apply', { ...input, previewToken: 'untrusted' });
  assert.equal(stale.status, 409);
  assert.equal(stale.body.error, 'SONG_IMPORT_PREVIEW_STALE');
  assert.equal(signals.length, 0);
  const applied = await call('import-apply', { ...input, previewToken: plan.body.data.previewToken });

...
test('gift audit page loads dedicated assets without inline behavior', () => {
  ...
  assert.match(html, /<link rel="stylesheet" href="\/css\/gift-audit\.css\?v=[^"]+">/);
...
test('gift audit analysis separates matches, misses, and server-only gifts', async () => {
  ...
  const results = crossReference([
    { userName: '用户A', giftName: '粉丝团灯牌', giftId: '31164', comboCount: 2 },
  ...
  assert.deepEqual(toPlain(results.map(({ source, userName, status }) => ({ source, userName, status }))), [

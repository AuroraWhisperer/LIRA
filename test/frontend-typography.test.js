...
const MICRO_ALLOWLIST = [
  /\.ui-eyebrow\b/, /\.queue-eyebrow\b/, /\.panel-kicker\b/,
  /\b(?:thead|table)\b[^,]*\bth\b/, /\.(?:status-badge|status-chip|source-badge)\b/
];
...
const HEAVY_ALLOWLIST = [
  /\.ui-metric\b/, /\.(?:metric|stat|total)-value\b/,
...
function readEntry(...parts) {
  ...
  return fs.readFileSync(entryPath, 'utf8').replace(
    /@import\s+url\(['"]([^'"]+)['"]\);/g,
    (_all, imported) => {
...
test('shared CSS defines the exact monotonic typography token contract', () => {
  ...
  assert.deepEqual(Object.fromEntries(Object.keys(TOKENS).map(name => [name, actual[name]])), TOKENS);
...
test('composed Admin HTML and runtime templates have no inline typography', () => {
  const pattern = /style\s*=\s*["'`][^"'`]*(?:font-size|font-weight|font-family|line-height|letter-spacing)[^"'`]*["'`]/gi;
  const sources = [['composed Admin HTML', readAdminHtml()]];
  for (const root of ['public/js/admin', 'public/js/playback']) {
    for (const file of filesBelow(path.join(ROOT, root), '.js')) sources.push([path.relative(ROOT, file), fs.readFileSync(file, 'utf8')]);
  }
  const violations = [];
...
test('common Admin copy uses standard weights with explicit presentation exceptions', () => {
  ...
  for (const rule of rules(desktopCss())) {
    ...
    if (!value || /^(?:normal|bold|400|500|600|700|var\(--type-weight-(?:regular|medium|semibold|bold)\))$/.test(value)) continue;
...
test('OBS and configurable preview typography stay outside Admin roles', () => {
  ...
  assert.doesNotMatch(overlayCss, /(?:\.ui-(?:display|page|section|card|body|control|caption|eyebrow|metric)\b|var\(--type-)/);
  const preview = read('public', 'css', 'admin', 'desktop-lyric-preview.css');
  for (const name of ['--preview-font', '--preview-size', '--preview-weight', '--preview-line-height', '--preview-letter-spacing']) {
    assert.ok(preview.includes(`var(${name})`), `${name} must remain user-configurable`);
  }
  const queue = read('public', 'js', 'overlays', 'queue-render.js');

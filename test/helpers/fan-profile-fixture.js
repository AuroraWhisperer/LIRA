...
function fanFixture(t) {
  ...
  const fixture = {
    ...
    restart() { closeDatabases(db); open(); },
    run(action, input = {}, scope = SCOPE) { return service.execute(scope, action, input); },
    create(input = {}, scope = SCOPE) {
      return service.execute(scope, 'create', { alias: '小海', identity: IDENTITY, ...input });
    },
    detail(profileId, scope = SCOPE) { return service.execute(scope, 'detail', { id: profileId }); },
    ...
    consume(events, extra = {}, scope = SCOPE) {
      ...
      return service.consumeFacts(scope, { version: 1, streamerId: JSON.parse(scope)[1],
        epoch: settings.epoch || 'fixture-epoch', after, nextCursor: after + events.length,
        events: events.map((event, index) => ({ cursor: after + index + 1, id: `fixture-${after + index + 1}`,
          identity: IDENTITY, observedAt: currentTime, kind: 'identity', name: '海边听歌', nameComplete: true, ...event })), ...extra });
    },
  };
  return fixture;
}

...
function createDom() {
  ...
  function createNode(tagName) {
    ...
    let html = '';
    const node = {
      tagName, children: [], attributes, listeners, style: {}, className: '', hidden: false,
      parentNode: null, ownerDocument: documentRef,
      get isConnected() { return this === documentRef.body || Boolean(this.parentNode?.isConnected); },
      get parentElement() { return this.parentNode; },
      get textContent() { return text + this.children.map((child) => child.textContent).join(''); },
      set textContent(value) { text = String(value); html = ''; this.replaceChildren(); },
      get innerHTML() { return html || text; },
      set innerHTML(value) { html = String(value); text = html.replace(/<[^>]*>/g, ''); this.replaceChildren(); },
      setAttribute(name, value) { attributes.set(name, String(value)); },
      getAttribute(name) { return attributes.get(name) ?? null; },
      removeAttribute(name) { attributes.delete(name); },
      append(...children) {
        children.forEach((child) => { child.remove(); child.parentNode = this; this.children.push(child); });
      },
      ...
      blur() { documentRef.activeElement = null; },
      getBoundingClientRect() {
        const siblings = [...this.parentNode?.children || []].sort((a, b) => Number(a.style.order || 0) - Number(b.style.order || 0));

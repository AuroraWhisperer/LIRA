...
function seedClearAllConfiguration({ songDb, musicDb, giftDb }) {
...
function seedSongBusinessData(songDb, timestamp) {
  songDb
    .prepare(
      `
...
function seedAiBusinessData(songDb) {
  songDb
    .prepare(
      `
...
function seedGiftBusinessData(giftDb, timestamp) {
...
function seedPlaybackBusinessData(musicDb, checkinDb, timestamp) {
  musicDb
    .prepare(
      `
...
function seedClearAllBusinessData(databases) {
  const { songDb, superChatDb, giftDb, musicDb, checkinDb } = databases;
  const timestamp = now();
  seedSongBusinessData(songDb, timestamp);
  seedAiBusinessData(songDb);
  superChatDb
    .prepare(
      `
  ...
  `,
    ...
    .run(timestamp, timestamp);
  const giftSourceId = seedGiftBusinessData(giftDb, timestamp);
  seedPlaybackBusinessData(musicDb, checkinDb, timestamp);
  return giftSourceId;
}

function assertClearAllResultStructure(result) {
...
function assertBusinessCleared(databases) {
  assertSongBusinessCleared(databases.songDb);
  assertAiBusinessCleared(databases.songDb);
  assertEventAndPlaybackCleared(databases);
}

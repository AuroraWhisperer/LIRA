...
function loadTodo(
  ...
  const source = readJsModuleBundle('public', 'js', 'admin', 'todo.js');
...
test('streamer workbench keeps on-air cues and live notes', () => {
  ...
  assert.match(
    html,
    /<strong>主播工作台<\/strong>\s*<small>本场提词与直播速记<\/small>/,
  );
  assert.match(
    html,
    /id="plannerSessionForm"[\s\S]*id="plannerSessionDate"[\s\S]*id="plannerSessionTime"[\s\S]*id="plannerSessionTitle"[\s\S]*id="plannerSessionGoal"/,
  );
  assert.match(
    html,
    /id="plannerTaskForm"[\s\S]*id="plannerTaskTitle"[\s\S]*id="plannerTaskStage"/,
  );
  assert.match(
    html,
    /id="plannerBeforeList"[\s\S]*id="plannerLiveList"[\s\S]*id="plannerAfterList"/,
  );
  assert.match(
    html,
    /id="plannerNoteForm"[\s\S]*id="plannerNoteBody"[\s\S]*id="plannerNoteType"[\s\S]*id="plannerNoteList"/,
  );
  assert.match(
    planner,
    /本场提词[\s\S]*<option value="before">开场<\/option>[\s\S]*<option value="live">互动<\/option>[\s\S]*<option value="after">收尾<\/option>/,
  );
  assert.match(
    planner,
    /直播速记[\s\S]*话题灵感[\s\S]*观众请求[\s\S]*高光时刻/,
  );
  ...
  assert.deepEqual(
    JSON.parse(
      JSON.stringify(
        todo
          .getTasks()
          .map(({ title, stage, done }) => ({ title, stage, done })),
      ),
    ),
    [
      {
...
test('streamer workbench keeps its split desk readable on narrow windows', () => {
  ...
  assert.match(
    styles,
    /\.planner-workspace\s*\{[^}]*grid-template-columns:\s*minmax\(0, 1\.55fr\) minmax\(300px, 0\.75fr\)/,
  );
  assert.match(
    styles,
  ...
  assert.match(
    styles,
    /@media \(max-width: 1100px\)[\s\S]*?\.planner-workspace\s*\{[^}]*grid-template-columns:\s*1fr/,
  );
  ...
  assert.match(
    styles,
    /@media \(prefers-reduced-motion: reduce\)[\s\S]*?\.planner-task-row/,
  );
});
...
test('streamer workbench replaces the six v1 built-in tasks shown to existing users', () => {
  ...
  const { todo } = loadTodo(
    new Map([[LEGACY_STORAGE_KEY, JSON.stringify(legacyTasks)]]),
  );

  assert.deepEqual(
    JSON.parse(
      JSON.stringify(
        todo.getTasks().map(({ title, stage }) => ({ title, stage })),
      ),
    ),
    [

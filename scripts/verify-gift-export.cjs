...
(async () => {
  ...
  try {
    ...
    assert.ok(pixels.subarray(800 * 208 * 4).some((value, index) => index % 4 === 3 && value > 0), 'second row must be captured');
    console.log(JSON.stringify({ ok: true, width: 800, height: 400, dpi: process.env.GIFT_TEST_DPI || '1', preview: file }));
    ...
    assert.equal(singlePng.readUInt32BE(20), 192);
    ...
    assert.equal(tall.readUInt32BE(20), 8096);
    assert.ok(nativeImage.createFromBuffer(tall).toBitmap().subarray(800 * 8000 * 4).some((v, i) => i % 4 === 3 && v > 0));
    console.log('PASS: 40 records split into 8096px and 192px images; last row visible');

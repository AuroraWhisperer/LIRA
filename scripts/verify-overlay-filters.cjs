...
const style = { style: 'signal', fullscreenDurationSeconds: 6, overlayUrl: 'https://synthetic.example.test/overlay/syntheticKey_123' };
const evaluate = code => win.webContents.executeJavaScript(code);
const wait = expression => evaluate(`(async () => { const until=Date.now()+5000; while (!(${expression})) {
...
async function capture(name) {
  ...
  const height = await evaluate('Math.ceil(document.querySelector(".fixture-shell").getBoundingClientRect().height)+40');
...
async function run() {
  ...
  win = new BrowserWindow({ width: 1280, height: 1100, useContentSize: true, show: false,
  ...
  registerLicenseIpc({ ipcMain, getMainWindow: () => win, getDesktopBaseUrl: () => origin,
    ...
    licenseManager: { getState: () => 'authorized', getSnapshot: () => profile, getProfile: async () => profile,
      onStateChanged: () => () => {}, getOverlaySettings: async () => style,
      ...
      updateOverlayFilters: async patch => {
        ...
        writes.push(patch); settings = { ...settings, ...patch }; return structuredClone(settings);
      },
      getOverlayViewers: async () => ({ roomId: '998877', viewers: [
        { uid: '10001', name: '星河来客' }, { uid: '10002', name: '晚风听众' }, { uid: '10003', name: '小小月亮' },
      ] }),
    },
  });
  await win.loadURL(origin + '/admin?desktop=1');
  await wait('!document.getElementById("danmakuBlacklistUid").disabled');
  assert.equal(await evaluate('document.getElementById("danmakuOverlayUrl").getBoundingClientRect().top < document.querySelector(".danmaku-style-picker").getBoundingClientRect().top'), true);
  ...
  await evaluate('document.getElementById("danmakuViewerSearch").value="晚风";document.getElementById("danmakuViewerSearch").dispatchEvent(new Event("input"))');
  ...
  const overflow = await evaluate('[...document.querySelectorAll(".danmaku-overlay-filters *")].filter(e=>e.getClientRects().length && e.getBoundingClientRect().right>document.documentElement.clientWidth+1).map(e=>e.id)');
  assert.deepEqual(overflow, []);
  console.log(JSON.stringify({ ok: true, screenshots: output, writes: writes.length, checks: 'real Electron/IPC, link position, add UID, add word, search/select viewer, clear words, failed save retains input' }));
}

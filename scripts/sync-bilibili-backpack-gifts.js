...
const GIFT_CONFIG_URL =
  'https://api.live.bilibili.com/xlive/web-room/v1/giftPanel/giftConfig?platform=pc&source=live&room_id=1';
const MAPPING_FILES = [
...
async function main() {
  ...
  for (const gift of gifts) {
    ...
    if (
      existing?.imagePath &&
      isUsableWebp(path.join(PUBLIC_DIR, existing.imagePath))
    ) {
      const existingPath = existing.imagePath.replace(
        /^\/img\/bilibili-gifts\//,
        '',
      );
      const hasOwnImage =
        path.posix.basename(existingPath) === `${gift.id}.webp`;
      ...
      if (hasOwnImage && existingPath !== expectedPath) {
        ...
        !exactMapping &&
        path.posix.dirname(existingPath) !== giftCategory(gift)
      ) {
        localPathById.set(gift.id, expectedPath);
  ...
  appendMissingMappingRows(
    mappingDocuments,
    gifts,
    resolvedBefore,
    localPathById,
    requireExactMappingIds,
  );
  ...
  console.log(
    `背包礼物同步完成：代码 ${gifts.length} 个，金瓜子本地图片 ${goldCount} 张，新增下载 ${downloadedCount} 张，按价格调整目录 ${relocatedCount} 张，清理无需本地化图片 ${removedCount} 张。`,
  );
}
...
async function fetchBackpackGifts() {
  ...
  if (!response.ok)
    throw new Error(`Bilibili 礼物配置请求失败：HTTP ${response.status}`);
  const payload = await response.json();
  if (Number(payload?.code) !== 0 || !Array.isArray(payload?.data?.list)) {
    throw new Error(
      `Bilibili 礼物配置返回错误：${payload?.message || payload?.code || '无数据'}`,
    );
  }

  return payload.data.list
    .filter((entry) => Number(entry?.bag_gift) === 1)
    .map(normalizeGift)
    .filter(Boolean);
}
...
function resolveMappings(documents) {
  const result = new Map();
  for (const document of documents.values()) {
    for (const [id, mapping] of parseGiftMappingDocument(
      document.lines.join('\n'),
    )) {
...
function appendMissingMappingRows(
  documents,
  gifts,
  resolvedMappings,
  localPathById,
  ...
  for (const gift of gifts) {
    if (resolvedMappings.has(gift.id) && !requireExactMappingIds.has(gift.id))
      continue;
  ...
  for (const [fileName, missing] of missingByFile) {
    ...
    for (const gift of missing.sort(
      (left, right) => left.rmb - right.rmb || left.id - right.id,
    )) {
      const image =
        gift.coinType === 'gold'
      ...
      const values = [
        gift.id,
        image,
        escapeMarkdownCell(gift.name),
        formatNumber(gift.battery),
        formatMoney(gift.rmb),
      ];
...
async function downloadImage(url, targetPath) {
  ...
  if (!response.ok)
    throw new Error(`礼物图片下载失败：HTTP ${response.status} ${url}`);
...
function updateManifest(gifts, mappings) {
  ...
  const remoteOnlyIds = new Set(
    gifts.filter((gift) => gift.coinType !== 'gold').map((gift) => gift.id),
  );
  const entries = (Array.isArray(manifest.gifts) ? manifest.gifts : []).filter(
    (entry) =>
      !(entry?.bagGift === true && remoteOnlyIds.has(Number(entry?.id))),
  );
  const indexById = new Map(
    entries.map((gift, index) => [Number(gift?.id), index]),
  );
  const additions = [];
  ...
  for (const gift of gifts) {
    ...
    if (!mapping?.imagePath)
      throw new Error(`礼物 ${gift.id} 缺少本地图片映射。`);
    ...
    const entry = {
      ...
      rmb: gift.rmb,
      category: path.posix.dirname(
        relativePath.replace(/^bilibili-gifts\//, ''),
      ),
      bagGift: true,
    };
...
function verifyLocalCatalog(gifts, mappings) {
  ...
  const manifestById = new Map(
    manifest.gifts.map((gift) => [Number(gift?.id), gift]),
  );
  for (const gift of gifts) {
    const mapping = mappings.get(gift.id);
...
function removeUnneededGeneratedImages(gifts, mappings) {
  ...
  for (const gift of gifts) {
    ...
    const mappedPath =
      mappings
        .get(gift.id)
        ?.imagePath?.replace(/^\/img\/bilibili-gifts\//, '') || '';
    if (mappedPath === ownRelativePath || !untracked.has(repositoryPath))
      continue;
    const targetPath = path.resolve(GIFT_DIR, ownRelativePath);
    if (!targetPath.startsWith(`${GIFT_DIR}${path.sep}`))
      throw new Error(`礼物图片路径越界：${targetPath}`);

...
const EXE_NAME = PKG.build.nsis.artifactName.replace('${version}', VERSION).replace('${ext}', 'exe');
...
function ensureGithubRelease() {
  ...
  try {
    run('gh', ['release', 'create', TAG, '--repo', `${OWNER}/${REPO}`, '--title', VERSION, '--notes-file', notesPath]);
  } finally {
...
async function findMissingAssets() {
  ...
  const uploaded = new Map(
    (release.assets || []).filter((asset) => asset.state === 'uploaded').map((asset) => [asset.name, asset]),
  );
  const missing = [];

...
const { createRemoteGiftCatalogCache } = require('../src/bilibili/gift/remote-catalog-cache');
const { createRemoteGiftImageCache } = require('../src/bilibili/gift/remote-gift-image-cache');
const { createGiftCatalogInitializer } = require('../src/bilibili/gift/gift-catalog-initializer');
const { createRemoteLicenseClient } = require('../src/electron/license/remote-license-client');

async function run(options = {}) {
  const dataDir = path.resolve(
    options.dataDir || process.argv[2] || process.env.LIRA_DATA_DIR || path.join(__dirname, '..', 'data'),
  );
...
function formatProgress(state = {}) {
  ...
  if (state.phase === 'images') return `Gift images: ${state.completed}/${state.total} (${state.percent}%)`;
  if (state.status === 'ready') return `Gift catalog ready: ${state.available}/${state.total} images available`;
  return `Gift catalog initialization failed: ${state.error || 'unknown error'}`;
}

if (require.main === module) {
  run().catch((error) => {
    process.stderr.write(`Gift catalog initialization failed: ${error?.message || error}\n`);
    process.exitCode = 1;
  });
}

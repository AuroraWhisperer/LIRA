...
try {
  ...
  for (let i = 0; i < 100000; i += 1) {
    fixture.insertGift(source.id, `bench-${i}`, {
      giftId: `gift-${i % 20}`, giftName: `礼物 ${i % 20}`,
  ...
  profiles.transaction(() => {
    for (let i = 0; i < 1000; i += 1) {
      const profile = profiles.save(scope, {
        alias: `用户-${i}`, tags: [], summary: '', archived: false,
        milestoneReminders: false, expiryReminders: false,
      }, null, at);
      for (let j = 0; j < 50; j += 1) profiles.records.insert(scope, profile.id, {
        kind: 'note', occurredAt: at, data: { text: `合成记录-${j}` },
      });
    }
  ...
  for (const direction of ['desc', 'asc']) {
    results[`deep-${direction}`] = measure(() => store.listHistory({
      ...options, sortDirection: direction, cursor: { sortValue: '2026-09-01T10:36:40.000Z', id: 50001 },
    }));
    plans[`deep-${direction}`] = traces.map(({ sql, params }) => ({
      sql, params, plan: prepareGift(`EXPLAIN QUERY PLAN ${sql}`).all(...params),
    }));
  }
  results.recent = measure(() => store.listRecent({ sourceScope: { sql: 'source_id = ?', params: [source.id] }, limit: 30 }));
  plans.recent = traces.map(({ sql, params }) => ({ sql, params, plan: prepareGift(`EXPLAIN QUERY PLAN ${sql}`).all(...params) }));
  ...
  const report = {
    environment: {
      os: `${os.type()} ${os.release()} ${os.arch()}`, cpu: os.cpus()[0].model,
      ...
      journalMode: prepareGift('PRAGMA journal_mode').get(), synchronous: prepareGift('PRAGMA synchronous').get(),
      cacheSize: prepareGift('PRAGMA cache_size').get(),
      commit: git('rev-parse', 'HEAD'),
      relevantDiff: git('diff', '--', 'src/storage/database-migrations.js', 'src/storage/gift-query-store.js',
        'src/bilibili/gift/query-service.js', 'src/fans/profile-service.js', 'src/storage/fan-record-store.js', 'src/storage/fan-profile-store.js'),
    },
    fixture: { gifts: 100000, profiles: 1000, fanRecords: 50000, requests: 100000,
      seed: 'deterministic counter, no randomness in query data', fixedTime: at, equalTimestampGroup: 10,
      warmSamples: 30, coldDefinition: 'first query after insertion; OS cache is not flushed',
      limitations: 'No Electron event-loop or Device/monitor/network measurement; queue uses a missing key among NULL links.' },
    results, plans,
  };
  fs.writeFileSync(output, `${JSON.stringify(report, null, 2)}\n`);
  console.log(JSON.stringify(Object.fromEntries(Object.entries(results).map(([key, value]) => [key, { p50Ms: value.p50Ms, p95Ms: value.p95Ms, selectCounts: value.selectCounts.slice(0, 2) }]))));
} finally {
  fixture.close();
}

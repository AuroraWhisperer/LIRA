...
| --- | --- | --- | --- |
| 星河牌粉丝档案管理库，用户提供的视频 | 作者介绍包含本地档案、生日／上舰周年／观看纪念日提醒、大航海有效期、舰礼寄送台账与支出记账；明确为手动录入或导入 | 档案、会员周期和纪念日确实构成一个完整工作流；旧粉丝手动补录与备份从第一版考虑，外部表格导入按第 13 节分期 | 未实测；不能以“自动核算有效期”推断能够自动取得 B 站真实到期日。[作者视频](https://www.bilibili.com/video/BV1d4bJ64E1R/) |
...
适合第一版的内容：身份与称呼、自由备注、互动记忆、话题与注意事项、歌曲与偏好、会员记录、纪念日、搜索筛选、完整备份恢复，以及经确认的本机旧点歌补录。外部 CSV／其他工具档案导入放在第三阶段，具体范围见第 11.4、13 节。
...
| 清理行为 | 歌曲数据清理会删除 `requests`／`queue` 并重置自增序列 | 已归档快照与待归档数据均须独立保留；自增行号不能单独充当永久来源标识 |
...
一级界面只保留两个页签：**档案**、**提醒**。设置、列表导出、完整备份与恢复放在右上角更多操作；本机旧点歌补录从建档流程或档案音乐页进入，外部表格导入随第三阶段提供。已有工作台日历未来可显示相同提醒的只读入口，不复制一套日期规则。

档案页顶部是标题、搜索与“新建档案”，下面是简洁的今日提醒摘要和快捷筛选。提醒为空时不展示空的大卡片。

推荐快捷筛选：全部、在舰、曾上舰、特别关注、待补资料。“在舰”按第 9.1 节的当前资格规则筛选；只有历史上舰观察的档案不能一直留在“在舰”中。未知／待核实会员状态独立可筛选，不混进“已过期”。筛选结果允许交集，但身份不是人的永久分组：到期后档案和互动全部保留。
...
快速档案默认显示概览，按“称呼与身份 → 基础资料 → 可以聊什么 → 相处提醒入口 → 最近共同经历 → 音乐摘要 → 个人备注”组织。临近生日在基础资料旁提示；音乐摘要只占一行，优先显示人工确认的偏好，空间允许时附最近点歌，没有偏好时显示最近点歌，无数据时收起。完整偏好、常点分类与歌曲记录通过音乐页签查看。完整生日年份、长备注、来源详情按需展开。
...
概览按“基本资料 → 可以聊的话题 → 相处提醒入口 → 最近的一段记忆 → 一行音乐摘要 → 个人备注”组织。生日等临近事项可以出现在当前人物身份区，音乐摘要按第 5.4 节展示，详细偏好与点歌观察在音乐页展开。相处提醒默认折叠，主播主动打开才展示正文。

音乐页先展示人工确认的“喜欢／不喜欢”，再展示“点歌观察”及其统计范围，最后列出具体歌曲记录。大航海页先展示当前等级、有效期及其来源，再展示累计／连续天数与历史，通过常驻的“编辑大航海”补录或修正日期及天数。

以虚构资料制作的交互示例用于评审上述信息优先级：可切换人物与页签、搜索筛选、展开档案、记录互动和处理提醒。示例中的保存仅改变演示内容，不连接业务数据库；最终桌面实现仍需按第 14 节验收。

### 5.7 大航海与互动记录的手动编辑

根据用户确认，这两块已有内容都必须可以手动编辑，不能只有新增入口。

| 位置 | 编辑入口 | 可修改内容 |
| --- | --- | --- |
| 大航海页 | 标题区常驻“编辑大航海” | 等级、状态及其观察时间、首次上舰、本段开始、到期日期、累计天数、连续天数、天数截至日期 |
| 互动页的手写记录 | 每条记录右侧常驻“编辑” | 发生日期、互动正文；保存后按日期重新排序，概览同步显示修订后的内容 |
| 互动页的点歌记录 | 每条记录右侧常驻“编辑” | 点歌日期、歌名、歌手；音乐页同步显示同一份修订结果 |

大航海起止日期允许留空，以支持只知道累计／连续天数的旧舰长。天数必须带截至日期；留空表示未知，0 表示确认是零。天数为负数或非整数、缺少截至日期、起止日期倒置，以及同一截至日期的已确认累计小于已确认连续时，阻止保存并指出字段。

修改天数保存为新的人工基线，包含数值、截至日期与来源；修改起止日期或等级保存为对应会员记录的修订。修改状态保存为带观察时间的人工记录，没有有效区间时不延展为持续在舰。汇总值由已确认采用的区间和基线按第 9.3、9.4 节计算，原始自动事实始终保留。

有效区间、基线或状态之间的证据冲突可以保存为“待核实”修订，但不自动替换当前已采用版本。受影响的状态／天数／到期日期显示“待核实”，旧值只作“上次确认值”展示；暂停依赖冲突依据的正式提醒，生日等无关提醒照常。核对时展示双方来源、时间与影响范围，由主播明确选择采用平台依据或人工修订，再重新计算筛选、统计与未处理提醒。已核对的同一份证据重放不重新制造冲突，新增或更正且影响该范围的证据仍需核对。

手写记录可以修正正文与日期；自动点歌记录的原始事件 ID、身份和来源保留，人工修改作为独立修订保存，不把“点歌板”改造成一条冒充平台的新事实。界面提供“查看原始记录”，业务实现保留修订时间与前后值。取消编辑不改变原记录，保存失败保留输入；不在悬停之后才显示编辑入口。
...
幂等键使用请求接受时的认证归属和不可复用的稳定请求标识，例如随成功请求事务保存的 UUID；`request_id`／`queue_id` 仅用于追溯，不能单独作为永久来源键。同一次重连、重放、重复回调沿用该标识，只记一次；同一个人隔天再次点同一首歌，是新的有效记录，不能按歌名永久去重。旧流水补录前应为缺少稳定标识的源记录持久化分配一次，重复导入和备份恢复保留该标识。
...
| 当前状态／等级 | 覆盖查询时刻的已确认资格及档位；仅有历史观察时注明待核实 | 历史上舰记录可永久证明当前在舰，或全部历史都已完整 |
...
“在舰”筛选只纳入有已确认有效区间覆盖查询时刻、且当前资格没有未解决冲突的档案，区间可来自平台或人工确认。只有上舰事件或某时的等级／状态观察、没有可用有效区间时，显示“曾观察到上舰，当前待核实”及观察时间，不纳入“在舰”，也不据此判定已过期。已确认区间到期后可显示该段“已到期”；若另有期限未知的续费观察或冲突，当前资格仍为待核实。当前等级从仍生效的确认依据取得，不永久沿用最近购买的档位。
...
人工修正保存为独立修订，不直接篡改原始自动事件。新数据与手动修正冲突时按第 5.7 节进入核对流程，并暂停受影响的正式提醒。连续周期使用稳定标识：只有确认发生中断才开始新周期；调整本轮起始日期、基线或到期日不自行创建新周期，未知历史缺口不作为已中断证据。
...
同一人同一天的多个事项合并展示，但各自保留处理状态。提醒按“档案 + 稳定规则标识 + 本次事项标识”去重，发生日期用于安排提醒，不单独决定是否为新事项：

- 生日和每年重复的纪念日以发生年度区分本次事项；修改同年度月日保留处理状态，下一年度是新事项。
- 连续在舰里程碑以稳定连续周期标识和天数阈值区分事项；同一周期改日期或基线不重新庆祝，确认中断后新一轮达到相同阈值可以提醒。
- 累计里程碑按累计阈值区分事项，不因中断或基线修订重复庆祝。一次性纪念日使用稳定事项标识；到期提醒绑定其确认依据，修正同一到期边界保留事项标识，确认续费形成的新到期边界生成新事项。

修改日期时撤销旧的通知安排并重新安排同一未处理事项，不额外创建一条；已处理／已忽略状态保留。提前 7 天与当天属于同一事项的两个通知时机，分别去重；提前已处理或忽略本次后，当天不再通知。无关备注修改、重新打开页面、程序重启或重连不能让同一事项复活。

天数下调到未达标时，尚未处理的里程碑退出到期提醒，待可信依据再次达到阈值后恢复同一事项。已经处理的里程碑保留处理历史并注明“修订后未达标”，本轮再次达标不自动重复提醒。例如本轮百日提醒已处理，随后修正为 90 天，十天后不再次通知；确认中断后的新一轮百日可以通知。
...
| MembershipRecord | 上舰事件、有效区间、等级、置信来源、修订与连续周期标识；人工基线需单独语义 |
| Reminder | 日期／里程碑规则、稳定事项标识、计划日期，以及本次事项的处理状态 |
...
- 一条远端事实入档与消费游标应原子提交，重复读取安全。
- 本地点歌成功事务必须同时持久化符合归档条件的歌曲快照，或包含完整快照的待归档记录，携带稳定请求标识、当时的认证归属与目标档案。若采用后续消费，档案写入与完成标记原子提交；恢复不能按当前登录账号重新认领。点歌成功后即使进程崩溃、源队列／歌曲流水被清理，也能独立完成归档；仅依赖事后查询 `requests` 不满足此承诺。
...
输入范围分开交付：第一阶段支持恢复 LIRA 完整档案备份；第二阶段支持第 8.1 节中经确认的本机旧点歌补录；第三阶段再支持外部 CSV／其他工具档案导入。首发普通列表导出的 CSV 不承诺可恢复成完整档案，也不提供尚未实现的外部导入入口。
...
删除档案不联动删除原始礼物账本。普通队列／歌曲数据清理不删除档案副本和未消费的待归档记录；待归档数据不能再依赖已清理的源行或关联曲库才能恢复。全量数据清理必须明确列出是否包含粉丝档案及其待归档数据，只有用户选择包含时才一并删除。备份恢复前预览归属与冲突，并保留恢复前快照。
...
| 阶段 | 内容 | 完成标准 |
| --- | --- | --- |
| 第一阶段：可用档案 | 百宝箱入口、身份与称呼、生日／星座／MBTI、共同话题／相处提醒／备注、互动手记、搜索筛选、手动会员区间与基线及已有记录编辑、基本提醒、列表导出、完整备份恢复 | 离线也能完整维护一个粉丝；老舰长信息可手动补录和修订；备份可恢复；私人内容不会进入投屏 |
| 第二阶段：兑现自动联动 | 已建档粉丝成功点歌归档、经确认的本机旧点歌补录、点歌人快捷入口、手动音乐偏好、常点分类；私有身份／会员契约、入场改名更新、可信上舰自动建档与恢复 | 同一身份不串档，重放不重复；客户端重启或普通流水清理不丢待归档请求；期限缺失明确待核实 |
| 第三阶段：增强能力 | 农历规则、外部 CSV／其他工具档案导入及字段映射、已验证会员区间的更完整恢复、工作台日历联动；按实际需求再考虑跨电脑同步与舰礼履约 | 每项有独立需求与验收；外部导入先预览归属、字段与冲突，不因“别人有”直接扩充 |
...
| A09 | 三个月前观察到上舰，此后没有有效区间证据 | 显示“曾观察到上舰，当前待核实”及观察日期；不纳入“在舰”，不自动判定已过期，不凭礼物金额算到期 |
| A10 | 上舰多个消息、提前续费、中断后再上舰 | 同笔不重复；续费区间合并；中断后连续段重新开始 |
| A11 | 手动到期与新平台证据冲突，随后选择采用一方 | 冲突可保存为待核实，旧值标上次确认；暂停受影响的到期／里程碑提醒，生日不受影响；选择后重算，同份证据重放不重开冲突 |
...
| A17 | 档案保存失败／进程在归档前终止 | 手动输入保留并提示；已成功点歌从同一成功事务保存的快照或待归档记录恢复，幂等补齐 |
...
| A22 | 修改已有互动的日期与正文，然后新增另一条互动 | 原记录更新并按日期重排；概览同步；修订仍关联原记录，不因位置变化串到新记录 |
| A23 | 手动修正自动点歌的歌名、歌手与日期 | 互动与音乐页一致；保留原始事件及来源，标注人工修订；不新增一次点歌 |
| A24 | 已有舰长资料只改累计／连续天数，起始日期未知 | 保存带截至日期的人工基线，不强迫编造日期；负数、小数、缺截至日期或同截至日已确认累计小于连续时阻止保存；与区间证据冲突按 A11 核对 |
| A25 | 本轮百日提醒已处理，连续天数修正为 90 天后再次达到 100 天 | 取消编辑不保存；保存后保留原始依据与处理历史并注明修订；本轮再次达标不重复提醒，确认中断后的新一轮百日可以提醒 |
| A26 | 点歌成功、待归档时进程退出，重启后先清理歌曲流水再恢复归档 | 待归档快照不随普通清理删除，无需源行即可归档一次；重复恢复不多记 |
| A27 | 保留档案、清理歌曲数据后自增请求编号重用，再次真实点同歌 | 新请求取得新的稳定标识并正常归档，不与旧请求误去重；重放旧请求仍不重复 |
| A28 | 生日提醒已处理后修改同年度月日；或提前 7 天已处理后到当天 | 本年度处理状态保留、不重复通知，下一年度正常生成新事项 |
| A29 | 首发从完整备份恢复，或确认补入本机旧点歌记录 | 恢复保留归属、稳定来源标识和提醒状态；旧点歌补录先确认归属、范围并去重；普通 CSV 不作为完整备份，外部表格导入留到第三阶段 |
| A30 | 从点歌人入口打开有音乐偏好的档案，再打开无音乐资料的档案 | 概览一行可见人工偏好或最近点歌，详细记录进入音乐页；无数据时收起摘要，不显示空卡片 |
...
| 首次使用时的舰队名单 | 现有核查不足以保证完整历史名单可用 | 验证当前名单／历史来源；不具备时首发使用手动建档或恢复已有 LIRA 备份，外部名单导入按第三阶段交付 |
...
验证记录：调研时对照以上资料和相关源码核查关键结论，完成独立读者审查，并补清“当前在舰天数不得计入未来有效期”的边界。本轮修订补清手动修订与冲突处理、点歌恢复与清理、当前会员资格、提醒事项身份、导入分期和概览音乐摘要，并同步验收场景。仅涉及报告新增与修订，未运行应用、未迁移数据库、未进行真实粉丝数据采集，也未执行业务测试。

...
| `gift-catalog:update` | `{type:'gift-catalog:update', snapshot}`；`snapshot` 为服务器全局目录的规范化礼物数组、`version`、`stale` 与来源时间 | Electron main process 的远程目录缓存检测到新成功同步版本后，经本地运行时广播；该消息只表示服务器搜索缓存更新，Admin 不得用它覆盖当前直播间主目录，也不改变礼物事件或规则结算                                                                                                                                                                                            |

...
| `GET/POST /api/gifts/display-settings` | POST 固定 palette、三个严格递增正整数分 thresholds、visibleRows(1–10)、scrollSpeed(1–50)，后两者须为整数 | 读取/保存本地礼物展示配置，保存后广播刷新；速率线性对应每行 5–0.1 秒，默认 25。旧配置/请求的合法 intervalSeconds、paused/lowPower 兼容读入为速率 1，保留配色和行数，返回与保存仅使用新字段 | 400(设置无效) |

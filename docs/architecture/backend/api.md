...
| `GET /api/overtime/gifts`               | 无                                                                                                                                                     | 当前直播间礼物目录的最后成功快照：房间面板、在售盲盒产物与当前账号可用背包；服务器全局目录不会覆盖该快照                                                                                                                                                       | 400                                                             |
| `POST /api/overtime/gifts/refresh`      | `{}`                                                                                                                                                   | 手动刷新当前配置直播间的礼物面板、盲盒和背包；十秒内重复请求可返回缓存，并发请求单飞；是否配置远程目录不改变语义                                                                                                                                             | 400（未配置有效直播间号或 Bilibili 上游返回错误）               |
| `POST /api/overtime/gifts/server/search` | `{query}`：字符串，去除首尾空白后 **1–100 字符**                                                                                                      | 强制条件刷新服务器全局目录后按名称/ID 返回最多 100 个匹配项；远程失败但已有快照时使用旧缓存；图片成功下载后返回 `/overtime-gift-images/<basename>`，不修改当前直播间快照                                                                                     | 400（查询无效、未配置远程目录或没有可用远程快照）               |
| `POST /api/overtime/gifts/local/search` | `{query}`：字符串，去除首尾空白后 **1–100 字符**                                                                                                       | 兼容端点：三份固定 Markdown 中按名称/ID 匹配且本地图片实际存在的礼物，最多 100 个；Admin 不再调用，不修改自动刷新快照                                                                                                                                           | 400（查询无效或本地目录不可用）                                 |

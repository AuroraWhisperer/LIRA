...
| `gift_wishes` | 礼物许愿定义 | id PK、source_id FK、period(long/day/session)、礼物/variant 身份、类别、展示图片、整数 target、说明与创建时间、display_style（默认 card）和 text_template（默认空）；来源/创建时间索引 |
...
v13 由 `gift-wish-migration.js` 幂等建表，v14 追加每条许愿的展示样式和文字模板，旧许愿默认卡片且保留创建时间。原事件不变；许愿进度实时从账本计算，删除礼物流水会影响统计。状态存储由 `gift-wish-store.js` 拥有。

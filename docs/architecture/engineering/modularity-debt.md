...
| `src/electron/license/license-manager.js:createLicenseManager`；`authenticate`                      |        560 / 86 | 授权管理器；token、续期代次和撤销不能分散                        | 批次 B 已改具名输入并分离结果接受；authenticate 70 行，授权工厂 566 行继续登记                  | `test/license-manager.test.js`                                             |

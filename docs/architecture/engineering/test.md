...
> 涉及文件:[package.json](../../../package.json)(`test`/`check` 脚本)、[test/](../../../test/)(65 个 `*.test.js`)、[scripts/check-js.js](../../../scripts/check-js.js)、[scripts/inspect-wesing-playback.js](../../../scripts/inspect-wesing-playback.js)、[scripts/capture-bilibili-events.js](../../../scripts/capture-bilibili-events.js)、[scripts/bilibili-capture-electron/](../../../scripts/bilibili-capture-electron/)

本文档是测试的**唯一事实源**:测试框架与命令、全部测试文件的清单与归属、静态检查、专用诊断、辅助捕获脚本均只在此成表。构建/发布相关命令见 [build.md](build.md)。

## 1. 框架与命令

- **框架**:Node 内置 `node:test` + `node:assert/strict`,**零第三方测试依赖**([package.json:13](../../../package.json#L13));65 个测试文件全部基于 `node:test`。
- **全量运行**:`npm test` = `node --experimental-vm-modules --test --test-concurrency=4` — **并发数 4**。
- **为什么需要 `--experimental-vm-modules`**:源码是 CJS(`require`),但 **7 个测试在 vm 中求值浏览器端 ESM 模块**(`public/js/` 下的 `import`/`export`,经 `vm.SourceTextModule` 或动态 `import()` 加载):desktop-lyrics、frontend-regressions、playback-cache、playback-queue、playback-store 系列、provider-manager、song-library-filter、song-library-filter-menu。去掉该 flag 这些测试会失败。
- **单文件运行**:`node --experimental-vm-modules --test test/xxx.test.js`(flag 必须保留)。
- **测试方式**:全部离线单元测试 — 不启动真实服务、不碰网络;服务端模块直接 require 真实实现并注入内存数据库或 mock;浏览器模块用 vm + 假 `window`/`localStorage` 求值(如 provider-manager.test.js 注入 mock `musicAPI`)。

## 2. 测试清单(唯一成表处,按簇分组)

| 测试文件 | 被测模块 | 覆盖文档 |
|---|---|---|
| **AI 助手(9)** | | [backend/ai.md](../backend/ai.md) |
...
| **Bilibili 弹幕/协议(11)** | | [backend/bilibili/danmaku.md](../backend/bilibili/danmaku.md) |
...
| **礼物(6)** | | [backend/bilibili/gift.md](../backend/bilibili/gift.md) |
...
| [gift-service.test.js](../../../test/gift-service.test.js) | 礼物落库服务(packet-parser) | 同上 |
| [guard-gift.test.js](../../../test/guard-gift.test.js) | `bilibili/packet-parser`(舰队/守护礼物) | 同上 |
| **音乐服务(8)** | | [backend/music/services.md](../backend/music/services.md) |
...
| **全民 K 歌(wesing,5)** | | [backend/music/wesing.md](../backend/music/wesing.md) |
...
| **加班机(3)** | | [backend/overtime.md](../backend/overtime.md) |
...
| **服务器核心(3)** | | [backend/server-core.md](../backend/server-core.md) |
...
| **存储(1)** | | [backend/storage.md](../backend/storage.md) |
| [playback-store.test.js](../../../test/playback-store.test.js) | `storage/playback-store`(播放状态库) | 同上 |
| **前端(12)** | | 见各列 |
| [frontend-regressions.test.js](../../../test/frontend-regressions.test.js) | `public/` 全站页面/CSS/JS(回归) | [frontend/pages.md](../frontend/pages.md) |
...
| [playback-queue.test.js](../../../test/playback-queue.test.js) | `public/js`(播放队列逻辑) | 同上 |
...
- **命名与布局**:`test/` 平铺,**不建子目录**;`xxx.test.js` 命名,对应 `src/xxx.js`(或 `scripts/xxx.js`)模块,测试即模块级契约。

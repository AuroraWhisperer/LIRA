...
| 业务门面                   | 内部模块                                                                                                                                      | 所有权边界                                                                                |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `settings.js`              | `settings-auth.js` / `settings-form.js` / `settings-license.js` / `settings-blindbox.js` / `settings-operations.js`                           | 门面接 DOM 与兼容 API；内部模块分别拥有认证、表单、授权设备、盲盒 URL 和保存/导入导出操作 |
...
| `Events.GIFT_CATALOG_UPDATED`                                                                    | stateService(gift-catalog:update)→ overtime.js  | 服务器目录版本变化时只重绘加班机礼物选择器；不触发歌库或礼物事件重载 |
...
加班机礼物选择器的 `GET /api/overtime/gifts` 由本地运行时优先提供服务器全局目录缓存，并通过 `gift-catalog:update` 在远程同步版本变化时局部重绘；没有远程快照时继续显示旧本地房间目录，本地映射搜索仍可手动添加目录外礼物。远程请求由 Electron main process 发起，生产与 SSH loopback 入口的配置说明见 [backend/overtime.md](../backend/overtime.md) §1.5。

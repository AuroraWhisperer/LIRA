...
| `/danmaku`         | [overlays/danmaku.html](../../../public/pages/overlays/danmaku.html)                                               | OBS/直播姬浏览器源、管理页预览 `<iframe>`                    | 固定弹幕姬地址；固定区域样式按顺序显示普通文字、身份信息和 B 站表情，全屏随机样式只显示发送者与正文并在全画布随机定位，按 `danmakuFullscreenDurationSeconds` 自动消失；身份横卡使用右侧头像、四档身份底色和 384×640 等比缩放设计画布，`?preview=1&style=…` 只用于 Admin 的确定性样本预览                                                                                                                                                                                                                                                                                                          |

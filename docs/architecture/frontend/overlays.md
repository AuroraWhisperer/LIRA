...
独立 interactions scope，仅展示当前类别 3 会话；空场透明。推荐浏览器源 800×600，选项区 352px，选项行 88px，翻页步长 320px，每页 8 秒。主持端按同一布局估算 P 屏至少 P×8 秒；更短时长仅提示，不延长截止。每场从第一屏开始，实时更新保留 DOM 与滚动位置，调用 `startOverlayPages` 一次并在切场/卸载清理。用户交互或页面隐藏会暂停翻页。

投票使用固定顺序与 0–100% 标尺，票数/百分比固定在完整轨道内，零票不隐藏；结束标最高/并列最高，零参与无胜者。评分收集中均分区域为空，结束保留两位小数及人数；无人评分显示暂无有效评分。接收曾中断时保留提示。页面 GET、刷新或开多个实例均不启动/延长收集。截止由本地服务控制，与页面可见性无关。

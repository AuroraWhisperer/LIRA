...
- **风格 3–6 的词条缩放**:每种词条按素材自身可见区域的宽高比整体缩小,词条盒、位图、文字窗口和序号共用同一坐标系,不再通过压低行高裁去上下装饰。风格 3 在 CSS 背景坐标中排除原 PNG 顶部和右侧的大块透明留白,不改写原始素材;风格 4/5 使用裁除透明余量后的完整 PNG 比例并占内容窗宽度的 94%;风格 6 使用完整 PNG 比例占 72%,三款列表起点都避开画框顶部前景装饰。

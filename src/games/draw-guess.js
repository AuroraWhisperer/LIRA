...
const {
  DEFAULT_WORDS,
  getWordCategorySummaries,
} = require('./draw-guess-words');
...
function createDrawGuessState(options = {}) {
  ...
  const requestedCategoryIds = normalizeCategoryIds(
    options.categoryIds,
    normalizedWords,
  );
  const words =
    requestedCategoryIds === null
      ? normalizedWords
      : normalizedWords.filter((entry) =>
          requestedCategoryIds.includes(entry.categoryId),
        );
  ...
  return {
    ...
    categoryIds: requestedCategoryIds || [
      ...new Set(words.map((entry) => entry.categoryId).filter(Boolean)),
    ],
...
function submitGuess(state, danmaku = {}, nowMs = 0) {
  if (!state || state.phase !== 'drawing')
    return reject('当前不在答题阶段。', state);
  const uid = String(danmaku.uid || '').trim();
  if (!uid) return reject('无法识别观众身份。', state);
  if (state.correct.some((item) => item.uid === uid))
    return reject('本局已经答对。', state);
  const guess = normalizeGuessText(danmaku.message);
  if (
    !guess ||
    !state.answerAliases.some((answer) => normalizeGuessText(answer) === guess)
  ) {
...
function finishRound(state, nowMs = 0, options = {}) {
  ...
  return {
    ...state,
    phase:
      state.round >= state.totalRounds && reveal && !deferFinal
        ? 'finished'
        : 'round-result',
...
function revealAnswer(state) {
  if (!state || state.phase !== 'round-result' || state.answerRevealed)
    return state;
  return {
    ...state,
...
function applyDrawOperation(state, input = {}) {
      ...
      state: {
        ...
        canvas: {
          ...
          totalPoints: Math.max(
            0,
            state.canvas.totalPoints - stroke.points.length,
          ),
  ...
  if (!points)
    return reject(
      `每次请提交 1-${MAX_POINTS_PER_OPERATION} 个有效坐标。`,
      state,
    );

  const strokeIndex = state.canvas.strokes.findIndex(
    (stroke) => stroke.id === strokeId,
  );
  const existing = strokeIndex >= 0 ? state.canvas.strokes[strokeIndex] : null;
  if (!existing && state.canvas.strokes.length >= MAX_STROKES)
    return reject('本局笔画数量已达上限。', state);
  if (existing && (existing.color !== color || existing.width !== width))
    return reject('同一笔画的样式不能改变。', state);
  if ((existing?.points.length || 0) + points.length > MAX_POINTS_PER_STROKE)
    return reject('单笔坐标数量已达上限。', state);
  if (state.canvas.totalPoints + points.length > MAX_TOTAL_POINTS)
    return reject('本局画布坐标数量已达上限。', state);

  const strokes = state.canvas.strokes.map((stroke) => ({
    ...stroke,
  ...
  return words
    .map((entry) => {
      ...
      const categoryId = String(entry?.categoryId || '')
        ...
        .slice(0, 32);
      const category =
        String(entry?.category || '综合')
          .trim()
          .slice(0, 24) || '综合';
      const aliases = [
        ...new Set(
          [word, ...(Array.isArray(entry?.aliases) ? entry.aliases : [])]
            .map((value) =>
              String(value || '')
                .trim()
                .slice(0, 24),
...
function normalizeCategoryIds(value, words) {
  ...
  const available = new Set(
    words.map((entry) => entry.categoryId).filter(Boolean),
  );
...
function normalizePoints(points) {
  if (
    ...
    points.length < 1 ||
    points.length > MAX_POINTS_PER_OPERATION
  )
    return null;
  const normalized = [];
...
function normalizeRoundDurationSeconds(value) {
  const seconds = Number(
    value === undefined ? DEFAULT_ROUND_DURATION_SECONDS : value,
  );
  return Number.isInteger(seconds) &&
    seconds >= MIN_ROUND_DURATION_SECONDS &&
    seconds <= MAX_ROUND_DURATION_SECONDS
    ? seconds

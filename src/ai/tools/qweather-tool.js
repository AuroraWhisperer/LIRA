    ...
    return requestWithQuota(
      ...
      (payload) => {
        ...
        if (isAmbiguousLocation(location, candidates)) {
          return {
            ambiguous: true,
            candidates: candidates.slice(0, 3).map(normalizeLocation),
          };
        }
...
function normalizeLocation(item) {
  return {
    ...
    adm1: String(item?.adm1 || ''),
    adm2: String(item?.adm2 || ''),
...
function shouldUseForecast(date) {
  return !['', 'today', '今天', 'now', '现在'].includes(
    String(date || '')
      .trim()
      .toLowerCase(),
  );

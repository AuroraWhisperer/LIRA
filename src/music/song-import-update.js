  ...
  const rows = input.rows.map((raw, index) => {
    ...
    try {
      ...
      const blank = new Set(
        present.filter(
          (field) =>
            !SONG_IMPORT_ALIASES[field].some((key) => Object.hasOwn(raw, key) && String(raw[key] ?? '').trim() !== ''),
        ),
      );
      if (present.includes('isEnabled') && !blank.has('isEnabled')) {
        ...
        if (
          !['是', '可点', '启用', 'true', 'yes', 'y', '1', '否', '不可点', '停用', 'false', 'no', 'n', '0'].includes(
            enabled,
          )
        ) {
    ...
    const after = existing
      ? Object.fromEntries(
          Object.entries(STORED_FIELDS).map(([field, column]) => [
            field,
            field === 'isEnabled' ? Boolean(existing[column]) : (existing[column] ?? ''),
          ]),
        )
      : { ...song };
    if (existing) {

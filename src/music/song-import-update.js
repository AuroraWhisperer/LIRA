...
function buildImportPlan(currentSongs, input, currentCategories) {
  if (
    ...
    (input.allowEmptyClear !== undefined &&
      typeof input.allowEmptyClear !== 'boolean')
  ) {
    throw importError(
      'SONG_IMPORT_INPUT_INVALID',
      '请提供 1 至 5000 行歌曲，空值选项必须是布尔值。',
      400,
    );
  }
  ...
  const rows = input.rows.map((raw, index) => {
    ...
    try {
      ...
      for (const field of present) {
        for (const alias of SONG_IMPORT_ALIASES[field]) {
          ...
          if (
            ...
            !(field === 'isEnabled' && typeof value === 'boolean')
          ) {
      ...
      if (song.requestPrice.length > 1000)
        throw new Error('点歌价格超过 1000 长度限制');
      const blank = new Set(
        present.filter(
          (field) =>
            !SONG_IMPORT_ALIASES[field].some(
              (key) =>
                Object.hasOwn(raw, key) && String(raw[key] ?? '').trim() !== '',
            ),
        ),
      );
      if (present.includes('isEnabled') && !blank.has('isEnabled')) {
        const value = SONG_IMPORT_ALIASES.isEnabled
          .map((key) => raw[key])
      ...
      const signature = JSON.stringify(
        present.map((field) => [field, blank.has(field) ? '' : song[field]]),
      );
      ...
      if (String(error.message).includes('价格别名冲突'))
  ...
  for (const group of groups.values()) {
    ...
    if (existing) {
      for (const field of present) {
        ...
        if (blank.has(field) && (!allowEmptyClear || field === 'isEnabled'))
          continue;
        after[field] = song[field];
      }
    }
    entry.differences = Object.entries(STORED_FIELDS).flatMap(
      ([field, column]) => {
        const before = existing
          ? field === 'isEnabled'
            ...
            : (existing[column] ?? '')
          : null;
        return before === after[field]
          ? []
          : [{ field, before, after: after[field] }];
      },
    );
    ...
    if (entry.status !== 'unchanged')
      changes.push({ id: existing?.id, song: after });
  }
...
function previewSongImport(store, input) {
  const { changes, ...preview } = buildImportPlan(
    store.listRows(),
    input,

...
function createFanMergeService({ store, now, detail, requireProfile }) {
  function preview(scope, input) {
    ...
    if (
      ...
      (input.targetRevision && target.revision !== input.targetRevision)
    ) {
    ...
    if (
      patch.identity &&
      identityKey(patch.identity) !== identityKey(target.identity)
    )
  ...
  function merge(scope, input) {
    ...
    if (
      !['source', 'target'].includes(input.prefer) ||
      input.targetRevision !== plan.target.revision
    )

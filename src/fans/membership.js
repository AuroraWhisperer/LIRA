...
function coveredDays(intervals, at, after = '') {
  const days = intervals
    ...
    .map((item) => ({
      start: Math.max(
        dayStart(dayOf(item.start)),
        after ? dayStart(addDays(after, 1)) : -Infinity,
      ),
      end: dayStart(addDays(dayOf(Math.min(item.end - 1, at)), 1)),
    }))
    .filter((item) => item.end > item.start);
  return unionIntervals(days).reduce(
    (total, item) => total + Math.round((item.end - item.start) / 86400000),
    0,
  );
}
...
function summarizeMembership(records, atValue = Date.now()) {
  ...
  const all = records.filter(
    (r) => r.kind === 'membership' && !r.data.excluded,
  );
  ...
  const observations = adopted
    .filter(
      (r) =>
        r.data.type === 'observation' && Date.parse(r.data.observedAt) <= at,
    )
    .sort((a, b) => b.data.observedAt.localeCompare(a.data.observedAt));
  const unknownRenewal =
    observations[0] &&
    (!latest || Date.parse(observations[0].data.observedAt) >= latest.end);
  const bases = adopted
    .filter((r) => r.data.type === 'baseline' && r.data.asOf <= today)
    .sort(
      (a, b) =>
        b.data.asOf.localeCompare(a.data.asOf) ||
  ...
  const totalDays = totalBase
    ...
    : intervals.length
      ? recordedDays
      : null;
  let continuousDays = segment
    ? daysBetween(dayOf(segment.start), today) + 1
    : null;
  let continuousAsOf = segment ? today : null;
  let cycleId =
    active.sort((a, b) => a.start - b.start)[0]?.record.data.cycleId || null;
  if (continuousBase) {
    const base = continuousBase.data;
    const extension = merged.find(
      (r) =>
        r.start <= dayStart(addDays(base.asOf, 1)) &&
        r.end > dayStart(addDays(base.asOf, 1)),
    );
    if (
      today === base.asOf ||
      (extension && extension.end > at && extension.start <= at)
    ) {
      continuousDays =
        base.continuousDays +
        coveredDays([extension].filter(Boolean), at, base.asOf);
    ...
    : active.length
      ...
      : unknownRenewal
        ...
        : latest
          ? 'expired'
          : 'unknown';
  const expiry = segment
    ? intervals.find((item) => item.end === segment.end)
    : latest;
  return {
    status,
    level:
      status === 'active'
        ? Math.min(...active.map((r) => r.record.data.level))
        : null,
    ...
    continuousAsOf,
    totalAsOf:
      totalBase && !coveredDays(merged, at, totalBase.data.asOf)
        ? totalBase.data.asOf
        : today,
...
function membershipConflicts(candidate, records, excludeId) {
  ...
  for (const record of adopted) {
    ...
    if (
      data.type === 'interval' &&
      other.type === 'interval' &&
      candidate.original.source !== record.original.source
    ) {
      const overlaps = data.startAt < other.endAt && other.startAt < data.endAt;
      if (
        overlaps &&
        (data.startAt !== other.startAt ||
          data.endAt !== other.endAt ||
    ...
    if (data.type === 'baseline' && other.type === 'interval') {
      ...
      if (
        ...
        (data.continuousDays !== null &&
          summary.continuousDays > data.continuousDays)
      )
        conflicts.push(record.id);
    }
    if (

...
function createFanRecordStore(db) {
  function list(scope, profileId) {
    return db
      .prepare(
        'SELECT * FROM fan_records WHERE scope = ? AND profile_id = ? ORDER BY occurred_at DESC, id',
      )
      .all(scope, profileId)
      .map(decodeRecord);
  }
  ...
  function insert(scope, profileId, record) {
    if (record.sourceKey) {
      ...
      if (existing) {
        if (existing.profile_id !== profileId)
          throw new Error('相同来源记录已关联另一份档案，请先核对。');
    ...
    return decodeRecord(
      db
        .prepare('SELECT * FROM fan_records WHERE scope = ? AND id = ?')
        .get(scope, id),
    );
  }
  ...
  return {
    ...
    bySource: (scope, key) =>
      decodeRecord(
        db
          .prepare(
            'SELECT * FROM fan_records WHERE scope = ? AND source_key = ?',
          )
          .get(scope, key),
      ),
  };

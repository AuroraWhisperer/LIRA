...
function createFanProfileStore(db) {
  ...
  function byIdentity(scope, key) {
    return decodeProfile(db.prepare('SELECT * FROM fan_profiles WHERE scope = ? AND identity_key = ?').get(scope, key));
  }
  ...
  function save(scope, profile, key, now) {
    ...
    if (profile.id) {
      const result = db
        ...
        .run(key, JSON.stringify(profile), now, scope, profile.id, profile.revision);
      if (result.changes !== 1) throw new Error('档案已更新，请重新加载后核对；未保存的输入仍保留。');
      return get(scope, profile.id);
    }
    const id = randomUUID();
    db.prepare('INSERT INTO fan_profiles (id, scope, identity_key, data, updated_at) VALUES (?, ?, ?, ?, ?)').run(
      id,
      scope,
      key,
  ...
  function getScope(scope) {
    const row = db.prepare('SELECT data FROM fan_scopes WHERE scope = ?').get(scope);
    return row
      ? JSON.parse(row.data)
      : {
          initialized: false,
  ...
  function states(scope, profileId) {
    return db
      .prepare('SELECT item_key, data FROM fan_reminder_states WHERE scope = ? AND profile_id = ?')
      .all(scope, profileId)
  ...
  function remove(scope, id, suppress) {
    const row = db.prepare('SELECT identity_key FROM fan_profiles WHERE scope = ? AND id = ?').get(scope, id);
    if (!row) throw new Error('档案不存在。');
    if (suppress && row.identity_key) {
      db.prepare('INSERT OR IGNORE INTO fan_suppressions VALUES (?, ?)').run(scope, row.identity_key);
    }
    db.prepare('DELETE FROM fan_profiles WHERE scope = ? AND id = ?').run(scope, id);
  }

  function removeAll(scope) {
    const result = db.prepare('DELETE FROM fan_profiles WHERE scope = ?').run(scope);
  ...
  function snapshot(scope, data, now) {
    const id = randomUUID();
    db.prepare('INSERT INTO fan_restore_snapshots VALUES (?, ?, ?, ?)').run(id, scope, now, JSON.stringify(data));
    return id;
  }
  ...
  function restoreProfile(scope, profile, key, now) {
    ...
    ).run(profile.id, scope, key, JSON.stringify(profile), profile.revision || 1, now);
    return get(scope, profile.id);
  }

  function moveRecords(scope, from, to) {
    if (!get(scope, from) || !get(scope, to)) throw new Error('待合并档案不存在。');
    db.prepare('UPDATE fan_records SET profile_id = ? WHERE scope = ? AND profile_id = ?').run(to, scope, from);
  }
  ...
  function getSnapshot(scope, id) {
    const row = db.prepare('SELECT data FROM fan_restore_snapshots WHERE scope = ? AND id = ?').get(scope, id);
  ...
  function claimRequest(id, scope) {
    const row = db.prepare('SELECT stable_id, owner_scope FROM requests WHERE id = ?').get(id);
    if (!row || (row.owner_scope && row.owner_scope !== scope)) throw new Error('点歌归属已变化，请重新预览。');
    const stableId = row.stable_id || randomUUID();
    db.prepare('UPDATE requests SET stable_id = ?, owner_scope = ?, identity_type = ? WHERE id = ?').run(
      stableId,
      scope,
      'uid',
      id,
  ...
  return {
    ...
    unsuppress: (scope, key) =>
      db.prepare('DELETE FROM fan_suppressions WHERE scope = ? AND identity_key = ?').run(scope, key),
  };
}

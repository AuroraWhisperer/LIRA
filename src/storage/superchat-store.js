...
function createSuperChatStore(superChatDb) {
  return {
    findByPlatformId(platformId) {
      return normalizeSuperChatRow(superChatDb.prepare(`
    ...
    insert(input) {
      ...
      `).run(
        input.platformId, input.uid, input.userName, input.price, input.message,
        input.requesterGuardLevel, input.requesterMedalName, input.requesterMedalLevel,
        input.createdAt, input.createdAt
      );
      return normalizeSuperChatRow(
        superChatDb.prepare('SELECT * FROM super_chats WHERE id = ?')

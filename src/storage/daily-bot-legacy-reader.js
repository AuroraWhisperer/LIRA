...
function createDailyBotLegacyReader(db, settingsStore) {
  function libraries() {
    ...
    return { blessings: parse(settings.checkinBlessings, CHECKIN_BLESSINGS),
      fortunes: parse(settings.fortunePool, FORTUNES) };
  }
  return {
    summary() {
      const row = db.prepare(`SELECT count(*) AS count,min(total_days) AS minDays,
        max(total_days) AS maxDays,max(last_checkin_date) AS lastDate FROM checkin_users`).get();
      const saved = libraries();
      return { ...row, customLibraries: JSON.stringify(saved.blessings) !== JSON.stringify(CHECKIN_BLESSINGS) ||
    ...
    read() {
      ...
      try {
        ...
        const checkins = db.prepare(`SELECT uid,user_name AS userName,total_days AS totalDays,
          first_checkin_at AS firstCheckinAt,last_checkin_at AS lastCheckinAt,last_checkin_date AS lastCheckinDate
          FROM checkin_users ORDER BY uid COLLATE BINARY`).all().map((row) => ({ ...row }));

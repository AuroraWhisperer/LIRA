  ...
  return {
    ...
    insertRequest(input) {
      ...
      try {
        ...
        `).run(
          input.songId, input.songName, input.artist, input.categoryName,
          input.requesterUid, input.requesterName,
          input.requesterGuardLevel, input.requesterMedalName, input.requesterMedalLevel,
          input.source, input.status, input.isPinned, input.pinnedAt,
        ...
        `).run(
          queueId, input.songId, input.songName, input.artist, input.categoryName,
          input.requesterUid, input.requesterName,
          input.requesterGuardLevel, input.requesterMedalName, input.requesterMedalLevel,
          input.message, input.source, input.createdAt
        );

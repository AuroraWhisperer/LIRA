...
const {
  ...
  sanitizeSettings,
  sanitizeTakeover,
} = require('../shared/daily-bot-contract');
...
function createDailyBotController({ licenseManager, getLegacyReader, sourceLabel, now = Date.now }) {
  ...
  function context() {
    ...
    const owner =
      !disposed && licenseManager.isAuthorized() && identity?.streamerId
        ? JSON.stringify([
    ...
    if (current?.owner !== owner) {
      current = owner ? { owner, id: randomUUID() } : null;
      draft = null;
    }
    return current;
  }
  ...
  async function prepare(captured, input) {
    ...
    const status =
      settings.takeover.state === 'importing'
        ? await call(captured, 'status', { id: settings.takeover.importId })
        : null;
    const legacy = reader().read();
    const sourceHash = digest(legacy);
    const data = {
      schemaVersion: 1,
      sourceId: digest({ host: hostname(), path: sourceLabel }),
      cutoffAt: status?.cutoffAt || new Date(now()).toISOString(),
      libraryChoice: input.libraryChoice,
      checkins: legacy.checkins,
      blessings:
        input.libraryChoice.checkin === 'builtin'
          ? CHECKIN_BLESSINGS
          : input.libraryChoice.checkin === 'corrected'
            ? input.blessings
            : legacy.blessings,
      fortunes:
        input.libraryChoice.fortune === 'builtin'
          ? FORTUNES
          : input.libraryChoice.fortune === 'corrected'
            ? input.fortunes
            : legacy.fortunes,
    };
  ...
  async function unchanged(captured, item, startedRevision) {
    ...
    draft = null;
    invalid('DAILY_BOT_SOURCE_CHANGED');
  }
  async function apply(captured, input) {
    ...
    const state = sanitizeTakeover(
      await call(captured, 'start', {
        body: {
          id: item.id,
          sourceId: data.sourceId,
          cutoffAt: data.cutoffAt,
          digest: item.digest,
          totalRows: data.checkins.length,
          totalBatches,
          libraryChoice: data.libraryChoice,
          expectedRevision: item.expectedRevision,
          legacyStoppedConfirmed: true,
  ...
  async function invoke(request) {
    ...
    try {
      ...
      switch (request.action) {
        case 'open':
          exact(input, []);
          result = { data: await read(captured) };
          break;
        case 'summary':
          exact(input, []);
        ...
        case 'update': {
          ...
          if (
            !['checkin', 'fortune'].includes(input.kind) ||
          ...
          result = {
            data: sanitizeSettings(
              await call(captured, 'update', {
                kind: input.kind,
                body: { enabled: input.enabled, expectedRevision: input.expectedRevision },
              }),
            ),
          };
          break;
        ...
        case 'decide': {
          ...
          if (
            !['no-legacy', 'fresh-start'].includes(input.decision) ||
            input.legacyStoppedConfirmed !== true ||
            !Number.isSafeInteger(input.expectedRevision) ||
            input.expectedRevision < 0
          )
            invalid();
          if (input.decision === 'no-legacy' && (summary().count || summary().customLibraries))
            invalid('DAILY_BOT_LEGACY_PRESENT');
          await call(captured, 'decide', { body: input });
          result = { data: await read(captured) };
          break;
        }
        case 'prepare':
          result = await prepare(captured, input);
          break;
        case 'apply':
          result = await apply(captured, input);
          break;
        case 'cancel': {
      ...
      return {
        ...
        accountName: String(licenseManager.getCloudSyncIdentity()?.accountName || ''),
      };
    } finally {
      busy = false;
    }
...
function safePreflight(value) {
  return {
    ...
    issues: Array.isArray(value.issues)
      ? value.issues.slice(0, 100).map((item) => ({

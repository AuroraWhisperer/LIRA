...
function createLicenseOperations(options = {}) {
  ...
  async function getWelcomeSettingsV2() {
    return overlayOperation(async (token) => {
      try {
        return sanitizeWelcomeV2(await remote.getWelcomeSettingsV2(token));
      } catch (error) {
  ...
  async function getGiftEventsInternal(input = {}) {
    ...
    const syncEpoch = input.syncEpoch === null || input.syncEpoch === undefined ? null : input.syncEpoch;
    if (
      (after !== null && after !== undefined && (!Number.isSafeInteger(after) || after < 0)) ||
      !Number.isInteger(limit) ||
      limit < 1 ||
      limit > 200 ||
      (syncEpoch !== null && (typeof syncEpoch !== 'string' || !syncEpoch || syncEpoch.length > 128))
    ) {
      throw new RemoteLicenseError('INVALID_GIFT_CURSOR', '礼物事件游标无效。');
    }
    return withAuthorizedToken(
      async (token) => {
        const result = await remote.getGiftEvents(after === null || after === undefined ? null : after, limit, token, {
          syncEpoch,
          signal: input.signal,
        });
        try {
  ...
  async function getGiftHistoryInternal(input = {}) {
    const pageToken = input.pageToken === null || input.pageToken === undefined ? null : input.pageToken;
    if (pageToken !== null && (typeof pageToken !== 'string' || !pageToken || pageToken.length > 4096)) {
      throw new RemoteLicenseError('INVALID_BOOTSTRAP_TOKEN', '礼物历史页令牌无效。');
    }
    return withAuthorizedToken(
      async (token) => {
  ...
  async function getSongPageBackground() {
    const result = await withAuthorizedToken((token) => remote.getSongPageBackground(token));
  ...
  async function uploadSongPageBackground(bytes, fileName) {
    ...
    if (!buffer?.length) throw new RemoteLicenseError('BACKGROUND_IMAGE_REQUIRED', '请选择图片文件。');
    if (buffer.length > SONG_BACKGROUND_MAX_BYTES)
      throw new RemoteLicenseError('PAYLOAD_TOO_LARGE', '图片超过 5MB，请压缩后再上传。');
    const extension =
      String(fileName || '')
        .split('.')
        .pop()
    ...
    if (!contentType)
      throw new RemoteLicenseError('BACKGROUND_FORMAT_UNSUPPORTED', '仅支持 PNG / JPG / WebP / GIF 图片。');
    const result = await withAuthorizedToken((token) => remote.uploadSongPageBackground(buffer, contentType, token));
    return addSongBackgroundPreviewUrl(result, remote.baseUrl);
  }

  async function deleteSongPageBackground() {
    return withAuthorizedToken((token) => remote.deleteSongPageBackground(token));
  }

...
const { lotteryError, normalizeRules, buildCandidatePool, assertSameContext } = require('./rules');

function publicCode(error) {
  const code = error?.code;
  return typeof code === 'string' && /^LOTTERY_[A-Z_]{1,64}$/u.test(code) ? code : 'LOTTERY_OPERATION_FAILED';
}
...
function createDynamicLotteryService({
  ...
  async function getContext() {
    ...
    if (!job || job !== active || !sameIdentity(job.identity, identity()) || !sameIdentity(job.identity, context)) {
  ...
  function ownedTask(taskId, scope) {
    if (typeof taskId !== 'string' || taskId.length > 64) throw lotteryError('LOTTERY_TASK_NOT_FOUND');
    const task = store.getTask(taskId);
    if (!task || task.streamerId !== scope.streamerId) throw lotteryError('LOTTERY_TASK_NOT_FOUND');
    return task;
  }
  ...
  function launch(scope, taskId, kind, operation) {
    ...
    job.promise = Promise.resolve()
      ...
      .catch((error) => {
        ...
        if (['LOTTERY_BILIBILI_CHALLENGE', 'LOTTERY_BILIBILI_RATE_LIMITED'].includes(code))
          pauseRequests(scope.streamerId, code);
        if (job.taskId && drawStore.getRound(job.taskId)) {
          drawStore.setStatus(ownedTask(job.taskId, scope), 'paused', code, clock.nowMs());
        }
      })
      .finally(() => {
  ...
  function getState({ taskId } = {}) {
    ...
    const ownJob = active && sameIdentity(active.identity, scope) ? active : null;
    ...
    return {
      ...
      error: lastError && sameIdentity(lastError.identity, scope) ? lastError.code : '',
    };
  }

  function createTask(input) {
    const scope = identity();
    if (typeof input?.requestId !== 'string' || !/^[a-zA-Z0-9-]{16,64}$/u.test(input.requestId))
    ...
    if (existing) {
      if (existing.rules.inputSignature !== signature) throw lotteryError('LOTTERY_DRAW_CONFLICT');
      return getState({ taskId: existing.id });
    }
    if (active?.requestId === input.requestId && sameIdentity(active.identity, scope)) {
      if (active.signature !== signature) throw lotteryError('LOTTERY_DRAW_CONFLICT');
      return getState();
    }
    if (active) throw lotteryError('LOTTERY_BUSY');
    resumeRequests(scope.streamerId);
    launch(scope, null, 'collect', async (job) => {
      const inspected = await provider.inspectDynamic(link.url, job.controller.signal);
      await getContext();
      ...
      if (
        rules.requiredActions.some(
          (source) =>
            !Number.isSafeInteger(inspected.target.expectedReactions?.[source]) ||
  ...
  function handleCollectionPause(taskId, scope) {
    ...
    if (['LOTTERY_BILIBILI_CHALLENGE', 'LOTTERY_BILIBILI_RATE_LIMITED'].includes(reason))
      pauseRequests(scope.streamerId, reason);
  }

  function actOnTask({ taskId, action, revision } = {}) {
    const scope = identity();
    if (!['pause', 'resume', 'draw'].includes(action)) throw lotteryError('LOTTERY_RULES_INVALID');
    if (action === 'pause') {
      if (active && sameIdentity(active.identity, scope) && (!taskId || active.taskId === taskId)) {
        ...
        if (active.taskId) {
          ...
          drawStore.setStatus(task, 'paused', 'LOTTERY_OPERATION_PAUSED', clock.nowMs());
        }
      }
    ...
    if (active) {
      if (active.taskId === taskId && sameIdentity(active.identity, scope)) return getState({ taskId });
      throw lotteryError('LOTTERY_BUSY');
    }
    if (['completed', 'exhausted'].includes(task.status)) return getState({ taskId });
    if (revision !== task.revision) throw lotteryError('LOTTERY_DRAW_CONFLICT');
    const round = drawStore.getRound(task.id);
    if (action === 'draw' && store.getActiveScan(task.id)?.status !== 'completed')
      throw lotteryError('LOTTERY_COLLECTION_INCOMPLETE');
    if (action === 'resume' && !round && store.getActiveScan(task.id)?.status === 'completed')
      return getState({ taskId });
    resumeRequests(scope.streamerId);
    launch(scope, taskId, round || action === 'draw' ? 'draw' : 'collect', async (job) => {
      await provider.verifyOwner(task.ownerUid, job.controller.signal);
      await getContext();
      if (round || action === 'draw') await draw.start(task, job.controller.signal);
      else {

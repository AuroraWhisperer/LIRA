...
function throwApiError(response, payload) {
  ...
  if (
    ...
    fail(
      ...
      'Bilibili verification or risk control interrupted the request.',
      { httpStatus: response.status, bilibiliCode },
    );
  }
...
function createLotteryProvider({ request, getContext, nowMs = Date.now }) {
  ...
  async function ensureCurrentContext(context) {
    ...
    if (
      ...
      fail(
        ...
        'The trusted account session changed during the request.',
      );
    }
  }
  ...
  async function getVerifiedAccount(context, signal) {
    ...
    const payload = await requestApiJson(
      context,
      'account_and_wbi',
      `${API_ORIGIN}/x/web-interface/nav`,
      signal,
    );
    ...
    if (!data || data.isLogin !== true) {
      fail(
        'LOTTERY_BILIBILI_AUTH_REQUIRED',
        'Bilibili did not verify the current login session.',
      );
    }
    ...
    if (
      !imageInfo ||
      typeof imageInfo.img_url !== 'string' ||
      typeof imageInfo.sub_url !== 'string'
    ) {
      fail('LOTTERY_UPSTREAM_INVALID', 'WBI signing material is missing.');
    }
    let mixinKey;
    try {
      mixinKey = createBilibiliWbiMixinKey(
        imageInfo.img_url,
        imageInfo.sub_url,
      );
    } catch (_) {
  ...
  async function resolveDynamicLink(value, context, signal) {
    ...
    for (let index = 0; index < MAX_SHORT_LINK_REDIRECTS; index += 1) {
      ...
      if (![301, 302, 303, 307, 308].includes(response?.status)) {
        fail(
          ...
          'Bilibili short link did not return an allowed redirect.',
        );
      }
      ...
      if (!location) {
        fail(
          'LOTTERY_DYNAMIC_LINK_INVALID',
          'Bilibili short link redirect is missing its destination.',
        );
      }
      let destination;
      try {
    ...
    fail(
      'LOTTERY_DYNAMIC_LINK_INVALID',
      'Bilibili short link exceeded the redirect limit.',
    );
  }
  ...
  async function inspectDynamic(url, signal) {
    ...
    if (target.ownerUid !== account.ownerUid) {
      fail(
        ...
        'The dynamic does not belong to the verified Bilibili account.',
      );
    }
    return {
      target,
  ...
  async function readPage({ target, source, cursor = null, signal } = {}) {
    if (source !== 'comment') {
      fail(
        ...
        'This interaction source has not passed capability verification.',
      );
    }
    const context = await getContext();
    const account = await getVerifiedAccount(context, signal);
    ...
    const payload = await requestApiJson(
      context,
      'comment_page',
      `${API_ORIGIN}/x/v2/reply/wbi/main?${query}`,
      signal,
    );
  ...
  async function readRelation(uid, signal) {
    ...
    if (relation && hasOwn(relation, 'attribute')) {
      attribute = Number.isSafeInteger(relation.attribute)
        ? relation.attribute
        : null;
    }

...
function createGiftWishService({
  store,
  gifts,
  catalog,
  getRoomId,
  ...
  async function getSnapshot() {
    ...
    const items = store.list(source.sourceId).map((wish) => {
        ...
        wish.period === 'long'
          ...
          : wish.period === 'day'
            ...
            : session.state === 'offline' || session.ended_at
              ...
              : session.started_at;
      const end =
        wish.period === 'session' && session.stale ? session.checked_at : asOf;
      ...
      return {
        ...
        imagePath:
          artwork.get(wish.variant_id || wish.gift_id) || wish.image_path,
        target: wish.target,
        label: wish.label,
  ...
  function save(input) {
    if (!input || typeof input !== 'object' || Array.isArray(input))
      throw invalid('许愿参数无效。');
    const { source } = scope(input.viewRevision || null);
    if (
      !Number.isSafeInteger(input.target) ||
      input.target < 1 ||
      input.target > 999999999
    ) {
      throw invalid('目标数量必须是 1–999999999 的整数。');
    }
    if (typeof input.label !== 'string' || [...input.label].length > 40)
      throw invalid('许愿说明最多 40 个字。');
    const label = input.label.trim();
    if (input.id !== undefined) {
      if (
        typeof input.id !== 'string' ||
        !store.update(source.sourceId, input.id, input.target, label)
      )
        throw invalid('这条许愿已不存在，请刷新。');
      return { id: input.id };
    }
    if (!PERIODS.has(input.period)) throw invalid('请选择有效的许愿周期。');
    if (store.list(source.sourceId).length >= 30)
      throw invalid('最多保留 30 条许愿，请先删除不需要的许愿。');
    const gift = catalogItems().find(
      (item) => (item.variantId || String(item.id)) === input.giftKey,
    );
    if (!gift) throw invalid('礼物已不在目录中，请重新选择。');
    if (gift.giftCategory !== 'guard' && !gift.variantId)
      throw invalid('礼物资料尚未同步，请刷新礼物库后重新选择。');
    const wish = {
      id: randomUUID(),
      period: input.period,
  ...
  function remove(input) {
    ...
    if (
      typeof input?.id !== 'string' ||
      !store.remove(source.sourceId, input.id)
    )
      throw invalid('这条许愿已不存在，请刷新。');
    return { removed: true };
  }

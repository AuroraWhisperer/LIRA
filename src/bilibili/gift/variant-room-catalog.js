...
function mergeVariantRoomCatalog(room, server, customBoxes) {
  ...
  const gifts = (room?.gifts || []).map(gift => {
    ...
    const { variantId: _variantId, giftIdentity: _giftIdentity, ...unresolved } = gift;
  ...
  const findCustom = item => {
    const matches = server.gifts.filter(gift => gift.id === String(item?.giftId) &&
      gift.name === item?.name && gift.priceRaw === Math.round(Number(item?.price) * 1000));

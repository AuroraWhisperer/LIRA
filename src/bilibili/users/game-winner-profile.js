...
function createGameWinnerProfileResolver(options = {}) {
  const getHostIdentity = typeof options.getHostIdentity === 'function'
    ? options.getHostIdentity
    : () => null;
  const resolveRoomInfo = typeof options.resolveRoomInfo === 'function'
  ...
  return async function resolveGameWinnerProfile(winner = {}) {
    ...
    let identity = role === 'viewer'
      ? normalizeIdentity(winner)
      : null;
    try {
      if (role === 'host') identity = await resolveHostIdentity();
      if (!identity.uid) return { avatarUrl: '', name: identity.name };

      const profile = await ensureProfile(identity.uid, { fields: [...PROFILE_FIELDS] });
      return {
...
module.exports = { createGameWinnerProfileResolver, normalizeIdentity, normalizeWinnerRole };

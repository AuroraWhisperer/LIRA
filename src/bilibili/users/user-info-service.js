...
class UserInfoService {
  ...
  async ensure(uid, options = {}) {
    ...
    if (explicitRoomId && (!this.roomScope
  ...
  listRecent(options = {}) {
    ...
    this.cleanupExpired();
    return this.identityCache.listRecentUids()
  ...
  listOnline(options = {}) {
    ...
    if (!this.matchesRequestedRoom(options.roomId)) return [];
    this.cleanupExpired();
    return this.identityCache.listOnlineUids()
      .map(uid => this.records.get(uid))
      .filter(Boolean)
      .map(record => this.project(record, selection))
  ...
  notify(record, changedFields) {
    for (const subscription of [...this.subscriptions]) {
      ...
      if (subscription.roomId && (!this.roomScope
  ...
  matchesActiveRun(context) {
    ...
    return cleanText(context.roomId) === this.activeRoomRun.roomId
...
function normalizeSource(value, diagnostics) {
  ...
  else if (diagnostics && typeof diagnostics.record === 'function') diagnostics.record(entry);
...
function roomEvidence(value, source, observedAt) {
  return { value, source, observedAt, authority: SOURCE_AUTHORITY[source], verified: true };
}
...
function shouldReplaceName(current, value, source, observedAt) {
  ...
  if (incomingQuality !== currentQuality) return incomingQuality > currentQuality;
...
function shouldReplaceRoomField(current, incoming, nowMs) {
  ...
  if (incoming.authority !== current.authority) return incoming.authority > current.authority;

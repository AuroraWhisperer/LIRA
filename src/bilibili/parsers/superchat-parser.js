...
function extractBilibiliSuperChatMessage(packet, roomOwnerUid = '') {
  ...
  return {
    id: cleanText(
      readObjectValue(data, ['id', 'message_id', 'messageId', 'token']),
    ),
    message: cleanText(
      readObjectValue(data, ['message', 'message_trans', 'messageTrans']),
    ),
    price: normalizeSuperChatPrice(
      readObjectValue(data, ['price', 'rmb', 'price_text', 'priceText']),
    ),
    uid: cleanText(
      readObjectValue(data, ['uid', 'mid']) ||
        readObjectValue(userInfo, ['uid', 'mid']),
    ),
    userName:
      cleanText(

...
function extractBilibiliDanmakuAvatarUrl(info) {
  ...
  const face =
    (user && (user.face || user.face_url || user.faceUrl)) || (base && (base.face || base.face_url || base.faceUrl));
...
function appendEmoteMap(records, seen, value) {
  ...
  for (const [key, emote] of Object.entries(value)) {
    ...
    appendEmote(records, seen, key || emote.text || emote.emoji || emote.descript, emote, 'inline');
  }
}

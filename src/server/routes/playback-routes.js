...
const routes = {
  ...
  'POST /api/playback/history/clear': storeRoute(async (context, request) => {
    const body = await request.body();
    return context.playback.clearHistory({
      clientId: clientIdOf(request, body),
    });
  }),
  ...
  'GET /api/playback/queue-state': storeRoute(
    (context, request) =>
      context.playback.getQueueState({ clientId: clientIdOf(request) }) || {
        payload: null,
        updatedAt: '',
      },
  ),
  ...
  'POST /api/playback/queue-state': storeRoute(async (context, request) => {
    const body = await request.body();
    return context.playback.saveQueueState(body.payload, {
      clientId: clientIdOf(request, body),
    });
  }),
  ...
  'GET /api/playback/playlists': storeRoute((context, request) => {
    ...
    if (id) {
      return {
        id: Number(id),
        tracks: context.playback.listPlaylistTracks(id),
      };
    }

...
async function handleOverlayApi(context, principal, request, res) {
  ...
  const reply = (data) =>
    sendJson(res, 200, {
      ok: true,
      data: projectOverlayResponse(principal.scope, pathName, data),
    });
  if (method === 'GET') {
    switch (pathName) {
      ...
      case '/api/state':
        return reply(context.system.getState());
      case '/api/songs':
      ...
      case '/api/overtime/gifts/catalog':
        return reply(context.overtime.getGlobalGiftCatalog());
      case '/api/games/session':
        return reply(context.games.getSession());
      case '/api/games/winner-profile':
        return reply(await context.games.getWinnerProfile());
      case '/api/wheel':
        return reply(context.wheel.getState());
      case '/api/clock/config':
        return reply(getClockConfig(context.settings.get()));
      case '/api/opening/config':
        return reply(getOpeningConfig(context));
  ...
  if (method === 'POST') {
    ...
    switch (pathName) {
      ...
      case '/api/games/session/draw': {
        ...
        return result.accepted
          ? reply({ revision: result.revision })
          : sendJson(res, 400, { ok: false, error: result.reason || '绘画操作无效。' });
      }
      case '/api/wheel/spin':
        return runAction(() => context.wheel.spin(), reply, res);
    }
  }

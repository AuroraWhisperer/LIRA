...
function normalizeSettingValue(key, rawValue) {
  ...
  if (key === 'queueLimit' || key === 'userCooldownSeconds') {
    ...
    if (typeof rawValue === 'string' && !/^\s*[0-9]+\s*$/u.test(rawValue)) return null;
...
function normalizeCloudSettingsSnapshot(input) {
  ...
  const values = {};
  const effectEnabled = normalizeSettingValue('giftEffectDanmakuEnabled', input.giftEffectDanmakuEnabled ?? false);
  ...
  if (Object.prototype.hasOwnProperty.call(input, 'giftBlindBoxCustomConfigV2')) {
    const value = normalizeSettingValue('giftBlindBoxCustomConfigV2', input.giftBlindBoxCustomConfigV2);
    if (value === null) throw new Error('INVALID_GIFT_BLIND_BOX_CONFIG');

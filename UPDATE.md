...
- 📅 **盲盒分析支持日期筛选**：新增单日及日期区间日历，默认今天，区间包含首尾两天；观众排行、盲盒汇总与开盒记录同步筛选，支持跨月选择、取消和返回今天，沿用授权来源隔离。
- 🎁 **整理礼物面板与盲盒主题**：扩充季节盲盒配色，统一最近礼物和盲盒卡片的主题识别；调整礼物状态、开关及布局，减少已同步状态的重复提示。
- 🔗 **集中直播画面与网页歌单地址**：补充互动、礼物展示及三类许愿地址，支持复制和打开；网页歌单地址随当前授权账号更新，拒绝无效地址并避免旧账号响应覆盖新状态。
- 👤 **优化粉丝档案编辑体验**：按基本信息、生日与性格、备注与提醒分组编辑，整理档案布局、日期展示及备份恢复提示，保留既有档案与恢复规则。
- 🎵 **界面与回归维护**：调整点歌工作区布局和全民 K 歌图标，补充日期、盲盒主题、直播地址及粉丝档案回归覆盖，并同步接口说明。

**使用提示**：本次仅发布客户端，不包含服务端部署。盲盒分析按本机日期筛选，首页今日统计及 OBS 的既有统计范围保持不变。

## v5.0.5 变更

- 🔐 **修复账号切换与登录竞争**：登录、退出和云端凭据替换按平台协调执行，取消旧登录窗口并等待保存结束，避免迟到结果覆盖新账号；收紧音乐、B 站、更新和主窗口 IPC 的调用来源检查。B 站凭据继续加密保存，停止明文 Cookie 导出并清理旧文件。
- 🛡️ **加强异常输入与日志保护**：为 AI、音乐、B 站及授权服务响应增加容量检查，完善压缩弹幕、实时事件和 WebSocket 分片处理，减少异常大包和高频消息的资源占用；扩展敏感字段及错误日志脱敏，更新相关依赖。
- ⏱️ **完善加班结算与礼物边界**：随机逐件结算超过单次 100,000 次时保留完整礼物及待结算状态，管理页提示数量超限，避免长时间阻塞；礼物目录超限或读取失败保留上次完整缓存，并补充礼物、粉丝和互动状态的边界校验。
- 🎵 **改善播放与界面生命周期**：限制播放缓存占用，账号切换后阻止旧请求回填；串行发布歌词时间轴并限制等待队列，完善取消、重试和状态顺序处理；修复相关管理设置、礼物展示及异步界面更新问题。
- ✅ **扩充稳定性回归覆盖**：新增真实 Electron 账号竞争、窗口与媒体资源回收、网络容量、消息压力及结算边界测试，同步架构与安全说明，清理退役抓包材料和审计文件。

**使用提示**：本次发布客户端，不包含服务端部署。数量超限的随机逐件礼物会保留为待结算，不会被静默丢弃或截断；旧明文 Cookie 导出开关不再生效。

## v5.0.4 变更

- 🎁 **礼物许愿新增纯文字展示**：每条许愿可选择礼物卡片或纯文字，支持自定义模板并插入礼物名称、已收数量和目标数量，管理预览与本地 OBS 同步展示。旧许愿默认保留卡片，切换样式不重置创建时间和计数，文字按纯文本安全渲染。
- 📖 **扩充图文使用文档**：按上手、功能、配置、网页端、常见问题及参考内容整理手册，补充客户端和网页端操作截图与说明，保留目录、搜索和原文定位入口；新增可复用的隔离示例截图工具。
- 🎛️ **整理管理界面与展示布局**：调整礼物展示、许愿、弹幕回复、投票评分及点歌工作区的布局和控件层级，拆分点歌与桌面样式文件，沿用现有页面入口。
- 🧹 **退役旧引导与闲置资源**：移除旧七步引导弹窗，保留交互式导览及旧设置兼容；清理重复图片和已退役的本地签到、抽签执行代码，保留云端职责边界与旧库只读兼容。
- 🧩 **模块与测试维护**：独立整理历史醒目留言迁移模块，拆分粉丝档案测试和共用夹具，统一代码格式，补充许愿展示、持久化兼容及文档结构回归覆盖，并同步相关架构说明。

**使用提示**：本次仅发布客户端，不包含服务端部署；正式服务器 OBS 的许愿新样式需配套服务端更新。文档中的示例截图用于说明操作位置，不代表真实账号、远程服务或直播端到端验收。

## v5.0.3 变更

- 👤 **修复部分 B 站收藏集头像无法显示**：头像资料、弹幕头像解析和图片代理支持以双斜线开头的地址，并规范化为 HTTPS；没有缩略参数的收藏集原图改取 256×256 WebP 缩略图，避免原图过大导致加载失败。已有缩略参数和普通头像地址保持不变，继续限制可信域名、图片类型和 2 MiB 大小，图片请求不携带登录 Cookie。
- 🎨 **调整礼物横幅蓝色档配色**：改用更鲜明的蓝色渐变，同步用于预览、滚动展示和图片导出。
- ✅ **补充头像回归测试**：覆盖收藏集缩略图、相对协议地址、普通图片格式、文件大小及恶意域名拒绝。

**使用提示**：头像修复作用于客户端处理链路；本次不包含服务端部署。

## v5.0.2 变更

- 🔠 **调整礼物横幅字号**：昵称由 18px 增至 24px，礼物文字由 14px 增至 18px，礼物名称进一步突出；数量由 40px 调整为 28px，并微调文字位置与行高。保留长文字自动缩小及大数量画布扩展。
- 🎨 **调整蓝色金额档底色**：采用新的不透明浅蓝配色，滚动展示、预览和图片导出同步使用。
- ✅ **同步展示回归测试**：更新字号断言，验证长名称适配、节点复用及 PNG/OBS 数量布局。

## v5.0.1 变更

- 🎛️ **投票与评分管理界面重整**：改为分页签切换投票和评分，集中展示本场设置、互动状态及结果；直播画面设置独立展开并支持返回，预览随当前互动类型切换。已有会话时锁定对应类型，支持方向键及 Home/End 切换可用页签。
- 📺 **互动直播画面采用透明底色**：移除整体背景颜色与背景不透明度控件，评分卡片不再显示外边框；继续保留文字、进度条、轨道、整体不透明度等设置。统一颜色选择器和参数滑块样式，整理预览布局。
- 🎁 **礼物许愿展示更精简**：改为礼物图片、已收数量、目标数量及进度条的紧凑横向布局，透明背景，缩小卡片间距。OBS 建议宽度仍为 440，每份心愿约 96 像素高。
- 📝 **许愿说明调整为备注**：备注不在直播画面中可视展示，管理页明确提示；礼物名称与周期等信息保留无障碍描述，相关许愿编辑与来源切换测试同步更新。

**使用提示**：投票与评分的旧背景设置不再影响本地展示，整体不透明度仍可调整。以上为客户端管理与本地 OBS 页面更新，本次不包含服务端部署；投票/评分真实直播间端到端验收仍待补。

## v5.0.0 变更

- 💾 **离线歌单修改支持重启恢复**：歌曲新增、编辑、删除及清空时，在同一事务中保存当前账号的待上传快照。上传失败或退出后继续保留，下次启动恢复并重试；旧上传结果不会清除新修改，切换账号也不会上传其他账号的待传内容。
- 🎵 **修复单曲循环与退出账号时的播放行为**：单曲循环仅在自然播放结束时重复，手动下一首及失败跳过正常推进队列。退出音乐平台时使该平台尚未完成的播放请求失效，避免迟到结果恢复播放，同时保留其他平台的播放与错误恢复。
- 🎁 **本日礼物循环增加最低金额**：支持整数或一位小数，默认 0 不限制；正数按当天同一送礼人、同款礼物合并后的累计金额判断，仅展示严格大于阈值的卡片。保留原始流水、历史筛选和导出行为，并调整蓝色金额档配色。
- 🧩 **整理模块边界与公共逻辑**：管理页逐步改用显式模块依赖，礼物查询与维护操作归位到存储层；复用主题、时间格式、歌词规范化、音乐授权状态、Cookie 细节、B 站错误和敏感字段识别等公共逻辑，保留既有接口及来源隔离。
- ✅ **测试与契约维护**：增加歌单恢复、播放生命周期、存储事务及公共逻辑回归覆盖；测试支持离线、浏览器、桌面、安装器和服务端契约分组，合并重复测试夹具，密码兼容样例使用锁定的服务端契约。

**使用提示**：歌单恢复保护本版本起成功保存的待传修改，不能恢复此前已被覆盖的数据，也不提供多设备冲突合并。礼物最低金额只影响本日循环展示；正式服务器上的相关展示需配套服务端更新。本次仅发布客户端，不包含服务端部署；跨仓库弹幕共享及关联解码器复用仍延期，投票/评分真实直播间端到端验收仍待补。

## v4.2.15 变更

- 🎨 **投票与评分支持自定义直播画面**：新增标题、提示、评分规则、文字与背景颜色、进度条与轨道颜色、背景与整体不透明度、字号、圆角及状态和人数显示设置。草稿即时预览，支持切换投票/评分示例和恢复默认，点击应用后保存并同步本地 OBS 展示。
- 📊 **投票与评分卡片布局优化**：投票选项与票数整合到进度条内，卡片随内容收紧；评分采用独立边框卡片与均分框，规则支持多行文本与自动换行，留空隐藏。收集中保留占位，结束后再显示均分与人数；外观更新保留选项节点、翻页与滚动位置。
- ⭐ **粉丝特别关注置顶**：特别关注的档案优先排列并显示星标，其后继续按大航海身份、灯牌等级等排序。卡片和详情优先显示最新平台昵称，常用称呼单独保留展示。
- 🗂️ **粉丝档案支持曾用名管理**：展示最近三个不同曾用名，可按由近到远手动编辑，并支持按曾用名搜索；可靠的新昵称观察自动更新历史，排除当前昵称和重复项，沿用现有备份格式。
- 🔎 **使用文档搜索定位更清晰**：点击搜索结果后在原文高亮匹配词，定位完成后强化提示；更换或清空搜索时移除旧高亮。
- 🖥️ **管理界面与验证同步**：整理 AI 助手、礼物特效和粉丝档案布局，补充外观校验、设置隔离、昵称历史、关注排序和展示同步测试，更新相关接口与存储文档。

**使用提示**：投票与评分外观保存在本机，不参与云端设置同步；修改评分规则文案仅影响展示，不改变 1–10 分及末次有效评分规则。投票/评分的真实直播间端到端验收仍待补，本次发布不包含服务端部署。

## v4.2.14 变更

- 🎁 **新增礼物许愿**：支持长效、本日、本场三种目标，提供礼物选择、目标与文案编辑及独立 OBS 展示。按当前授权来源的礼物流水统计数量，支持普通礼物、盲盒及大航海；修改目标不重置起算时间，删除许愿不删除流水。
- 📊 **新增弹幕投票与观众评分**：小游戏新增“投票与评分”类别及独立展示页。投票支持自定义选项、倒计时和提前结束，每个账号首票有效；评分支持 1–10 分，结束前可修改，停止后公布平均分与参与人数。
- 🔄 **完善互动会话与实时接入**：仅在活动开始后收集实时弹幕，结束后冻结结果；与进行中的原有互动游戏互斥，切换账号或房间中断旧场。展示页只读，评分收集中不公开均分，改善异步帧顺序及旧连接回调隔离。
- 🎨 **调整礼物展示**：固定排列弹幕礼物卡的最大缩放由 1.8 倍调整为 1.5 倍，微调礼物横幅蓝色配色、舰长头像框与图文间距。
- 🎲 **动态抽奖可先编辑草稿**：未登录或授权暂不可用时仍可填写新抽奖规则，创建和执行继续检查授权状态，已有任务与运行中任务仍保持编辑限制。
- ✅ **测试与契约文档同步**：补充许愿计数、直播窗口、投票评分规则、连接隔离、展示权限及界面同步测试，更新接口、存储、实时消息和 OBS 展示说明。

**使用提示**：本场许愿依赖可确认的直播状态；上游不可用时暂停计数，恢复后重新按窗口统计。投票与评分需客户端实时弹幕鉴权成功，当前已完成自动化与隔离界面验证，真实直播间端到端验收仍待补，尚不能识别所有跨报文重放。新增展示页使用客户端本地地址；正式服务器上的展示改动需配套 LIRA Server 更新，本次发布不包含服务端部署。

## v4.2.13 变更

- 🗂️ **粉丝档案支持每日自动核对大航海身份**：新增默认关闭的自动更新开关，开启后客户端在北京时间 12:10 同步当前名单，错过时在当日启动后补更新；成功或失败均有提示。按账号、房间和日期去重，保留私人资料与历史，失败不写入半份名单。
- 👤 **修复礼物卡片头像缺失与加载失败**：今日礼物优先复用同一 UID 的已知头像，缺失时通过现有 B 站资料查询补查；暂时失败后可在后续刷新恢复，空资料不覆盖已知头像。滚动区单独重试失败图片，导出延长头像等待时间，原始礼物账本和金额不变。
- ⚡ **滚动礼物更新更平稳**：连击时就地更新数量、颜色和身份资料，复用已加载图片与可见卡片；重复通知不再重复改写画面，按需读取设置与素材，继续保留滚动进度和定时对账。
- 🎁 **弹幕礼物卡与横幅展示优化**：五种固定排列样式的礼物卡随可用宽度整体缩放，最大为设计尺寸的 1.8 倍；修正缩放后的高度计量，避免列表裁剪不准。微调礼物横幅配色与排版。
- 🖥️ **礼物与动态抽奖界面整理**：调整礼物页面分区和响应式布局，动态抽奖将操作区与规则说明并排呈现；关闭礼物历史时清理选择状态，完善粉丝档案设置说明。
- ✅ **回归测试与契约文档同步**：补充自动更新时机、账号切换与事务保护、头像查询恢复、滚动节点复用及礼物缩放测试，更新桌面、存储和展示文档。

**使用提示**：每日身份更新需在粉丝档案设置中手动开启，并依赖客户端运行、有效授权及可读取的 B 站名单。头像补查仅用于今日且已确认 UID 的展示资料，未知身份与历史记录不会按昵称推测。正式服务器 OBS 礼物卡缩放需配套 LIRA Server 更新，本次客户端发布不包含服务端部署。

## v4.2.12 变更

- 🔎 **使用文档新增搜索**：支持按多个关键词检索正文及折叠的常见问题，高亮匹配内容并展示摘要；点击结果定位原文并展开对应问题，支持回车打开首条结果、Esc 清空搜索。
- 💬 **弹幕小表情与表情包分别显示**：按 B 站消息来源区分行内小表情与整张表情包，小表情即使单独发送也保持与正文字号一致；整张表情包在原尺寸基础上放大 1.4 倍，保持比例与画布限制。同步本地预览，兼容缺少类型字段的旧消息。
- 🎁 **礼物特效操作更清晰**：调整配置卡片与按钮层级，明确“查找礼物 ID”和“OBS 浏览器源地址”入口；弹幕触发开关集中显示启用状态，分开展示两种触发示例并补充无障碍说明。
- 🎲 **动态抽奖布局优化**：将抽奖记录和新建抽奖入口移到页面顶部，整理账号说明与管理操作，便于查看历史记录和开启下一场抽奖。
- ✅ **回归测试与协议文档同步**：补充表情来源分类、消息缓冲、公开数据投影、渲染和本地预览测试，更新 B 站协议与弹幕展示说明。

**使用提示**：正式服务器 OBS 弹幕中的新表情分类与尺寸需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署。

## v4.2.11 变更

- 🎨 **弹幕姬支持本机字体与正文颜色**：字体选择接入已安装字体列表，置顶推荐字体；九种样式均可单独设置弹幕正文颜色，支持预览、应用与恢复默认，昵称、身份和礼物配色保持原样。背景不透明度统一使用公共滑块控件。
- 🖼️ **礼物导出设置移入预览**：在“最近礼物 → 查看全部 → 导出所选”的预览右侧调整输出方式、背景和保存文件夹，即时应用于本次导出并保存为后续默认设置；保留已选记录与样式快照，导出期间禁用设置并明确反馈保存结果。
- 🖱️ **礼物记录支持点击与框选**：点击记录行即可切换选择，拖动框选当前页记录并支持边缘自动滚动；每页记录增至 100 条，继续支持跨页选择与全部筛选结果导出。
- 📺 **滚动礼物与横幅优化**：滚动速率 1–50 档调整为每行 5–0.1 秒，新配置默认 25 档；预览使用透明背景并精简状态文字，微调礼物图与数量位置、蓝色金额档配色，完善大航海礼物图片识别。
- 🗂️ **粉丝档案管理与布局优化**：新增经确认后清除当前账号全部档案的入口，包含已归档档案、档案内记录及提醒状态；保留原始礼物账本、排除名单、设置与现有恢复点。整理基本资料、话题、约定与记录布局，并调整弹幕姬和 AI 助手界面层级。
- ✅ **测试与文档同步**：补充字体与颜色校验、礼物框选、导出设置、滚动速率及档案清除的回归覆盖，更新使用指南和相关接口、展示文档。

**使用提示**：弹幕本机字体和正文颜色保存到正式 OBS 展示需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署。自定义字体需安装在浏览器源所在电脑；清除全部档案前建议保存完整备份，后续同步仍可重新自动建档。

## v4.2.10 变更

- 🎛️ **弹幕姬支持按样式调参**：九种样式可分别设置预设字体与正文字号，有底色的样式可调整背景不透明度，支持的样式可切换主题插画与 B 站礼物原图。切换样式保留各自草稿，恢复默认只影响当前样式，预览后需显式应用到直播画面。
- 💬 **弹幕设置与预览优化**：将黑名单与屏蔽词移到直播链接下方，新增独立参数区并整合全屏停留时间；本地预览刷新和切换样式时保留配置，礼物原图加载失败时回退为主题插画。
- 🗂️ **粉丝档案排序与同步提示优化**：列表先按总督、提督、舰长及其他身份分组，再按本房间灯牌等级排序；同步大航海名单弹窗展示已获取的房主昵称和头像，并调整档案操作区布局。
- 🎨 **弹幕与礼物配色调整**：加深经典弹幕正文及身份底色，改善船锚标识并补充旧版 OBS 内核的默认配色兼容；更新礼物横幅金额分色，提高底色不透明度。
- ✅ **回归测试与契约文档同步**：补充样式参数校验、配置保存、本地预览、粉丝排序和房主资料快照测试，更新弹幕展示说明及实施记录。

**使用提示**：弹幕样式参数和正式 OBS 礼物原图需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署。旧服务器不支持时会禁用新参数并提示更新；蝴蝶结和流光气泡不提供独立礼物图片选项。

## v4.2.9 变更

- 🎁 **本日礼物卡片合并展示**：滚动礼物和图片导出可将当天同一 UID、同一礼物的记录合并，累计数量与实际金额，并采用当天最新可靠的昵称、头像和大航海身份。导出只合并所选记录，历史日期与原始礼物流水保持不变；身份未知的记录不按昵称合并。
- 📺 **滚动礼物连续循环**：超过可见行数后连续滚动，首尾无停顿衔接，支持 1–50 档速度；刷新保留滚动进度，记录不足时静态展示。移除旧暂停和低功耗选项，保留已有行数与金额分色设置。
- ✨ **礼物横幅进一步精简**：基础宽度由 504 缩至 428 像素，滚动展示、预览和双倍清晰度 PNG 导出同步适配，大数量仍自动延长；完善送礼头像与大航海身份资料传递。
- 🗂️ **粉丝档案身份与编辑优化**：按最近同步的大航海名单显示舰长、提督、总督的姓名颜色和身份，完整名单中缺席的粉丝保留档案并恢复普通展示。新增独立设置页，将编辑表单整理为基本信息、生日与偏好、备注与提醒三个分区，集中同步、备份和档案管理操作。
- 🔄 **修复续期导致档案操作中断**：同一账号后台续期后保留当前档案详情和编辑上下文；真正切换账号、服务器或失去授权时仍使旧上下文失效，过期异步结果不能写入。
- 🖥️ **启动与使用说明优化**：调整管理页初始化顺序与启动显示，精简萌时钟时间轴中的时段标识及更新页面的外部入口，更新使用指南布局和 AI 触发词说明，明确触发词需自定义并保存。

**使用提示**：礼物合并与新增头像／身份资料需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署。服务器不支持且没有同来源、同日缓存时，礼物保持逐条展示；历史缺失资料不会凭空补齐。粉丝身份需手动同步大航海名单，身份展示不推算会员到期日。

## v4.2.8 变更

- 🎁 **礼物姬按功能分区**：百宝箱中的礼物姬新增“礼物边框”“滚动礼物”“图片导出”三个分区。最近礼物保留查询、选择、预览和导出；滚动行数、间隔、金额分色与 OBS 地址统一移到礼物姬管理。
- 🖼️ **导出设置独立保存**：无需先选择礼物即可设置拼图或逐条导出、透明或白色背景及保存文件夹。设置自动保存并用于下一次导出，已创建的预览任务保持原配置；旧版保存文件夹继续生效。
- ✨ **礼物横幅更紧凑**：横幅基础尺寸调整为 504 × 72，更新大航海身份图标与排版，长昵称和礼物名称自动缩小以适应空间；滚动展示、预览和双倍清晰度 PNG 导出同步适配。
- ✅ **交互与回归覆盖同步**：滚动与样式设置支持独立草稿、保存和取消修改，导出预览显示本次配置；补充默认设置持久化、任务配置隔离与原生目录选择权限测试，更新使用指南及接口、存储说明。

**使用提示**：展示和导出设置请到“百宝箱 → 礼物姬”调整；导出记录仍从“礼物 → 最近礼物 → 查看全部”选择。修改导出默认设置只影响之后创建的导出任务。

## v4.2.7 变更

- 💬 **弹幕姬新增显示屏蔽**：支持按 UID 或包含词屏蔽普通弹幕，可从直播间在线榜搜索并勾选观众，也可一键清空屏蔽词；保存状态即时反馈，失败保留输入，切换账号清除旧状态。规则对保存后收到的普通弹幕生效，不影响礼物、点歌、机器人回复或 B 站禁言状态。
- 🗂️ **粉丝档案按灯牌等级排序**：手动同步大航海名单时读取当前房主的粉丝灯牌等级，优先展示等级较高的档案；缺少等级的档案排在后面，同级沿用最近互动排序。精简列表信息与空状态，调整列表和详情高度，恢复导航展开箭头。
- 🎵 **点歌队列恢复简洁展示**：点歌人一行恢复为“昵称 · 来源 · 时间”，移除行内档案按钮；随机点歌及带筛选条件的随机点歌仍正常入队，但不再新增粉丝档案点歌记录，普通点歌继续归档，已有记录保留。
- ⚡ **礼物与粉丝查询提速**：优化最近礼物、历史深页翻页及重复筛选计数，批量读取粉丝候选档案的记录和提醒，并为点歌队列关联补充索引；沿用正式数据库迁移，保留排序、筛选与账号来源隔离。
- 🎁 **透明弹幕样式显示礼物总额**：礼物数量紧随名称，右侧显示本次已结算的人民币总额；同步本地预览和展示数据，兼容缺少金额的旧消息，并调整头像、身份及礼物排版细节。
- 🎛️ **管理界面操作更统一**：统一字号、速度、透明度等参数滑块的外观与聚焦反馈，保留数字精调与正负偏移标记；整理点歌、播放、礼物、AI 和百宝箱的设置文案、状态提示与操作布局。
- 📖 **重整使用指南与回归覆盖**：按实际操作步骤梳理安装、直播连接、点歌、礼物、弹幕姬及常见问题说明；补充显示屏蔽、灯牌同步、查询迁移和礼物金额展示测试，同步存储、桌面与 OBS 契约文档。

**使用提示**：弹幕显示屏蔽及在线弹幕中的礼物金额需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署；在线榜最多读取 150 人，并非完整在线观众名单。已有粉丝档案需在升级后手动同步大航海名单，才能补充灯牌等级。

## v4.2.6 变更

- 🗂️ **粉丝档案支持手动同步大航海名单**：一键读取当前已配置直播间的完整名单，按 UID 预建或更新昵称、头像和大航海等级，保留私人资料与人工修订；重复同步去重，读取失败不写入部分名单，切换账号或房间后取消旧任务。同步不推算会员到期日，也不把名单缺席当作下舰。
- ☁️ **签到与抽签可直接开启**：移除首次启用时的旧数据处理面板，连接云端账号后即可操作开关；累计签到和每日抽签继续保存在云端，支持配套服务器整库备份恢复后继续使用，客户端不再自动读取或上传本机旧数据。
- 👤 **设置页新增直播间资料卡**：并排展示登录账号与已保存直播间的房主资料，将六项常用设置整理为三列两行；修改房间时清除旧资料，避免迟到响应覆盖新房间，查询失败可重试。
- 🎁 **礼物横幅与图片导出优化**：横幅基础宽度调整为 560 像素，优化头像内距、昵称和礼物图排布；多位数量保持字号并向右延展，PNG 按实际宽度的两倍导出，避免裁切。金额分色改为直观的起止区间，相邻边界自动同步，预览与保存沿用同一配置。
- 💬 **百宝箱交互与文案优化**：整理固定回复的说明、编辑入口及欢迎词库布局；修复粉丝档案按钮文字选中后难以辨认、“更多”菜单收起等交互细节。
- 🔧 **修复 Windows 旧实例查询偶发超时**：减少系统 TCP、进程与用户身份查询开销，保留精确端点和归属核验，改善旧实例退出与重新启动的稳定性。
- ✅ **回归测试与文档同步**：补充大航海名单分页与事务导入、账号隔离、房主资料、金额区间和动态导出尺寸测试，同步接口、存储、桌面及 OBS 展示说明。

**使用提示**：签到／抽签直接开启及云端整库恢复需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署。大航海名单需手动同步，手写档案仍保留在本地；礼物 OBS 浏览器源建议宽度调整为 900，以容纳多位数量。

## v4.2.5 变更

- 🗂️ **新增粉丝档案**：百宝箱新增私密档案入口，支持资料与互动记录、生日及会员里程碑提醒、大航海证据和人工基线；成功点歌自动归档，队列可快捷查看档案，支持 CSV 导出、完整备份及冲突确认恢复。手写资料保留在本地，按账号隔离，普通业务流水清理不删除档案。
- ☁️ **签到与抽签改为云端运行**：客户端提供独立开关、状态刷新和一次性旧数据接管，支持核对本机历史、分批导入及中断后继续；移除本地机器人执行与旧词库编辑入口，避免双端重复回复。接管完成前默认关闭，失败保留最后确认状态，旧服务器不回退本地发送。
- 🔐 **OBS 展示与桌面管理权限隔离**：展示页面使用按页面限定的访问凭据，HTTP 与 WebSocket 仅提供该页面所需数据和操作；管理页面凭据由 Electron 可信主窗口请求注入，展示页预览采用沙箱隔离，并补齐重连、重启恢复与跨页面访问限制。
- 🎁 **礼物流水新增金额筛选**：支持按单条礼物记录总金额严格大于指定人民币金额筛选，留空不限；分页、总数、全选及图片导出共用相同条件，并整理筛选栏布局。
- 🕒 **萌时钟与预览细节优化**：调整数字时钟和时间轴样式，数字时钟使用 AM／PM 标识；适配沙箱中的时钟配置预览、开播动画及相关工具预览。
- 🔧 **修复 Windows 旧实例清理超时**：优化系统 TCP 归属查询，减少启动时的查询开销，保留端点、用户身份及进程创建时间核验，避免合法旧实例因查询超时无法正常退出。
- ✅ **回归测试与文档同步**：补充粉丝档案、云端接管、礼物金额筛选、桌面请求鉴权、OBS 数据投影及连接恢复测试，同步接口、存储、桌面与前端架构说明。

**升级提示**：云端签到／抽签及粉丝事实同步需要配套 LIRA Server 更新，本次客户端发布不包含服务端部署。签到／抽签升级后需在客户端明确完成接管并手动开启；未完成接管或服务器不支持时不会由本地补发。粉丝手写资料不会上传，未知会员到期日不估算。自动化验证不代表真实直播、OBS 或云端跨日运行已完成实测。

## v4.2.4 变更

- 🎁 **礼物流水支持筛选与图片导出**：新增日期、用户和礼物组合筛选，支持跨页勾选与选择全部筛选结果；选中记录可预览为统一礼物横幅，通过桌面原生窗口批量保存 PNG，内容较多时自动拆图，并保留头像、大航海身份和金额分色。
- 📺 **新增本日礼物循环展示**：提供独立 `/gift-feed` OBS 页面，按北京时间读取本日礼物并循环展示，支持刷新、跨日切换和展示设置同步；与导出共用横幅样式，历史资料缺失时使用占位，不根据昵称补认身份。
- 👋 **进场欢迎设置升级**：接入服务器新版欢迎配置，支持延迟、荣耀等级条件、分阶段词库与生僻字拼音设置；参数和词库独立编辑保存，整理固定回复区及折叠编辑器，保留账号切换隔离和旧服务器兼容提示。
- 💬 **新增流光气泡弹幕样式**：增加按观众、舰队与主播身份分色的柔光随机气泡，同步本地预览、样式选择和礼物消息展示，并调整奶油气泡及经典样式的视觉细节。
- ▶️ **播放与 OBS 状态恢复更稳定**：修复旧播放快照覆盖新状态、歌词发送端重建后难以恢复，以及队列、歌库、盲盒、游戏和转盘的迟到响应与重连对账问题；WeSing 设置保存成功后统一更新采集状态，失败时保留原配置。
- 🔐 **授权与本机访问保护加固**：授权失败保留原设备密钥，补充激活中断恢复和远端响应读取限制；本机旧实例清理验证实际连接与进程归属，收紧健康检查、HTTP 与 WebSocket 生命周期边界，并防止 AI 密钥随模型地址切换被隐式发送到其他来源。
- 🔧 **连接诊断与数据处理修复**：弹幕自动重连增加有上限的退避；补充从 B站登录、凭据读取到点歌过滤和入队的脱敏日志，修正歌库导入数量校验、礼物消息字段回退、主播游戏身份判定及超量请求错误反馈。
- ✅ **回归测试与文档同步**：补充礼物导出、欢迎配置、授权恢复、播放顺序、连接关闭、日志脱敏和存储边界测试，同步接口、桌面、前端与存储架构说明及实施记录。

**使用提示**：新版进场欢迎、流光气泡服务器展示及礼物头像／身份资料需要配套 LIRA Server 支持；本次客户端发布不包含服务端部署。本日礼物循环依赖客户端本地服务运行，旧礼物记录缺失的展示资料不会自动补齐。真实直播欢迎发送与 OBS 使用效果需在配套环境验证。

## v4.2.3 变更

- 📣 **新增 PK 对手信息播报开关**：弹幕姬固定回复区新增独立开关和状态刷新入口，接入服务器 PK 播报设置；默认关闭，开启后由配套服务器在 PK 开始时播报对手大航海与高能榜概况，退出客户端后服务器继续运行。
- 🔐 **播报状态以服务器确认为准**：同步期间防止重复操作，失败时保留最近确认状态；关闭失败明确提示服务器可能仍在播报。切换账号或离开页面后丢弃旧响应，旧服务器不支持时提示刷新或升级，不在本地补发消息。
- ✅ **配套测试与文档同步**：补充固定接口、认证通路、IPC 输入与响应校验、账号切换和页面释放测试，更新桌面架构说明与模块边界记录；收录仓库文件和 Git 状态审计脚本。

**使用提示**：该功能需要配套 LIRA Server 支持，本次客户端发布不包含服务端部署。榜单可能不完整，金额档位按高能榜贡献值估算；具体取数、统计与发送由服务器负责，客户端隔离测试不代表真实 PK 播报已完成实测。

## v4.2.2 变更

- 👋 **新增进场欢迎设置**：弹幕姬固定回复区新增欢迎开关与可编辑词库，支持增删、修改和保存；开关与词库独立提交，服务器确认后才显示生效，读取或保存期间保留新编辑，切换账号清除旧草稿并忽略迟到响应。
- 🎲 **队列支持一键随机点歌**：在“切歌”前新增“随机点歌”按钮，以当前已登录直播账号调用既有随机点歌规则，遵循暂停、冷却、歌库筛选、重复和队列上限限制；等待期间防止重复点击，不向直播间发送弹幕。
- 💬 **弹幕布局与身份展示优化**：固定排列样式统一跟随浏览器源尺寸、靠左从底部排列；经典样式和头像横卡按可用宽度缩小，高度不再压缩字号。经典样式增加主播绿色与舰队船锚标识，透明文字在头像下显示灯牌等级，头像横卡增加头像色彩背景并改善文字辨识度。
- 🖥️ **客户端界面细节调整**：桌面点歌队列恢复 450px 固定高度；调整弹幕姬标题、图标、边框和编辑区域层级，同步 AI 助手、滚动区域、本地预览与使用说明，并优化头像加载及蝴蝶结样式的表情排版。
- 📦 **安装过程提示更明确**：补充准备安装、备份本地数据、安装和恢复数据的阶段提示，调整保留数据时的等待文案。
- ✅ **配套测试与文档同步**：补充欢迎设置的 IPC 校验、账号切换和草稿保护，以及随机点歌、主播身份、弹幕缩放和渲染回归测试，更新接口与桌面、弹幕架构说明。

**使用提示**：进场欢迎与服务器弹幕展示需要配套 LIRA Server 支持；欢迎消息由服务器识别进场事件并发送，客户端不提供本地发送回退。本次客户端发布不包含服务端部署，隔离测试不代表真实直播进场欢迎已完成实测。

## v4.2.1 变更

- 💬 **弹幕样式与本地预览完善**：整理八套弹幕样式、名称及礼物卡片视觉，补充奶油气泡、本地表情与插画素材；预览可直接在本地打开、切换样式并浏览静态示例，无需等待服务器配置加载。
- 🔗 **OBS 弹幕链接按账号获取**：使用服务器返回的账号专属链接，校验来源与路径；切换账号立即清除旧链接并丢弃迟到响应，同步更新地址页与使用说明。
- 🔐 **授权续期与重试边界加固**：过期令牌不再保持已授权状态，续期保留会话约束；授权、云同步及礼物事件重连遵循服务器重试等待时间，避免重复请求与旧会话干扰。
- 🎁 **礼物历史按稳定账号身份隔离**：本地礼物来源加入服务器主播 ID，避免同名账号重建后读到原账号数据；旧来源数据保留但不自动归入新来源，新来源从已认证服务器重新同步历史。
- 🎵 **保护云端歌库与统一默认字号**：无效云端歌库快照不再被当作空歌库覆盖本地内容；点歌板及队列多套样式的默认正文字号统一为 28，同步调整设置页、预览与回退值。
- ✅ **服务端契约校验与文档同步**：新增固定服务端版本的契约校验与 CI 工作流，补充会话、响应边界、礼物来源隔离和本地预览测试，更新架构及使用文档，并统一 LIRA 英文名称说明。

**升级提示**：账号专属弹幕链接、会话续期及礼物身份隔离需要配套 LIRA Server 支持；OBS 请重新复制客户端提供的服务器链接。首次使用新版礼物来源会重新同步历史，旧来源记录不会自动合并。本次发布不包含服务端部署，本地静态预览与隔离测试不代表生产直播链路已完成实测。

## v4.2.0 变更

- 🖥️ **欢迎页视觉细节调整**：欢迎标题改为“「欢迎来到 LIRA」”，回声环采用线性扩散并逐渐淡出，调整起始透明度与关键帧，让扩散节奏更均匀。
- 🫧 **欢迎动画自动控制**：移除手动播放／暂停按钮及相关状态，保留画面不可见、页面进入后台时自动暂停，以及系统减少动态效果设置的实时响应；离开页面时继续停止动画并释放监听。
- ✅ **回归测试同步**：更新动画入口与自动控制测试，覆盖按钮移除、页面可见性、系统动态效果偏好变化及页面离开清理。

## v4.1.9 变更

- 🖥️ **登录与注册页布局优化**：统一登录、注册及首次准备状态的面板高度，长表单仅在右侧卡片内滚动；欢迎标题改为“欢迎来到 LIRA”，重新绘制唱片、中心标识与波形，并调整浅色、深色主题下的视觉层级。
- ⏸️ **欢迎动画支持暂停与自动节能**：新增播放／暂停按钮，唱片与波形仅在画面可见且页面处于前台时播放；遵循系统减少动态效果设置，保留用户手动暂停状态，离开页面时停止动画并释放监听。
- 🔧 **首次准备的不兼容提示更准确**：礼物目录数据无效或服务端目录协议不受支持时，明确提示联系管理员更新服务端，避免误导为网络问题；保留重试与返回登录入口。
- ✅ **回归测试与样式职责整理**：将欢迎插画与动画样式提取至独立 CSS，授权页基础样式降至无需体积豁免的范围；覆盖样式与脚本加载、登录卡片滚动边界、动画可见性与暂停恢复、系统动态效果偏好、页面离开清理及准备数据不兼容提示。

## v4.1.8 变更

- 💬 **弹幕姬接入服务器配置**：百宝箱与 OBS 地址页统一提供当前账号的服务器弹幕链接，关闭客户端后仍可接收消息；样式与全屏停留时长先编辑、预览，再通过“应用到服务器”保存，读取或保存失败保留草稿，切换账号丢弃旧配置与迟到响应。
- 🫧 **新增奶油气泡弹幕样式**：新增带外置圆形头像、奶油底色、粉色描边及昵称的全屏随机气泡，可通过现有服务器预览与应用流程选择，并沿用停留时长设置。
- 🎁 **礼物分类统一使用服务器结果**：目录、盲盒映射与加班礼物选择明确区分直送礼物、盲盒和盲盒产物；不再根据名称或关系自行推断分类，缺失或矛盾的分类不覆盖有效缓存，礼物身份变化后清除旧分类。
- 🔐 **修复账号切换后的请求串用**：授权请求绑定发起时的账号、设备及会话生命周期，阻止旧请求借用新账号重试、覆盖新账号资料或使新会话失效；同账号正常续期仍可完成原请求。
- 🎵 **修复较大云端歌库无法读回**：歌库响应改为按流读取并使用独立的 8 MiB 上限，解决合法歌库上传成功后因读取上限不足而失败的问题；超过上限及时停止读取，保留其他授权接口原有大小限制。
- 🖥️ **减少重复渲染并保护编辑内容**：旧的状态请求不再覆盖较新的实时更新，按实际变化刷新队列、礼物和设置；礼物画面草稿在同步及保存等待期间保留新输入，复用歌库分类与标签读取，并将部分百宝箱编辑器延后到首次进入时初始化。
- ▶️ **播放状态与存储边界整理**：统一队列、曲目及音乐平台状态的写入、保存和界面通知入口，保留异步播放请求失效处理；拆分礼物投影与统计存储职责，维持既有事务与同步语义。
- ✅ **回归测试与文档同步**：补充账号隔离、歌库容量、状态顺序、草稿保护、播放状态、工具初始化、礼物分类及云端弹幕配置测试，同步架构说明与审查、实施记录。

**升级提示**：云端弹幕配置、奶油气泡样式和显式礼物分类需要配套 LIRA Server 更新；旧目录缺少分类时不会被客户端接受。本次客户端发布不包含服务端部署。OBS 弹幕源请重新复制客户端提供的服务器链接；本地隔离测试不代表生产直播链路已完成实测。

## v4.1.7 变更

- 🎲 **动态抽奖结果更完整**：整理账号管理、抽奖设置、采集进度和本地历史入口；中奖结果独立展示，每页 5 人，包含采集时昵称、UID、主页链接、关注核验和完整参与评论，并保留开奖凭证。旧记录缺少昵称时回退显示 UID，新增昵称存储兼容已有抽奖记录。
- 💬 **恢复身份横卡弹幕样式**：新增独立的身份横卡选项，保留普通观众、舰长、提督和总督四档底色及右侧渐隐头像；同步预览与礼物视觉，调整各固定样式的礼物文字排版及图标。
- 🔔 **通知样式与礼物图片统一**：统一通知底色、文字层级和动作布局，移除关闭按钮，继续通过自动计时或业务状态移除；礼物通知优先展示本地目录中的准确礼物图片，盲盒展示实际产物，大航海使用内置徽章，缺图时显示统一占位。
- 🎁 **盲盒映射展示修正**：官方列表只展示已标为盲盒且具有对应身份核验奖池的条目，显示真实礼物编号，区分同名活动；权益奖池展示名称和价值，目录更新后同步移除已取消盲盒分类的条目。
- 🕒 **萌时钟预览切换更平滑**：调整样式和选项时原位更新预览，避免反复重载页面；完整参数预览跳过重复配置读取，首帧在时间与配置就绪后显示，并支持减少动态效果设置。
- 🖥️ **更新与工具页面整理**：更新页按状态展示检查、下载和重启操作，补充下载进度、已下载大小与速度；整理本地数据及支持入口，增加账号登录链接，并调整弹幕、AI 助手和使用指南布局。
- ✅ **回归测试与文档同步**：补充抽奖结果及迁移、通知生命周期、礼物图片、盲盒目录、弹幕样式、时钟预览和更新页测试，同步接口、存储及前端架构说明。

**使用提示**：盲盒目录分类及排除规则需要配套 LIRA Server 目录更新，本次客户端发布不包含服务端部署。动态抽奖的隔离测试不代表真实 B站登录、来源采集和开奖已完成实测。

## v4.1.6 变更

- 🎁 **礼物页新增云端互动开关**：自动答谢礼物与弹幕查询礼物统计可独立开启，默认等待服务器确认；支持刷新云端状态、保存中反馈与登录失效提示，切换账号丢弃旧状态，未确认成功时不显示已生效。自动答谢覆盖付费及已确认免费礼物，不含醒目留言；统计查询使用已记录的付费礼物。
- 🖥️ **客户端布局与滚动条统一**：整理点歌队列、歌库、礼物页、百宝箱、动态抽奖及登录设置的布局和间距，补齐窄窗口适配；操作区域统一滚动条样式，恢复长列表的滚动提示，增加四个主导航的选中态图标。
- 🎵 **歌词支持更清晰的手动浏览**：全屏歌词在手动滚动后暂停自动跟随，提供恢复跟随按钮，点选歌词或切歌后恢复；桌面歌词在手动浏览时显示滚动提示，并在时间线更新时清理旧浏览状态。
- 📺 **OBS 超长内容自动翻页**：盲盒榜单默认隐藏滚动条，超高内容及画猜、游戏结果区域每 8 秒分段翻页；手动操作后暂停，焦点停留或页面隐藏时不翻页，盲盒仍可通过链接参数恢复滚动条和手动浏览。
- 🔔 **礼物目录更新通知调整**：整理通知的布局和样式，更新通知隐藏关闭按钮，同步修正通知移除时的焦点转移。
- ✅ **配套测试与文档同步**：补充礼物互动控制、IPC 边界、账号切换、自动翻页及歌词和界面回归用例，更新架构说明、功能规格与滚动条审查实施记录。

**使用提示**：云端礼物互动需要配套 LIRA Server 支持，并在登录 B 站、同步后手动开启；服务器运行的互动在退出客户端后仍有效。本次发布不包含服务端部署，客户端模拟测试不代表真实 B 站登录和直播发送已完成实测。

## v4.1.5 变更

- 🎲 **百宝箱接入动态抽奖**：新增链接、中奖人数及点赞、转发、关注作者条件，按主评论 UID 去重并采集所选来源交集；提供暂停、继续、开奖、进度与最近活动结果，随机顺序先持久化，关注核验失败时暂停，确认不符合条件才递补，继续操作不重新随机。
- 🔐 **抽奖专用账号独立登录**：抽奖登录与直播 B站登录分离，按 LIRA 账号隔离会话和加密凭据；支持登录、退出及状态刷新，账号切换使旧操作失效，专用凭据不参与直播或云同步。
- 🎁 **弹幕姬新增礼物消息**：已结算礼物接入现有弹幕流，显示赠送者、礼物名和数量；六套样式各有礼物视觉及预览样本，支持消息去重与快照恢复，不改变普通聊天或礼物结算。
- 🖥️ **礼物管理界面整理**：调整礼物工作台、检测区、冲刺与盲盒播报的布局、间距和操作层级，同步相关响应式样式。
- 📦 **覆盖安装可自动关闭旧版**：交互安装先提示中断播放和直播互动，经确认后请求 LIRA 正常退出；等待进程退出后才继续处理数据，超时提供重试或取消，不强制结束进程。
- 📝 **配套测试与文档更新**：补充抽奖独立认证、桌面退出、安装器和弹幕礼物回归用例，同步接口、存储、桌面与前端架构说明及实施记录。

**使用提示**：动态抽奖为当前简化客户端流程，真实 B站来源、作者登录和粉丝关系尚未实测，不代表完整抽奖方案验收完成；视频点赞/转发名单不可可靠获取时会明确拒绝对应条件。多奖项、领奖、导出和独立公示网页尚未交付。点赞/转发名单与关注关系取采集或核验时状态，不还原历史关系。

## v4.1.4 变更

- 🎁 **弹幕代码触发礼物特效**：礼物特效页新增开关，支持 `礼物特效 代码`、`🎁 代码` 两种指令，与手动测试播放共用素材解析和官方透明特效播放器；补齐串行播放、等待过期、断线清理及停用或切换账号后的迟到结果拦截。
- 🔔 **通知反馈与交互修复**：进度和结果通知可原位更新，修复重复提示、旧状态覆盖、关闭计时和通知堆叠问题；补强盲盒、播放器、更新下载、AI 测试、歌库导入及账号操作的反馈准确性，增加可操作的关闭按钮、悬停暂停、长文换行与减少动态效果支持。
- 🎵 **歌库与点歌工作台完善**：重新整理歌库、导入导出页面布局和操作层级；修复 SC 复制未绑定及重复绑定问题，歌曲和 SC 复制统一提供失败反馈，表单校验就地显示并定位问题字段。
- 🖥️ **注册、使用指南与 OBS 样式调整**：授权页默认展示注册入口，明确旧设备授权已撤销时的重新登录提示；整理使用指南章节与导航，调整登录页、弹幕姬、萌时钟及管理界面的样式和说明。
- 📝 **减少高频日志并限制体积**：AI 成功请求改为定期元数据摘要，失败单独保存安全错误信息，不再记录请求和响应正文；停止逐礼物展示及歌词渲染的常规日志落盘，终端仅镜像警告和错误，补齐 UTF-8 字节、写入队列及单文件上限，并保留既有日志内容。
- 🧩 **动态抽奖基础模块准备**：加入动态链接与会话校验、读取适配器、独立数据库、持久请求预算、串行调度和截止后采集核心；当前尚未开放操作界面，完整抽奖与公示流程仍在开发中。
- ✅ **回归验证与文档同步**：补充通知生命周期、业务反馈、特效播放、日志边界及动态抽奖基础模块测试，同步架构、规格与实施记录。

**使用提示**：弹幕礼物特效需要配套 LIRA Server 支持并开启对应开关；本次客户端发布不包含服务端部署。动态抽奖尚未作为可用功能开放。

## v4.1.3 变更

- 🎁 **修复首次准备时礼物目录下载失败**：取消公共礼物目录的固定响应大小限制，支持持续增长的礼物变体和特效元数据；保留请求超时、固定来源和目录结构校验，其他接口保持原有上限并按 UTF-8 字节计算响应大小。
- 🔄 **准备失败后可返回登录并继续**：新增“返回登录”入口，保留本机授权并清空密码和授权码输入；点击“继续准备”可直接重试资源准备，无需重新激活。返回登录或授权失效后，迟到的进度不会覆盖当前页面，并防止重复发起准备请求。
- 🖥️ **登录与注册切换更清晰**：切换按钮增加边框、悬停反馈与实色选中状态，便于辨认当前模式。
- ✅ **回归验证与文档同步**：补充超过 8 MiB 的目录响应、其他接口上限保留、准备失败恢复及迟到进度处理测试，更新桌面授权与首次准备流程说明；独立整理登录与注册切换样式。

## v4.1.2 变更

- 🔐 **登录与设备授权入口明确区分**：登录页分为“登录已有账号”和“注册新账号”，分别提示管理员下发的短效登录码与注册激活码；补充同机重装恢复原授权、重复用户名、密码要求和历史设备记录冲突的反馈，移除客户端自行生成新设备授权码的入口。
- ☁️ **修复切换账号时直播间串用**：云同步按服务器、账号和主播身份确认本地直播间归属，新账号不再自动继承上一账号的房间；同一账号重启或临时授权中断保留原有同步行为，身份缺失或归属保存失败时阻止上传。
- 🎁 **礼物处理统一使用服务器结果**：移除客户端原始礼物检测、连击归并、金额推算和旧记录修复链路，统计、加班与礼物画面统一消费服务器处理后的事件；保留本地弹幕连接及用户姓名、头像和舰队身份采集，并完善礼物目录契约、图片缓存和同步恢复的模块边界。
- 💾 **安装目录内的数据按用途整理**：业务数据继续保存在 `data/`，浏览器会话移至 `data/browser/`，音乐和礼物缓存移至 `data/cache/`，日志与更新缓存仍使用安装目录下的 `logs/`、`updates/`；已知旧目录自动迁移并记录进度，支持中断后继续，遇到目标冲突停止而不覆盖文件。
- 🗑️ **卸载时可选择是否删除用户数据**：普通卸载默认保留用户数据并清理日志与更新下载；“同时删除用户数据”默认不勾选，勾选后还需再次确认，确认后清理当前安装及已知旧版用户目录中的数据，保留备份和云端资料。静默卸载保留用户数据，覆盖升级保留全部运行目录，并补强目录链接与删除失败保护。
- 🎵 **歌库导入与显示校验补强**：完善更新导入的行数、字段类型、启用值及重复数据校验，补强预览与提交一致性；点歌价格输入增加保存前有效性检查，手动输入时重置快捷选项，拆分导入解析、云端歌库同步和背景设置模块。
- 🖥️ **歌词、弹幕与管理界面整理**：桌面歌词预览和 OBS 歌词共用渲染模块，按设置用途拆分页面；整理弹幕消息渲染、礼物历史、主播工作台及播放器、萌时钟、小游戏等样式与界面模块，保留既有入口和配置行为。
- ✅ **模块治理与回归验证完善**：新增模块体积基线和增量检查，按功能拆分大型测试，优化 JavaScript 检查执行，并同步架构、规格、数据迁移和发布验证文档。

**升级提示**：登录码签发、同机授权恢复及礼物处理需要配套的 LIRA Server 支持；本次客户端发布不包含服务端部署。客户端不再提供本地礼物检测回退。首次升级会整理已知旧数据目录，发生目录冲突时请按错误提示处理后重新启动。

## v4.1.1 变更

- 🎵 **歌库支持预览后更新导入**：新增显式更新模式，按歌名和歌手匹配已有歌曲，预览新增、更新、未变化、冲突和无效数据及逐字段差异；缺列与空值默认保留原内容，可明确选择清空空值。提交前重新校验预览，整批事务写入并保留原歌曲 ID，不删除无关歌曲；默认仅新增模式继续跳过重复歌曲。
- 📝 **点歌价格与歌切编辑完善**：歌库支持价格快捷填写、实时预览、长度提示和歌切编辑，价格与歌切保留内部空格和换行；导入兼容更多价格列名并提示冲突，更新五行示例模板，导出保留已保存的核对平台。
- 🎁 **礼物身份与加班规则精确绑定**：礼物目录、图片和规则区分复用同一 Bilibili ID 的不同名称或标价，支持分别选择与配置；加班按事件中冻结的完整礼物身份匹配，避免新礼物误触发旧规则，历史结算不重新计算。
- 🖼️ **礼物图片与盲盒状态展示修正**：礼物选择器、加班规则、OBS 加班画面、最近收礼和盲盒映射统一使用缺图占位，避免串用其他礼物身份的图片；盲盒映射页展示官方映射及自定义配置的实际应用状态，完善等待服务器应用时的提示。
- 💾 **数据、日志与更新缓存保存在安装目录**：正式客户端统一使用安装目录下的 `data/`、`logs/` 和 `updates/`，兼容迁入已有 AppData 数据；覆盖安装先确认旧程序退出，再备份并恢复数据，失败保留源数据或恢复备份并提示位置，新版卸载程序保留这些目录。首次安装有 D 盘时优先推荐 `D:\LIRA`，仍可自行选择其他位置。
- 🛠️ **安装诊断与使用指南补充**：安装数据处理显示进度和具体错误，补充 Windows 安装诊断采集工具；使用指南增加英文路径建议和卸载程序位置说明。
- ✅ **规格、架构与回归覆盖同步**：补充歌库导入、资料编辑、礼物身份、图片回退、数据迁移、安装目录、诊断和更新缓存测试，并同步相关规范与架构说明。

**升级提示**：已有数字 ID 加班礼物规则会保留时间、数量及生效设置，但需要重新选择对应礼物完成身份绑定后才会触发。完整礼物身份和官方盲盒映射需配合支持相应协议的 LIRA Server；客户端发布本身不包含服务端部署。

## v4.1.0 变更

- 🖥️ **登录与使用指南更新**：重新整理登录页布局、首次准备流程和使用指南，统一相关入口及提示；修正已有账号密码的客户端兼容性，激活功能未就绪时阻止表单提交。
- 🎁 **礼物与盲盒展示修正**：礼物历史补强请求失效、分页和失败恢复；最近礼物卡片使用权威盈亏并区分未知金额与零值，修正盲盒亏损负号；加班礼物选择器区分普通礼物、盲盒本体和盲盒产物。
- 🎵 **播放与歌词稳定性提升**：修复清空队列、切歌和切换音质时迟到请求覆盖播放状态的问题，补强播放流与退出落盘恢复；修正网易云“我喜欢”歌单删除、QQ 加密音频流取消与背压、全民 K 歌采集生命周期和歌词动画降级。
- 💾 **清空数据与加班状态一致性修复**：清空期间暂停相关写入与礼物检测，避免旧异步任务恢复已删除状态；加班操作在保存成功后更新内存和广播，补强恢复调度、初始化失败清理及设置迁移事务。
- 🔌 **会话与运行时恢复完善**：本地后端重启后，OBS 页面在确认会话失效时刷新恢复；修复异常 WebSocket 升级请求、服务退出排空、桌面关闭流程和音乐平台请求头策略。
- 🤖 **AI 上下文隔离与顺序修复**：回答缓存纳入用户、上下文和配置，避免不同观众间串用回答；补强同用户并发回复的上下文顺序及失败恢复。
- 🛠️ **导入、安装与发布可靠性提升**：补强歌曲文件及 XLSX 解析边界，修正 Windows 卸载命令解析并补齐窗口图标打包；发布前校验工作树与标签归属，完善发布日志脱敏和附件完整性验证。
- ✅ **文档与回归覆盖同步**：更新相关架构、使用说明与修复记录，补充数据维护、播放、礼物、授权、生命周期和发布流程的回归测试。

## v4.0.19 变更

- 🗑️ **礼物流水支持本地与服务器联动清空**：管理端清理礼物数据库时会先删除当前授权账号在服务器上的礼物流水，再清理本地投影，避免远端历史记录重新同步回来；确认弹窗明确提示永久删除及多设备影响。
- 🛡️ **远端优先与失败保护**：服务器清理不可用或失败时保留本地数据；远端成功但本地清理失败时返回部分失败并启动投影重建，授权响应和删除计数继续经过主进程边界校验。
- 🎁 **礼物事件识别修复**：排除 `GIFT_STAR_PROCESS` 与 `WIDGET_GIFT_STAR_PROCESS` 等进度类指令，避免它们被当作普通礼物事件进入采集链路。
- ✅ **协议、架构与回归验证同步**：补充远端请求、授权校验、清理顺序、失败恢复、界面文案和礼物指令过滤测试，并更新礼物同步及前后端架构文档。

## v4.0.18 变更

- 🎁 **礼物 SSE 即时交接修复**：远端客户端完成严格网络字段校验后，控制器改为接收规范化礼物事件，避免派生金额字段被误判为非法，恢复连续终态礼物的即时本地投影与进度事件处理。
- 🔄 **静默连接自动追平**：礼物同步进入正常或旧版兼容状态后，每 10 秒通过既有串行游标流程检查一次新终态事件，覆盖 SSE 保持连接但未送达通知的情况，并在请求完成后重新计时，避免并发补拉。
- 🛡️ **同步生命周期保护**：定时追平继续受来源、授权、控制器和投影代际围栏约束；停止、销毁、重启、授权切换或错误退避都会取消过期回调，断线、乱序和游标缺口仍由原恢复流程处理。
- ✅ **架构与回归验证同步**：补充真实 SSE 解析边界、隐私字段拒绝、静默恢复、游标推进、慢请求防重叠、旧版模式、错误退避及生命周期失效测试，并更新桌面同步事实文档。

## v4.0.17 变更

- 🎁 **礼物历史抽屉布局优化**：扩大历史抽屉宽度并固定六列比例，让时间、礼物、数量、金额、用户和备注更稳定地对齐；长备注和盲盒标签支持自然换行，减少内容截断。
- ↕️ **表头三态排序**：时间、礼物、金额和备注表头按“升序、降序、恢复默认”循环，初始状态不再显示已排序；鼠标点击与键盘操作保持一致，并避免默认排序参数重复进入请求。
- 💰 **礼物金额显示统一**：历史金额及盲盒盈亏统一保留一位小数，零值明确显示为 `¥0.0`，提升表格数字可读性。
- ✅ **界面回归验证补充**：新增抽屉尺寸、列宽、备注换行、三态排序、默认请求及金额格式测试，同时保留既有内容转义和来源隔离断言。

## v4.0.16 变更

- 🎁 **礼物历史恢复 3.x 排序体验**：礼物历史支持按时间、礼物、数量、金额、用户和备注排序，服务端统一校验排序字段与方向，分页和筛选继续保持稳定的键集行为。
- 💾 **桌面用户数据目录固定化**：Electron 将用户数据、日志、缓存和本地持久化统一放入稳定的应用数据目录，安装包升级或启动位置变化时不再依赖当前工作目录，并保留已有数据迁移与退出时的落盘顺序。
- ⚡ **礼物查询与实时链路优化**：补强礼物查询服务、存储层和路由的排序/分页边界；连续 SSE 终态事件可即时投影，仍由游标补拉负责断线、乱序与缺口恢复。
- ✅ **架构与回归验证同步**：更新构建、存储、Electron、礼物协议和架构文档，新增桌面用户数据、礼物查询、排序恢复及路由回归测试。

## v4.0.15 变更

- 🎵 **云歌库点歌价格兼容**：同步时保留 `免费`、`舰长`、`30元SC` 等自由文本点歌价格，并继续兼容旧版数字和空值，避免描述性价格被错误转换或丢弃。
- 🧭 **歌库同步错误定位**：授权客户端与 IPC 仅透传经过边界校验的歌曲序号；遇到无效歌曲字段时，管理端会直接提示具体第几首歌曲，便于检查点歌价格、启用状态或排序。
- 🎁 **云端礼物即时投影**：在礼物同步已完成初始化、SSE epoch 已验证且游标连续时，终态礼物事件可立即进入幂等本地投影，无需等待游标补拉；断线、乱序、缺口和重建场景仍由拉取流程安全恢复。
- ✅ **规格与回归验证同步**：更新礼物账本、服务端权威礼物检测及桌面架构说明，补充自由文本价格、错误序号过滤、即时投影、失败回退和游标缺口测试。

## v4.0.14 变更

- 🔐 **Bilibili 云端采集登录门禁**：云端仅在当前 LIRA 账号已保存 Bilibili 凭据、直播间已配置并启用时启动监听，未登录不再匿名采集弹幕和礼物；本机登录状态与云端同步状态明确区分。
- 🧭 **账号与同步状态隔离**：云同步按远端来源和 LIRA 账号绑定，切换账号时丢弃旧账号待上传状态与版本基线，阻止迟到响应污染新账号；新账号不会继承未归属的本地 Cookie，同账号临时授权中断仍保留重试。
- 🎲 **盲盒在售目录刷新修正**：目录按已登录状态和当前直播间刷新，串行处理房间切换、忽略过期或错误房间响应，并在目录更新后通知相关界面；无登录或无房间时保持本地配置排序。
- 🖥️ **管理端提示与回归覆盖完善**：更新 Bilibili 登录帮助、状态和退出提示，补充礼物姬入口说明及盲盒、账号隔离、登录门禁和启动 wiring 测试，同步云端主播同步规格。

## v4.0.13 变更

- 📖 **使用指南布局优化**：目录改为自适应网格，按可用空间换行并动态计算章节滚动偏移；快速上手内容整理为四个清晰条目，精简重复说明与操作入口文案。
- 🎵 **点歌工作台视觉收敛**：队列面板统一使用工作台设计令牌，标题与数量归组，清空/切歌操作补充图标并规范按钮、空状态和焦点反馈，移除不必要的悬浮和渐变装饰。
- 🔌 **直播间连接状态修正**：未填写直播间号时单独提示“未设置直播间”，与已配置但关闭 Bilibili 监听的状态区分，避免连接状态歧义。
- ✅ **回归验证同步**：补充使用指南目录尺寸变化、窄窗口章节定位、队列面板布局及直播间状态边界测试。

## v4.0.12 变更

- 🎁 **礼物流水恢复精简布局**：查看全部礼物恢复 3.x 的六列表格与底部分页，固定读取全部历史；移除名称搜索、日期范围和独立同步栏，异常同步状态仅在标题旁提示，“清理显示”不会删除数据库记录。
- 🎲 **盲盒编辑与目录识别修正**：高级编辑入口始终可见并保留既有展开状态与草稿；修正同时带房间信息和盲盒关系的官方目录快照被误识别为在售目录的问题。
- ⏱️ **加班礼物展示简化**：礼物库统一使用简洁命名，礼物选项和既有规则不再显示或高亮“目录中/当前未在售”状态，下架规则仍完整保留并可继续编辑。
- 🕒 **萌时钟与弹幕预览优化**：萌时钟设置页改为更清晰的预览、固定网址和参数布局，放大预览区域并精简重复说明；弹幕预览同步增高，固定弹幕在拥挤画面中不再被压缩。
- 🔐 **首次准备流程精简**：首次激活后的资源准备页聚焦总进度、完成状态与重试操作，隐藏内部礼物目录、图片数量和当前处理项等实现细节，并统一错误提示。
- ✅ **文档与回归验证同步**：更新礼物流水和加班礼物规格，补充全量历史、清理显示、异步请求失效、盲盒编辑、目录快照及界面精简测试。

## v4.0.11 变更

- 📅 **主播工作台升级**：将新手提词模板重构为月历日程、可编辑置顶备忘和紧凑待办，支持日程增删改、备忘转待办、完成筛选及本地持久化；首次读取时保守迁移 v1/v2 的自定义内容。
- 🕒 **透明数显时钟新增**：新增白色粗窄数字的透明时钟样式，上排显示日期与英文星期、下排显示同字号时分秒，并修正横向时间轴分隔线的居中对齐。
- 💬 **弹幕姬样式与预览优化**：重排固定位置与全屏随机样式选择器，加入更直接的实时预览；“航海铭牌”更新为统一蝴蝶结分隔的居中样式，并精简保存反馈与相关界面文案。
- 🎁 **加班礼物选择器完善**：无需预填关键词即可加载本地完整礼物库，并在前端按名称或 ID 筛选；支持返回在售目录，细分加载、未缓存、空目录和无匹配状态，同时按需加载礼物图片。
- 🔄 **实时链路与并发稳定性修正**：云端歌库更新会刷新管理端与点歌板，歌词版本只接纳和派发一次；Bilibili 客户端替换会取消过期重连，弹幕发送按完整消息串行排队，避免分块交错与并发越过限频。
- ✅ **界面、文档与回归验证同步**：精简账户信息页并统一部分表单间距，更新时钟、弹幕、工作台、礼物与实时链路架构说明，补充迁移、筛选、取消、排序和通知边界测试。

## v4.0.10 变更

- 🎁 **礼物流水界面精简**：最近礼物的“查看全部”恢复为时间、礼物、数量、金额、用户和备注六列流水表，保留名称搜索、时间范围、翻页及同步状态；统计摘要、排行和趋势继续由服务器网页承载。
- 🎲 **盲盒映射展示优化**：当前房间在售盲盒优先排列，无自定义内容时隐藏高级编辑入口，并调整映射卡片的视觉层次；目录更新后会同步刷新在售排序。
- 🔐 **激活密码输入加固**：客户端要求 8–64 位半角字符并同时包含大写字母、小写字母、数字和特殊符号，输入时阻止中文、空格、控制字符及其他非半角字符，并提供明确的首项错误和规则提示。
- ✅ **文档与回归验证同步**：更新礼物历史职责、密码规则和使用指南，补充礼物面板、流水交互及密码输入边界测试。

## v4.0.9 变更

- 🔐 **登录密码体验完善**：新增密码显示/隐藏控制，激活成功后自动清空并恢复隐藏状态；同步新版密码规则提示及细分错误文案，同时保留已有账号旧密码的登录兼容性。
- 🪟 **登录窗口控制优化**：窗口按钮改用本地矢量图标，最大化时自动切换为还原状态，并完善悬停、按下、键盘焦点和无障碍标签反馈。
- 📖 **使用指南与回归验证同步**：更新首次激活的密码要求说明，补充密码交互、窗口控制、本地资源和错误状态测试。

## v4.0.8 变更

- 🎁 **盲盒关系与检测结果升级**：礼物目录扩展为按 ID 维护的官方盲盒关系，区分官方只读映射与主播自定义映射；远程检测结果新增稳定的盲盒 ID，并贯穿本地投影、历史与恢复流程。
- 🔄 **礼物目录初始化与后台更新**：首次授权后自动初始化金瓜子礼物资料及图片，后续在启动和运行期间增量检查；支持缺图修复、旧图回退、更新进度提示及无需远程搜索的本地目录消费。
- 🖼️ **礼物素材与安装包瘦身**：移除内置 Bilibili 礼物清单、图片树及背包同步脚本，房间礼物、最近礼物和加班规则统一按精确 ID 使用本地缓存；覆盖层素材改用 WebP，开播示例媒体不再随安装包分发。
- 🧰 **盲盒、加班与覆盖层体验完善**：官方映射保持只读，自定义草稿和迁移状态得到保留；心动与幸运盲盒延续原主题，其余盲盒使用统一主题，并同步修正图片、盈亏、加班和覆盖层展示。
- 🎵 **播放、歌库与运行时稳定性修正**：避免异步播放请求覆盖较新的选择，抽取共享覆盖层连接逻辑，将歌曲持久化职责收敛到存储层，并补强筛选、会话销毁、云同步停止及设置原子更新行为。
- 🔐 **授权与启动流程加固**：授权完成后的首次目录初始化由 Electron 主进程统一控制，完善远程授权重试、构建完整性校验、敏感输入清理以及初始化失败后的安全重试路径。
- 📦 **打包与发布可靠性增强**：Playwright 保持为开发依赖，安装包清除 Electron 默认示例应用并排除已迁移素材；发布脚本新增干净工作树、tag/HEAD 一致性及远端产物大小和 SHA-256 完整性校验。
- ✅ **回归验证与架构资料同步**：补充盲盒目录、礼物缓存、授权、云同步、播放、存储、覆盖层及发布流程测试，并同步相关规格、ADR 与架构文档。

## v4.0.7 变更

- 🎁 **礼物账本完整投影**：新增按来源隔离的本地礼物历史、增量同步、断点恢复和事务化状态，支持完整的付费最终礼物记录离线查询。
- 🔐 **主播来源与授权切换加固**：同步流程增加来源分区、授权代次和过期工作围栏，切换主播或重建投影时不会短暂暴露或写入旧来源数据。
- 📊 **礼物历史与统计增强**：新增字面搜索、组合键分页、时间范围统计、榜单与趋势数据，并明确同步状态及不完整数据提示。
- 🧹 **存储维护与清理修正**：礼物投影清空、重建、保留期和数据库维护改为事务化处理，移除旧调试礼物页面及无效路由边界。
- 🖥️ **管理端与覆盖层同步**：更新礼物历史、礼物审核、弹幕和相关管理界面，补充远程礼物同步、授权、存储和 UI 回归覆盖。
- ✅ **架构与规格资料同步**：新增来源分区礼物账本 ADR、设计规格和实施计划，更新 API、存储、Electron、Overlay 与发布相关文档。

## v4.0.6 变更

- ☁️ **云同步竞态修复**：为设置、歌库和 Bilibili 状态增加本地变更代次保护，避免上传期间的新修改被清除，或远端旧快照覆盖本地新状态。
- 🎁 **盲盒配置安全性增强**：空配置统一保存为 `[]` 并在重启后保持为空，设置接口复用既有规范化校验，拒绝格式错误、无有效产出及舍入后为零的价格。
- 💰 **礼物金额边界修正**：要求为正的礼物总价在规范化为两位小数后仍须大于零，避免极小金额以零值进入本地账本。
- 🛑 **桌面端退出流程加固**：退出时先停止礼物与云同步入口，等待两个控制器排空后再关闭运行时与存储，降低关机阶段丢失在途状态的风险。
- 🧹 **前端残留清理**：移除无引用的弹窗片段、样式和旧辅助函数，保留当前共享确认框、点歌板和播放状态行为。
- ✅ **回归验证与文档同步**：补充云同步、盲盒、礼物金额和 Electron 生命周期测试，并同步相关架构、规格与实施记录。

## v4.0.5 变更

- ☁️ **云端权威主播同步**：同步主播设置、Bilibili 登录状态和歌曲库，让桌面端与主播端保持同一云端租户状态。
- 🎁 **服务端礼物检测与目录增强**：将礼物解析、连击收尾和重复抑制收敛到服务端，并支持在售礼物刷新、盲盒/背包读取及远程礼物目录搜索与图片缓存。
- 💬 **全屏随机弹幕**：新增全屏随机弹幕样式和可配置显示时长，保留原有弹幕样式与身份展示行为。
- 🔐 **设备授权与运行稳定性加固**：完善授权会话恢复、远程请求、Bilibili 账号资料及 Electron 生命周期处理，强化错误收敛和安全边界。
- 🧰 **管理端与覆盖层体验优化**：调整弹幕、加班姬、设置、工具箱及相关覆盖层布局、交互和文案。
- ✅ **回归验证与架构资料同步**：补充云端同步、礼物、授权、弹幕和管理页测试，并同步 API、架构与功能设计文档。

## v4.0.4 变更

- ☁️ **云端权威主播同步**：完善 Electron 与服务端运行时之间的主播、歌曲、设置及 Bilibili 凭据同步，保持主播私有数据隔离和运行时状态一致。
- 🎵 **歌曲删除与替换安全性**：增加歌曲删除、云端替换的事务保护与历史记录处理，避免同步过程造成数据丢失或状态错乱。
- 🎁 **礼物事件稳定性**：修复 Bilibili V2 礼物组合与重复事件处理，完善礼物捕获回归覆盖。
- 💬 **弹幕工具与覆盖层调整**：优化弹幕工具页、弹幕覆盖层及工具箱侧栏相关布局和展示行为。
- ✅ **回归验证补充**：新增云端运行时同步、云端同步控制器和歌曲删除测试，并同步相关设计与实施文档。

## v4.0.3 变更

- 🧩 **管理页片段组合修复**：修正管理页、点歌页和工具箱页 HTML 片段的边界，避免片段重复关闭文档结构或留下未闭合容器，确保组合后的页面结构完整。
- ✅ **管理页结构回归覆盖**：补充片段边界、页面顺序及唯一 `main`/`body`/`html` 结构断言。

## v4.0.2 变更

- 🔐 **设备激活修复**：统一客户端与服务器对 SPKI 公钥 PEM 外层空白的规范化，修复 Node.js 生成的末尾换行导致激活码被判定为错误的问题。
...
- 🔐 **设备授权与授权门禁**：新增 LIRA 设备激活、设备绑定、授权状态恢复、会话续期和授权失效处理，未授权客户端限制管理页、API 与 WebSocket 访问。
- 🛡️ **本机设备安全校验**：使用本机密钥、硬件指纹、挑战响应和构建完整性信息验证设备，保护授权状态与远程授权通信。
- 🧰 **管理端账号与设备管理**：新增账号、当前设备资料及新设备授权码管理，支持生成、查看、刷新和撤销授权码。
- 🧪 **授权流程回归覆盖**：补充授权协议、授权状态、设备指纹、Electron IPC、服务端门禁和授权页面测试。

## v3.7.11 变更

- 🎁 **礼物特效底部信息板优化**：调整信息板位置、尺寸和响应式字号，使礼物说明在不同画面比例下保持稳定锚定。
- 🎬 **礼物特效动画细节修正**：优化信息板进出场动画，避免位移与布局尺寸冲突。
- ✅ **覆盖层回归测试同步**：补充礼物特效信息板布局与动画约束测试。

## v3.7.10 变更

- 🎁 **礼物特效覆盖层重构**：简化礼物边框合成结构、优化资源加载和覆盖层展示效果，更新对应样式与回归测试。
- 🎬 **开播动画预览同步**：调整开播动画配置处理，音量变化可即时同步到预览画面，并完善启动状态与设置初始化。
- 🧭 **主导航与工具箱视觉整理**：优化主导航活动态、工具箱分组和管理页工作区的细节表现。
- ✅ **覆盖层、管理页与配置测试同步**：补充礼物特效、开播动画及管理页结构测试。

## v3.7.9 变更

- 💬 **弹幕守护身份与展示增强**：新增守护身份资源，优化弹幕守护样式、数据处理、管理页工具和覆盖层展示。
- 🎬 **开播画面角色上传**：支持开播画面角色资源上传与安全校验，完善上传接口、设置持久化和覆盖层加载。
- 🎮 **覆盖层与工具箱细节修正**：调整游戏、开播画面、弹幕和相关工具页的布局、状态与导航细节。
- 🧭 **页面地址与文档同步**：更新浏览器源地址、架构文档和规格索引，保持前后端契约一致。
- ✅ **回归测试补充**：覆盖弹幕守护、开播画面上传、游戏覆盖层及管理页交互。

## v3.7.8 变更

- 🧭 **主导航与工具箱体验优化**：完善工具箱侧栏状态、主导航、使用指南、AI 助手和多个工具页的布局与交互。
- 🕒 **覆盖层视觉与稳定性增强**：更新时钟、弹幕和游戏覆盖层的样式、尺寸、状态展示及资源加载表现。
- ⏱️ **加班与礼物流程修正**：优化加班工具、礼物售卖目录和相关服务路由，补充礼物刷新与状态处理。
- 🎵 **歌库与点歌细节完善**：调整歌库筛选、歌曲操作和队列相关界面，改善桌面端操作反馈。
- ✅ **架构文档与回归测试同步**：更新前后端接口、存储、页面和覆盖层文档，并补充工具箱、覆盖层、礼物和加班测试。

## v3.7.7 变更

- 🎁 **Bilibili 礼物目录与背包同步**：扩展礼物目录、价格映射、盲盒配置和本地资源，新增背包礼物同步脚本及相关测试。
- 🎮 **游戏会话与覆盖层增强**：完善游戏路由、会话状态、游戏覆盖层展示及管理页工具箱入口。
- 🧰 **工具箱与管理页整理**：优化工具箱侧栏持久化、时钟、弹幕、礼物特效、开播画面、加班和使用指南的布局与交互。
- 🎵 **歌曲导入与点歌细节修正**：补充导入字段处理、歌库筛选及点歌板相关界面细节。
- ✅ **接口、架构与回归测试同步**：更新后端 API/服务文档，并补充礼物、游戏、覆盖层、工具箱和点歌流程测试。

## v3.7.6 变更

- 🎁 **礼物目录与映射补充**：更新 Bilibili 礼物目录、价格区间映射和新增礼物图片资源。
- 🖥️ **管理页界面整理**：优化桌面歌词、礼物、点歌板、工具箱和播放页的布局、文案与导航细节。
- 💬 **交互提示与队列细节修正**：调整上下文帮助、队列操作及相关控件行为，减少重复或不必要的界面元素。
- ✅ **回归测试与架构资料同步**：同步更新管理页、桌面歌词、播放页测试及前端架构文档。

## v3.7.5 变更

- 🎵 **歌曲数据与点歌流程增强**：扩展歌曲导入、文件编码、歌单筛选和点歌服务能力，补充歌曲导入表格及价格字段支持。
- 🕒 **时钟覆盖层稳定性修正**：完善时钟覆盖层稳定 URL、服务路由、设置同步和管理页交互，补充相关回归测试。
- 🧰 **工具箱与管理页体验优化**：调整管理页工作区、工具箱、礼物、桌面歌词、主播计划和帮助提示的布局与交互细节。
- 💬 **弹幕与覆盖层细节完善**：更新弹幕排行榜资源、弹幕样式和覆盖层页面表现。
- ✅ **架构资料与测试同步**：同步更新架构文档、规格索引及歌曲、时钟、工具箱和管理页测试。

## v3.7.4 变更

- 💬 **弹幕身份横卡风格完善**：完成风格 4 的身份横卡展示，区分普通用户、舰长、提督和总督身份，优化头像、用户名、正文和配色布局，并补充设置与覆盖层回归测试。
- 🧭 **工具箱导航分组优化**：重新整理工具箱分组和侧栏状态，改善桌面端导航、窄屏布局、面板切换和 URL/选中状态同步。
- 🎵 **播放页交互细节修正**：优化播放页抽屉、队列弹窗、全屏播放器和首页处理逻辑，修正打开、关闭、队列操作及相关页面组合行为。
- 💡 **帮助提示与界面文案统一**：完善上下文帮助组件、弹幕工具和各工具页文案，统一提示位置、可访问属性和交互状态。
- ✅ **测试与治理配置同步**：补充播放页、工具箱、弹幕和 UI 表面测试，并同步架构文档、测试文档及 Sol/Luna 工作流配置。

## v3.7.3 变更

- 💬 **弹幕姬身份横卡风格**：新增风格 4“身份横卡”，以普通用户、舰长、提督、总督四档颜色显示固定尺寸长方形弹幕，用户名与正文在左、头像在右；384×640 设计画布会按浏览器源宽高等比缩放，不拉伸或裁切内容。
- 🕒 **可爱时钟覆盖层**：新增时钟管理卡片与透明覆盖层，支持时间显示、主题样式和独立的 Browser Source 页面，并补充服务路由、样式及回归测试。
- 💡 **管理页可用性与视觉层级优化**：新增上下文帮助提示，统一桌面端字体层级、按钮颜色、面板布局和响应式细节，提升复杂管理页的可读性。
- 🎁 **礼物与边框特效增强**：完善礼物边框资源与独立强调色配置，优化礼物解析、礼物管理页面和特效覆盖层展示。
- 🛡️ **数据与守护礼物稳定性修复**：加强数据库清空失败后的恢复处理、守护礼物记账和 Bilibili 协议解析边界，避免异常状态破坏已有数据。
- ✅ **测试与架构资料同步**：新增时钟、帮助提示、字体层级和数据恢复测试，并同步更新 Admin、Overlay、存储及协议文档。

## v3.7.2 变更

- 🎮 **你画我猜内容扩展**：补充题库分类与词条管理能力，完善游戏会话、路由、设置初始化和覆盖层展示，新增对应的分类、工具与回归测试。
- 🎨 **队列主题设置隔离**：将队列样式设置拆分为独立共享模块，避免不同页面和覆盖层之间互相覆盖主题配置，并同步更新管理页、主题存储、接口和架构文档。
- 💬 **弹幕与游戏工具细节优化**：调整弹幕工具、游戏工具和相关主题样式，修正主题选择、参数控件及窄屏布局下的展示与交互。
- ✅ **回归覆盖补充**：新增并更新弹幕覆盖层、队列覆盖层、游戏路由、游戏会话和管理页测试，覆盖设置边界与页面组合行为。
...
- 🌐 **README 双语化**：README.md 重写为英文版，新增 README.zh-CN.md 中文版，两版互相链接；仓库简介同步新仓库链接与使用声明。
